#!/usr/bin/env bash
# Canonical test runner for nexai-agent. Run this instead of calling
...
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.nexai/nexai-agent/venv"; do
...
# Unset nexai_* behavioral vars too.
unset NEXAI_YOLO_MODE NEXAI_INTERACTIVE NEXAI_QUIET NEXAI_TOOL_PROGRESS \
      NEXAI_TOOL_PROGRESS_MODE NEXAI_MAX_ITERATIONS NEXAI_SESSION_PLATFORM \
      NEXAI_SESSION_CHAT_ID NEXAI_SESSION_CHAT_NAME NEXAI_SESSION_THREAD_ID \
      NEXAI_SESSION_SOURCE NEXAI_SESSION_KEY NEXAI_GATEWAY_SESSION \
      NEXAI_CRON_SESSION \
      NEXAI_PLATFORM NEXAI_INFERENCE_PROVIDER NEXAI_MANAGED NEXAI_DEV \
      NEXAI_CONTAINER NEXAI_EPHEMERAL_SYSTEM_PROMPT NEXAI_TIMEZONE \
      NEXAI_REDACT_SECRETS NEXAI_BACKGROUND_NOTIFICATIONS NEXAI_EXEC_ASK \
      NEXAI_HOME_MODE 2>/dev/null || true
...
# If a system-wide nexai pytest_live_guard plugin is installed at
# $HOME/.nexai/pytest_live_guard.py, force-load it here so every test run
...
if [ -f "$HOME/.nexai/pytest_live_guard.py" ]; then
  case ":${PYTHONPATH:-}:" in
    *":$HOME/.nexai:"*) ;;
    *) export PYTHONPATH="${PYTHONPATH:+$PYTHONPATH:}$HOME/.nexai" ;;
  esac
