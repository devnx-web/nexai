...
VERSION_FILE = REPO_ROOT / "nexai_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "274096618+nexai-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "shannon@devnx.com": "shannonsands",
    ...
    "nexai@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (NEXAI_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (NEXAI_HOME inject)
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "nexai@devnx.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/DevNX/nexai-agent",
    ...
    lines.append(f"# nexai Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for nexai Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="nexai Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  nexai Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"nexai Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"nexai Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'nexai Agent v{new_version} ({calver_date})' "
