# ============================================================================
# nexai Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$SkipSetup,
    [string]$Branch = "main",
    [string]$nexaiHome = "$env:LOCALAPPDATA\nexai",
...
function Install-Git {
    ...
    which nexai uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\nexai\git`` — never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    nexai needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the nexai copy of Git ourselves is predictable and
    ...
    ``NEXAI_GIT_BASH_PATH`` (User scope) so nexai can find it in a fresh
    ...
    # Download PortableGit into $nexaiHome\git.  Always works as long as
    # we can reach github.com — no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found — downloading PortableGit to $nexaiHome\git\ ..."
    ...
    try {
        ...
        $release = Invoke-RestMethod -Uri $releaseApi -UseBasicParsing -Headers @{ "User-Agent" = "nexai-installer" }

        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected — PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent nexai features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  nexai needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``NEXAI_GIT_BASH_PATH`` (User env scope) so nexai can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $nexaiHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $nexaiHome\git\usr\bin\bash.exe
    $candidates += "$nexaiHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$nexaiHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("NEXAI_GIT_BASH_PATH", $candidate, "User")
            $env:NEXAI_GIT_BASH_PATH = $candidate
            Write-Info "Set NEXAI_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe — nexai may not find Git Bash."
    Write-Info "If needed, set NEXAI_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$nexaiHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$nexaiHome\node;$env:Path"
        Write-Success "Node.js $version found (nexai-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # Fallback: download binary zip to ~/.nexai/node/
    ...
    try {
        ...
        if ($zipName) {
            ...
            if ($extractedDir) {
                if (Test-Path "$nexaiHome\node") { Remove-Item -Recurse -Force "$nexaiHome\node" }
                Move-Item $extractedDir.FullName "$nexaiHome\node"
                $env:Path = "$nexaiHome\node;$env:Path"

                $version = & "$nexaiHome\node\node.exe" --version
                Write-Success "Node.js $version installed to ~/.nexai/node/"
                $script:HasNode = $true
    ...
    if (Test-Path $InstallDir) {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                Write-Info "terminals, running nexai processes) and try again."
                throw
            }
        }
    }
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install nexai-agent package even with no extras. Inspect the uv pip install output above."
    }

    # Verify the dashboard deps specifically — they're the most common thing
    # users hit and lazy-import errors from `nexai dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable — `nexai dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `nexai dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up nexai command..."
    
    if ($NoVenv) {
        $nexaiBin = "$InstallDir"
    } else {
        $nexaiBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so nexai is globally available
    # On Windows, the nexai.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$nexaiBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$nexaiBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $nexaiBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set NEXAI_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\nexai instead
    # of the Unix default ~/.nexai
    $currentnexaiHome = [Environment]::GetEnvironmentVariable("NEXAI_HOME", "User")
    if (-not $currentnexaiHome -or $currentnexaiHome -ne $nexaiHome) {
        [Environment]::SetEnvironmentVariable("NEXAI_HOME", $nexaiHome, "User")
        Write-Success "Set NEXAI_HOME=$nexaiHome"
    }
    $env:NEXAI_HOME = $nexaiHome
    
    # Update current session
    $env:Path = "$nexaiBin;$env:Path"
    
    Write-Success "nexai command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.nexai directory structure
    New-Item -ItemType Directory -Force -Path "$nexaiHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$nexaiHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$nexaiHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$nexaiHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$nexaiHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$nexaiHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$nexaiHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$nexaiHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$nexaiHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$nexaiHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.nexai/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.nexai/.env"
        }
    } else {
        Write-Info "~/.nexai/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$nexaiHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.nexai/config.yaml from template"
        }
    } else {
        Write-Info "~/.nexai/config.yaml already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and nexai's prompt-injection scanner
...
"@
        ...
        Write-Success "Created ~/.nexai/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.nexai/"
    
    # Seed bundled skills into ~/.nexai/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.nexai/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.nexai/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$nexaiHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.nexai/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'nexai setup tools' later."
        return
    }
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\nexai-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\nexai-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
                try {
...
function Install-PlatformSdks {
    ...
    # ~/.nexai/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m nexai_cli.main setup
    } else {
        python -m nexai_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $nexaiCmd = "$InstallDir\venv\Scripts\nexai.exe"
    if (-not (Test-Path $nexaiCmd)) {
        $nexaiCmd = "nexai"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$nexaiHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'nexai whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$nexaiHome\logs\gateway.log"
            Start-Process -FilePath $nexaiCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$nexaiHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: nexai gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: nexai gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$nexaiHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$nexaiHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$nexaiHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$nexaiHome\nexai-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   nexai              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   nexai setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   nexai config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   nexai config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   nexai gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   nexai update       " -NoNewline -ForegroundColor Green
