#!/bin/bash
# ============================================================================
# nexai Agent Installer
# ============================================================================
# Installation script for Linux, macOS, and Android/Termux.
# Uses uv for desktop/server installs and Python's stdlib venv + pip on Termux.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/YOUR_ORG/nexai-agent/main/scripts/install.sh | bash
#
# Or with options:
...
# Python-driven tool session (e.g. nexai terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:YOUR_ORG/nexai-agent.git"
REPO_URL_HTTPS="https://github.com/YOUR_ORG/nexai-agent.git"
NEXAI_HOME="${NEXAI_HOME:-$HOME/.nexai}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${NEXAI_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$NEXAI_INSTALL_DIR"
...
#   code at /usr/local/lib/nexai-agent, command at /usr/local/bin/nexai,
#   data still at /root/.nexai (NEXAI_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.nexai/nexai-agent"
            echo "                   default (root, Linux): /usr/local/lib/nexai-agent"
            echo "  --nexai-home PATH  Data directory (default: ~/.nexai, or \$NEXAI_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, nexai installs the code under"
            echo "  /usr/local/lib/nexai-agent and links the command into"
            echo "  /usr/local/bin/nexai (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$NEXAI_HOME"
            echo "  (default /root/.nexai).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$NEXAI_HOME/nexai-agent are preserved in-place."
            exit 0
            ;;
        *)
...
# Decide where the repo checkout + venv live, and where the `nexai` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $NEXAI_HOME/nexai-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $NEXAI_HOME/nexai-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/nexai-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $NEXAI_HOME/nexai-agent — then preserve it)
#
# Always no-op when the user set --dir or $NEXAI_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in NEXAI_HOME.
    if is_termux; then
        INSTALL_DIR="$NEXAI_HOME/nexai-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$NEXAI_HOME/nexai-agent/.git" ]; then
            INSTALL_DIR="$NEXAI_HOME/nexai-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/nexai-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/nexai-agent"
        ROOT_FHS_LAYOUT=true
        log_info "Root install on Linux — using FHS layout"
        log_info "  Code:    $INSTALL_DIR"
        log_info "  Command: /usr/local/bin/nexai"
        log_info "  Data:    $NEXAI_HOME (unchanged)"
        return 0
    fi
...
get_nexai_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/nexai" ]; then
        echo "$link_dir/nexai"
    else
        echo "nexai"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/YOUR_ORG/nexai-agent/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/nexai-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/nexai-uv-installer.$$.sh")"
...
check_node() {
    ...
    if [ -x "$NEXAI_HOME/node/bin/node" ]; then
        export PATH="$NEXAI_HOME/node/bin:$PATH"
        local found_ver=$("$NEXAI_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (nexai-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.nexai/node/..."
    ...
    # Place into ~/.nexai/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$NEXAI_HOME/node"
    mkdir -p "$NEXAI_HOME"
    mv "$extracted_dir" "$NEXAI_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$NEXAI_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$NEXAI_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$NEXAI_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$NEXAI_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$NEXAI_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.nexai/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. nexai install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "nexai Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="nexai-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if nexai behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up nexai command..."

    if [ "$USE_VENV" = true ]; then
        NEXAI_BIN="$INSTALL_DIR/venv/bin/nexai"
    else
        NEXAI_BIN="$(which nexai 2>/dev/null || echo "")"
        if [ -z "$NEXAI_BIN" ]; then
            log_warn "nexai not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$NEXAI_BIN" ]; then
        log_warn "nexai entry point not found at $NEXAI_BIN"
    ...
    # Create a user-facing shim for the nexai command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    cat > "$command_link_dir/nexai" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$NEXAI_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/nexai"
    log_success "Installed nexai launcher → $command_link_display_dir/nexai"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "nexai command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v nexai' \
                >/dev/null 2>&1; then
        ...
        log_info "nexai not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# nexai Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# nexai Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# nexai Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so nexai works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.nexai directory structure (config at top level, code in subdir)
    mkdir -p "$NEXAI_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.nexai/.env (top level, easy to find)
    if [ ! -f "$NEXAI_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$NEXAI_HOME/.env"
            log_success "Created ~/.nexai/.env from template"
        else
            touch "$NEXAI_HOME/.env"
            log_success "Created ~/.nexai/.env"
        fi
    else
        log_info "~/.nexai/.env already exists, keeping it"
    fi

    # Create config.yaml at ~/.nexai/config.yaml (top level, easy to find)
    if [ ! -f "$NEXAI_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$NEXAI_HOME/config.yaml"
            log_success "Created ~/.nexai/config.yaml from template"
        fi
    else
        log_info "~/.nexai/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$NEXAI_HOME/SOUL.md" ]; then
        cat > "$NEXAI_HOME/SOUL.md" << 'SOUL_EOF'
# nexai Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how nexai communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.nexai/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.nexai/"

    # Seed bundled skills into ~/.nexai/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.nexai/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.nexai/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$NEXAI_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$NEXAI_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.nexai/skills/"
        fi
    fi
}
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ubuntu|debian|raspbian|pop|linuxmint|elementary|zorin|kali|parrot)
                ...
                log_info "This is standard Playwright setup — nexai itself does not require root access."
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (nexai --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'nexai setup' after install."
        return 0
    fi
    ...
    # Run nexai setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m nexai_cli.main setup < /dev/tty
    else
        python -m nexai_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$NEXAI_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for nexai to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$NEXAI_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'nexai whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                NEXAI_CMD="$(get_nexai_command_path)"
                $NEXAI_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'nexai whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'nexai gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $NEXAI_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $NEXAI_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: nexai gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: nexai gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $NEXAI_CMD gateway > "$NEXAI_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.nexai/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: nexai gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: nexai gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $NEXAI_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $NEXAI_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $NEXAI_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}nexai${NC}              Start chatting"
    echo -e "   ${GREEN}nexai setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}nexai config${NC}       View/edit configuration"
    echo -e "   ${GREEN}nexai config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}nexai gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}nexai update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'nexai' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'nexai' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'nexai' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
