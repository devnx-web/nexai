...
# Bootstrap Open WebUI against nexai Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.nexai/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-nexai.sh
...
#   NEXAI_API_PORT=8642
#   NEXAI_API_HOST=127.0.0.1
#   NEXAI_API_MODEL_NAME='nexai Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-nexai Agent WebUI}"
...
NEXAI_ENV_FILE="${NEXAI_ENV_FILE:-$HOME/.nexai/.env}"
NEXAI_API_PORT="${NEXAI_API_PORT:-8642}"
NEXAI_API_HOST="${NEXAI_API_HOST:-127.0.0.1}"
NEXAI_API_CONNECT_HOST="${NEXAI_API_CONNECT_HOST:-127.0.0.1}"
NEXAI_API_MODEL_NAME="${NEXAI_API_MODEL_NAME:-nexai Agent}"
NEXAI_API_BASE_URL="http://${NEXAI_API_CONNECT_HOST}:${NEXAI_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-nexai.sh"
LOG_DIR="$HOME/.nexai/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$NEXAI_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$NEXAI_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.nexai.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.nexai</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.nexai"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.nexai"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-nexai.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to nexai Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-nexai.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.nexai/logs/openwebui.log
StandardError=append:%h/.nexai/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-nexai.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$NEXAI_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring nexai API server is configured...'
  upsert_env API_SERVER_ENABLED true "$NEXAI_ENV_FILE"
  upsert_env API_SERVER_HOST "$NEXAI_API_HOST" "$NEXAI_ENV_FILE"
  upsert_env API_SERVER_PORT "$NEXAI_API_PORT" "$NEXAI_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$NEXAI_API_MODEL_NAME" "$NEXAI_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$NEXAI_ENV_FILE"
  ensure_env_permissions

  log 'Restarting nexai gateway so API server settings take effect...'
  nexai gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${NEXAI_API_CONNECT_HOST}:${NEXAI_API_PORT}/health" >/dev/null; then
    log 'nexai API server did not answer on the first check. Trying to start gateway in the background...'
    nohup nexai gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${NEXAI_API_CONNECT_HOST}:${NEXAI_API_PORT}/health" >/dev/null
  ...
  log "nexai API endpoint: ${NEXAI_API_BASE_URL}"
