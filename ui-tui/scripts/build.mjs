...
await build({
  ...
  // Skip the prebuilt @nexai/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@nexai/ink': resolve(root, 'packages/nexai-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The nexai_cli launcher
