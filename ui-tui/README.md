...
React + Ink terminal UI for NexAi. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `NEXAI_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.nexai/.nexai_history` or under `NEXAI_HOME`.
...
```text
ui-tui/
  packages/nexai-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      nexai-ink.d.ts    type declarations for @nexai/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent nexaiCLI subprocess for slash commands
```
