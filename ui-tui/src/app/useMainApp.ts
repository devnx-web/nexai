import { useApp, useHasSelection, useSelection, useStdout, useTerminalTitle, type ScrollBoxHandle } from '@nexai/ink'
...
export function useMainApp(gw: GatewayClient) {
  ...
  useTerminalTitle(model ? `${marker} ${model}${tabCwd ? ` · ${shortCwd(tabCwd, 24)}` : ''}` : 'nexai')
  ...
  const cwd = ui.info?.cwd || process.env.NEXAI_CWD || process.cwd()
