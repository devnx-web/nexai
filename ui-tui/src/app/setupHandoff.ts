import type { RunExternalProcess } from '@nexai/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`nexai ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching nexai: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`nexai ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
