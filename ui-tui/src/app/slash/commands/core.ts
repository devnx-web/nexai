import { forceRedraw } from '@nexai/ink'
    ...
    run: async (arg, ctx) => {
      ...
      if (!arg && ctx.composer.hasSelection) {
        ...
        if (text) {
          ...
          return sys(
            'clipboard copy failed — try NEXAI_TUI_FORCE_OSC52=1 to force the escape sequence; NEXAI_TUI_DEBUG_CLIPBOARD=1 for details'
          )
        }
      }
...

  {
    ...
    run: (arg, ctx) => {
      ...
      const lines = items.map((m, i) => {
        const tag = m.role === 'user' ? `You #${i + 1}` : `nexai #${i + 1}`
