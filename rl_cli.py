...
from nexai_constants import OPENROUTER_BASE_URL, get_nexai_home
...
if tinker_atropos_dir.exists():
    ...
    os.environ['NEXAI_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to nexai-agent directory if submodule not found
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.nexai/config.yaml if not provided)
    ...
    """
    # Load config from ~/.nexai/config.yaml
    config = load_nexai_config()
