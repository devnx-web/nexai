...
session-scoped state file under ``$NEXAI_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $NEXAI_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_nexai_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this nexai install, leave it first —
    ...
    env = os.environ.copy()
    env["NEXAI_MEET_URL"] = url
    env["NEXAI_MEET_OUT_DIR"] = str(out)
    env["NEXAI_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["NEXAI_MEET_HEADED"] = "1"
    if auth_state:
        env["NEXAI_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["NEXAI_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if NEXAI_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["NEXAI_MEET_MODE"] = mode
    if realtime_model:
        env["NEXAI_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["NEXAI_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["NEXAI_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["NEXAI_MEET_REALTIME_KEY"] = realtime_api_key
