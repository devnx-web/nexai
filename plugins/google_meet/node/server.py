...
Launched by ``nexai meet node run``.

Token handling
--------------
On first boot we mint 32 hex chars of entropy and persist them at
``$NEXAI_HOME/workspace/meetings/node_token.json``. Subsequent boots
...
via ``nexai meet node approve <name> <url> <token>``.
...
def _default_token_path() -> Path:
    return Path(get_nexai_home()) / "workspace" / "meetings" / "node_token.json"
...
class NodeServer:
    ...
    def __init__(
        ...
        display_name: str = "nexai-meet-node",
    ) -> None:
        self.host = host
        self.port = port
