...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with nexai-agent)
2. User-installed providers: ``$NEXAI_HOME/plugins/<name>/``
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$NEXAI_HOME/plugins/`` or None if unavailable."""
    try:
        from nexai_constants import get_nexai_home
        d = get_nexai_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$NEXAI_HOME/plugins/<name>/``) directories.  Bundled takes
...
def _load_provider_from_dir(provider_dir: Path) -> Optional["MemoryProvider"]:
    ...
    module_name = f"plugins.memory.{name}" if _is_bundled else f"_nexai_user_memory.{name}"
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    module_name = f"plugins.memory.{active_provider}.cli" if _is_bundled else f"_nexai_user_memory.{active_provider}.cli"
    try:
