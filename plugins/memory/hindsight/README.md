...
nexai spins up a local Hindsight daemon with built-in PostgreSQL. Requires an LLM API key for memory extraction and synthesis. The daemon starts automatically in the background on first use and stops after 5 minutes of inactivity.
...
Daemon startup logs: `~/.nexai/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p nexai ui start
```
...
Config file: `~/.nexai/hindsight/config.json`
...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `nexai` | Memory bank name (static fallback used when `bank_id_template` is unset or resolves empty) |
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `nexai-{profile}` isolates memory per active nexai profile. Empty placeholders collapse cleanly (e.g. `nexai-{user}` with no user becomes `nexai`). |
...
The LLM API key is stored in `~/.nexai/.env` as `HINDSIGHT_LLM_API_KEY`.
