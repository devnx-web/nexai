...
Config (env vars or nexai config.yaml under retaindb:):
...
class _Client:
    ...
    def upload_file(self, data: bytes, filename: str, remote_path: str, mime_type: str, scope: str, project_id: str | None) -> dict:
        ...
        token = self.api_key.replace("Bearer ", "").strip()
        headers = {"Authorization": f"Bearer {token}", "x-sdk-runtime": "nexai-plugin"}
    ...
    def read_file_content(self, file_id: str) -> bytes:
        ...
        resp = requests.get(url, headers={"Authorization": f"Bearer {token}", "x-sdk-runtime": "nexai-plugin"}, timeout=30, allow_redirects=True)
        resp.raise_for_status()
        return resp.content
...
class RetainDBMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        # Project resolution: RETAINDB_PROJECT > nexai-<profile> > "default"
        # If unset, the API auto-creates and uses the "default" project — no config required.
        explicit = os.environ.get("RETAINDB_PROJECT")
        if explicit:
            project = explicit
        else:
            nexai_home = str(kwargs.get("nexai_home", ""))
            profile_name = os.path.basename(nexai_home) if nexai_home else ""
            project = f"nexai-{profile_name}" if (profile_name and profile_name not in {"", ".nexai"}) else "default"

        self._client = _Client(api_key, base_url, project)
        self._session_id = session_id
        self._user_id = kwargs.get("user_id", "default") or "default"
        self._agent_id = kwargs.get("agent_id", "nexai") or "nexai"

        from nexai_constants import get_nexai_home
        nexai_home_path = get_nexai_home()
        db_path = nexai_home_path / "retaindb_queue.db"
        self._queue = _WriteQueue(self._client, db_path)

        # Seed agent identity from SOUL.md in background
        soul_path = nexai_home_path / "SOUL.md"
        if soul_path.exists():
