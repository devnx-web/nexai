...
def _load_supermemory_config(nexai_home: str) -> dict:
    config = _default_config()
    config_path = Path(nexai_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, nexai_home: str) -> None:
    config_path = Path(nexai_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `nexai memory setup`.
        # All other options are documented for $NEXAI_HOME/supermemory.json
    ...
    def save_config(self, values, nexai_home):
        ...
        _save_supermemory_config(sanitized, nexai_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from nexai_constants import get_nexai_home
        self._nexai_home = kwargs.get("nexai_home") or str(get_nexai_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._nexai_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "nexai", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "nexai_memory", "target": target, "type": "explicit_memory"},
