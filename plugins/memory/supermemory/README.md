...
Config file: `$NEXAI_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `nexai` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `nexai-{identity}` → `nexai-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per nexai profile:

```json
{
  "container_tag": "NexAi-{identity}"
}
```

For a profile named `coder`, this resolves to `nexai-coder`. The default profile resolves to `nexai-default`. Without `{identity}`, all profiles share the same container.
