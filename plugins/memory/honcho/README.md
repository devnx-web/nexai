...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/nexai>
...
```bash
nexai honcho setup    # full interactive wizard (cloud or local)
nexai memory setup    # generic picker, also works
```

Or manually:
```bash
nexai config set memory.provider honcho
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$NEXAI_HOME/honcho.json` | Profile-local (isolated nexai instances) |
| 2 | `~/.nexai/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active nexai profile: `nexai` (default) or `nexai.<profile>`.
...
| 4 | `per-session` strategy | nexai session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`nexai-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`nexai`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-nexai-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening nexai in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — nexai session ID (timestamp + hex). Every `nexai` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple nexai profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`nexai`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active nexai profile: `nexai` (default) or `nexai.<profile>` (e.g. `nexai -p coder` → host key `nexai.coder`).
...
| `NEXAI_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `nexai honcho setup` | Full interactive setup wizard |
| `nexai honcho status` | Show resolved config for active profile |
| `nexai honcho enable` / `disable` | Toggle Honcho for active profile |
| `nexai honcho mode <mode>` | Change recall or observation mode |
| `nexai honcho peer --user <name>` | Update user peer name |
| `nexai honcho peer --ai <name>` | Update AI peer name |
| `nexai honcho tokens --context <N>` | Set context token budget |
| `nexai honcho tokens --dialectic <N>` | Set dialectic max chars |
| `nexai honcho map <name>` | Map current directory to a session name |
| `nexai honcho sync` | Create host blocks for all nexai profiles |
