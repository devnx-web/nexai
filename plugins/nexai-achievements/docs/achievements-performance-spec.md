# NexAi Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: nexai-achievements plugin
...
### `GET /api/plugins/nexai-achievements/achievements`
...
### `POST /api/plugins/nexai-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/nexai-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/nexai-achievements/overview`
...
- Persist plugin data under:
  - `~/.nexai/plugins/nexai-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.nexai/plugins/nexai-achievements/scan_checkpoint.json`
...
- `~/.nexai/plugins/nexai-achievements/`
