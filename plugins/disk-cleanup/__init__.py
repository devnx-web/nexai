"""disk-cleanup plugin — auto-cleanup of ephemeral nexai session files.
...
1. ``post_tool_call`` hook — inspects ``write_file`` and ``terminal``
   ...
   under ``NEXAI_HOME`` and tracks them silently.  Zero agent
   compliance required.
...
2. ``on_session_end`` hook — when any test files were auto-tracked
   ...
   logs a single line to ``$NEXAI_HOME/disk-cleanup/cleanup.log``.
...
def _extract_paths_from_terminal(args: Dict[str, Any], result: str) -> Set[str]:
    ...
    if isinstance(cmd, str) and cmd:
        # Tokenise the command — catches `touch /tmp/nexai-x/test_foo.py`
        try:
...
All operations are scoped to NEXAI_HOME and /tmp/nexai-*.
...
def _handle_slash(raw_args: str) -> Optional[str]:
    ...
    if sub == "track":
        ...
        return (
            f"Not tracked (already present, missing, or outside NEXAI_HOME): "
            f"{path_arg}"
        )
...
def register(ctx) -> None:
    ...
    ctx.register_command(
        ...
        description="Track and clean up ephemeral nexai session files.",
    )
