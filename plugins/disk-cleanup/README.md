...
Auto-tracks and cleans up ephemeral files created during NexAi Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$NEXAI_HOME` and `/tmp/nexai-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `NEXAI_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `NEXAI_HOME` or `/tmp/nexai-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$NEXAI_HOME/disk-cleanup/` is itself excluded
- `$NEXAI_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
