"""disk_cleanup — ephemeral file cleanup for nexai Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under NEXAI_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly NEXAI_HOME and /tmp/nexai-*
Never touches: ~/.nexai/logs/ or any system directory.
"""
...
try:
    ...
    def get_nexai_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("NEXAI_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".nexai").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$NEXAI_HOME/logs/``."""
    return get_nexai_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$NEXAI_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under NEXAI_HOME or ``/tmp/nexai-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    nexai_home = get_nexai_home()
    try:
        path.resolve().relative_to(nexai_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/nexai-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("nexai-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside NEXAI_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under NEXAI_HOME (but leave NEXAI_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "nexai-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(nexai_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == nexai_home:
                continue
            try:
                rel_parts = dirpath.relative_to(nexai_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    nexai_home = get_nexai_home()
    try:
        rel = path.resolve().relative_to(nexai_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "nexai-agent",
        }:
            return None
        ...
        # Path isn't under NEXAI_HOME (e.g. /tmp/nexai-*) — fall through.
        pass
