"""
Microsoft Teams platform adapter for nexai Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "nexai"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        nexai_action = data.get("nexai_action", "")
        session_key = data.get("session_key", "")

        if not nexai_action or not session_key:
        ...
        choice = choice_map.get(nexai_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="nexai_approve",
                    data={**btn_data_base, "nexai_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="nexai_approve",
                    data={**btn_data_base, "nexai_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="nexai_approve",
                    data={**btn_data_base, "nexai_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="nexai_approve",
                    data={**btn_data_base, "nexai_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"nexai\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.nexai/.env")
