"""
LINE Messaging API platform adapter for nexai Agent.
...
to nexai Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``NEXAI_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            nexai_home = Path.home().joinpath(".nexai").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``nexai status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``nexai status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``nexai setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.nexai/.env`` via ``nexai_cli.config``.
    """
    print()
    ...
    try:
        from nexai_cli.config import get_env_var, set_env_var
    except ImportError:
        print("nexai_cli.config not available; set LINE_* vars manually in ~/.nexai/.env")
        return
