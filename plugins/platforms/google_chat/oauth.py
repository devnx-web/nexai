...
- Per-user tokens (keyed by sender email):
    ``${NEXAI_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${NEXAI_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${NEXAI_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${NEXAI_HOME}/google_chat_user_oauth_pending.json``
- Shared OAuth client (one per host):
    ``${NEXAI_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's NEXAI_HOME helper so the token follows the user's
# profile (e.g. tests can override via NEXAI_HOME=/tmp/...).
try:
    from nexai_constants import display_nexai_home, get_nexai_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where nexai_constants isn't importable
...
def _nexai_home() -> Path:
    """Resolve NEXAI_HOME at call time (NOT module import).

    Tests and ``NEXAI_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.nexai/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    return _nexai_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _nexai_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _nexai_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _nexai_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _nexai_home() / "google_chat_user_client_secret.json"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Install nexai-agent[google_chat]."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("  pip install 'nexai-agent[google_chat]'")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into NEXAI_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_nexai_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_nexai_home()}/google_chat_user_token.json"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for nexai (native attachment delivery)"
    )
