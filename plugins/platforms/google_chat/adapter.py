...
# ``model_tools`` import time, so ``nexai status``, ``nexai chat``, etc.
...
# ``nexai_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "nexai is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a nexai MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${NEXAI_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _nexai_home = _get_nexai_home()
        except (ModuleNotFoundError, ImportError):
            _nexai_home = _Path.home() / ".nexai"
        self._thread_count_store = _ThreadCountStore(
            _nexai_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "nexai is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("NEXAI_HOME", str(_Path.home() / ".nexai"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${NEXAI_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a nexai MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'nexai is thinking…' marker message.
        ...
        "nexai is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "nexai is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "nexai is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "nexai is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "nexai is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``nexai setup``.

    The setup wizard at ``nexai_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.nexai/.env`` so the next
    gateway restart picks them up.
    """
    from nexai_cli.cli_output import (
        print_info,
        print_success,
        print_warning,
        prompt,
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. nexai-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.nexai/.env")
    print_info("Restart the gateway: nexai gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``nexai cron`` running as a
    separate process from ``nexai gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the nexai plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'nexai-agent[google_chat]'",
        ...
        # the "nexai is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'nexai is thinking…' marker message "
