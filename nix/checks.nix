...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      nexai-agent = self'.packages.default;
      nexaiVenv = nexai-agent.nexaiVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "nexai-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${nexaiVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "nexai-cross-eval" { } (
          if failures != [] then
        ...
        package-contents = pkgs.runCommand "nexai-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${nexai-agent}/bin/nexai || (echo "FAIL: nexai binary missing"; exit 1)
          test -x ${nexai-agent}/bin/nexai-agent || (echo "FAIL: nexai-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${nexai-agent}/bin/nexai version 2>&1 | grep -qi "nexai" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "nexai-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in nexai nexai-agent nexai-acp; do
            test -x ${nexai-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "nexai-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking nexai --help ==="
          ${nexai-agent}/bin/nexai --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${nexai-agent}/bin/nexai --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "nexai-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${nexai-agent}/share/nexai-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${nexai-agent}/share/nexai-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "NEXAI_BUNDLED_SKILLS" ${nexai-agent}/bin/nexai || \
            (echo "FAIL: NEXAI_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: NEXAI_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "nexai-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${nexai-agent}/share/nexai-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${nexai-agent}/share/nexai-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "NEXAI_BUNDLED_PLUGINS" ${nexai-agent}/bin/nexai || \
            (echo "FAIL: NEXAI_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: NEXAI_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-tui = pkgs.runCommand "nexai-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${nexai-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${nexai-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "NEXAI_TUI_DIR" ${nexai-agent}/bin/nexai || \
            (echo "FAIL: NEXAI_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: NEXAI_TUI_DIR set in wrapper"
        ...
        # Verify NEXAI_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        nexai-node = pkgs.runCommand "nexai-node-version" { } ''
          set -e
          echo "=== Checking NEXAI_NODE in wrapper ==="
          grep -q "NEXAI_NODE" ${nexai-agent}/bin/nexai || \
            (echo "FAIL: NEXAI_NODE not set in wrapper"; exit 1)
          echo "PASS: NEXAI_NODE present in wrapper"

          NEXAI_NODE=$(sed -n "s/^export NEXAI_NODE='\(.*\)'/\1/p" ${nexai-agent}/bin/nexai)
          test -x "$NEXAI_NODE" || (echo "FAIL: NEXAI_NODE=$NEXAI_NODE not executable"; exit 1)
          echo "PASS: NEXAI_NODE executable at $NEXAI_NODE"

          NODE_MAJOR=$("$NEXAI_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All NEXAI_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify NEXAI_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "nexai-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(NEXAI_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking NEXAI_MANAGED guards ==="
          check_blocked "config set" ${nexai-agent}/bin/nexai config set model foo
          check_blocked "config edit" ${nexai-agent}/bin/nexai config edit
        ...
        extra-python-packages = let
          ...
          nexaiWithExtra = nexai-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "nexai-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${nexaiWithExtra}/bin/nexai || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${nexaiWithExtra}/bin/nexai || \
          ...
          if grep -q "PYTHONPATH" ${nexai-agent}/bin/nexai; then
        ...
        extra-dependency-groups = let
          nexaiWithGroups = nexai-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "nexai-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${nexaiWithGroups}"
          echo "venv: ${nexaiWithGroups.nexaiVenv}"
        ...
        in pkgs.runCommand "nexai-config-roundtrip" {
          ...
          merge_and_load() {
            local nexai_home="$1"
            export NEXAI_HOME="$nexai_home"
            ${configMergeScript} ${nixSettings} "$nexai_home/config.yaml"
            ${nexaiVenv}/bin/python3 -c '
import json, sys
