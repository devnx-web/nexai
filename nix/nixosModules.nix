# nix/nixosModules.nix — NixOS module for nexai-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: nexai runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $NEXAI_HOME/.env
# and read by nexai at startup — no container recreation needed for env changes.
#
# Tool resolution: the nexai wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.nexai-agent;
    effectivePackage =
    ...
    nexai-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-nexai-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "nexai-config-attrs";
      description = "nexai YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "nexai-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "nexai-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'NEXAI_DOC_EOF'\n${value}\nNEXAI_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "nexai-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/nexai";
    ...
    # nexai user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "nexai-container-entrypoint" ''
      set -eu

      NEXAI_UID="''${NEXAI_UID:?NEXAI_UID must be set}"
      NEXAI_GID="''${NEXAI_GID:?NEXAI_GID must be set}"

      # ── Group: ensure a group with GID=$NEXAI_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$NEXAI_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="nexai"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$NEXAI_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$NEXAI_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$NEXAI_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$NEXAI_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="nexai"
        TARGET_HOME="/home/nexai"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$NEXAI_UID" -g "$NEXAI_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$NEXAI_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$NEXAI_UID:$NEXAI_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure NEXAI_HOME is owned by the target user.
      ...
      if [ -n "''${NEXAI_HOME:-}" ] && [ -d "$NEXAI_HOME" ]; then
        find "$NEXAI_HOME" \! -user "$NEXAI_UID" -exec chown "$NEXAI_UID:$NEXAI_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/nexai-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/nexai-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/nexai ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/nexai
        chmod 0440 /etc/sudoers.d/nexai
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$NEXAI_UID" --regid="$NEXAI_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $NEXAI_HOME/.env.
    ...
    # Default: /var/lib/nexai/workspace → /data/workspace.
  ...
  in {
    options.services.nexai-agent = with lib; {
      enable = mkEnableOption "nexai Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = nexai-agent;
        description = "The nexai-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/nexai";
        description = "State directory. Contains .nexai/ subdir (NEXAI_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative nexai config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $NEXAI_HOME/.env at activation time.
          nexai reads this file on every startup via load_nexai_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $NEXAI_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $NEXAI_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `nexai gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the nexai user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the nexai plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. nexai discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          nexai_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as nexai (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in nexai-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the nexai CLI to environment.systemPackages and export
          NEXAI_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.nexai symlink to the service
            stateDir. These users are automatically added to the nexai group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.nexai-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the nexai CLI to system PATH and export NEXAI_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.nexai/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.NEXAI_HOME = "${cfg.stateDir}/.nexai";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.nexai-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the nexai user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.nexai-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed nexai
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure nexai is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.nexai        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.nexai/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.nexai/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.nexai/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.nexai/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.nexai/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."nexai-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.nexai
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.nexai ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.nexai ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.nexai -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.nexai/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.nexai/$_subdir"
            chmod 2770 "${cfg.stateDir}/.nexai/$_subdir"
            find "${cfg.stateDir}/.nexai/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.nexai/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.nexai/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.nexai/config.yaml
            chmod 0640 ${cfg.stateDir}/.nexai/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.nexai/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.nexai/.managed
          chmod 0644 ${cfg.stateDir}/.nexai/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.nexai/.container-mode <<'NEXAI_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    nexai_bin=${containerDataDir}/current-package/bin/nexai
    NEXAI_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.nexai/.container-mode
            chmod 0644 ${cfg.stateDir}/.nexai/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.nexai/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.nexai";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.nexai" ]; then
                  rm -f "${symlinkPath}"
                  echo "nexai-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.nexai -> stateDir/.nexai for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.nexai";
                target = "${cfg.stateDir}/.nexai";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "nexai-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.nexai/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.nexai/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.nexai/auth.json
              fi
            ''}
          ''}
          ...
          # nexai reads $NEXAI_HOME/.env at startup via load_nexai_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.nexai/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'NEXAI_NIX_ENV_EOF'
    ${envFileContent}
    NEXAI_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.nexai/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.nexai/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.nexai/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.nexai-agent = {
          description = "nexai Agent Gateway";
          ...
          serviceConfig = {
            ...
            # $NEXAI_HOME/.env by the activation script. load_nexai_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/nexai"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the nexai group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.nexai-agent = {
          description = "nexai Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              NEXAI_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              NEXAI_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env NEXAI_UID="$NEXAI_UID" \
                --env NEXAI_GID="$NEXAI_GID" \
                --env NEXAI_HOME=${containerDataDir}/.nexai \
                --env NEXAI_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/nexai gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
