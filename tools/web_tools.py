...
Backend is selected during ``nexai tools`` setup (web.backend in config.yaml).
When available, nexai can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.nexai/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``nexai tools``).
...
def _raise_web_backend_configuration_error() -> None:
    ...
    if managed_nous_tools_enabled():
        message += (
            ...
            "run `nexai tools` and select Nous Subscription as the web provider."
        )
...
def _web_requires_env() -> list[str]:
    ...
    refresh against the NexAi portal on every CLI startup (invoked at
...
def _is_nous_auxiliary_client(client: Any) -> bool:
    """Return True when the resolved auxiliary backend is NexAi Portal."""
    from urllib.parse import urlparse

    base_url = str(getattr(client, "base_url", "") or "")
    host = (urlparse(base_url).hostname or "").lower()
    return host == "devnx.com" or host.endswith(".devnx.com")
...
def _resolve_web_extract_auxiliary(model: Optional[str] = None) -> tuple[Optional[Any], Optional[str], Dict[str, Any]]:
    ...
    if client is not None and _is_nous_auxiliary_client(client):
        ...
        try:
            ...
            nous_portal_tags = lambda: []  # removed with nous provider
...
def _get_exa_client():
    ...
    if _exa_client is None:
        ...
        _exa_client.headers["x-exa-integration"] = "nexai-agent"
    return _exa_client
...
if __name__ == "__main__":
    ...
    if not nous_available:
        ...
        print("Set OPENROUTER_API_KEY, configure NexAi Portal, or set OPENAI_BASE_URL + OPENAI_API_KEY")
