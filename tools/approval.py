...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("NEXAI_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return os.getenv("NEXAI_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set NEXAI_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind NEXAI_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds NEXAI_SESSION_PLATFORM via
    ...
    """
    if os.getenv("NEXAI_CRON_SESSION"):
        return False
...
# via shell expansions like $HOME or $NEXAI_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_NEXAI_ENV_PATH = (
    r'(?:~\/\.nexai/|'
    r'(?:\$home|\$\{home\})/\.nexai/|'
    r'(?:\$nexai_home|\$\{nexai_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bnexai\s+gateway\s+(stop|restart)\b', "stop/restart nexai gateway (kills running agents)"),
    (r'\bnexai\s+update\b', "nexai update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart nexai-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart nexai-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(nexai|gateway|cli\.py)\b', "kill nexai/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill nexai` but not
    # `kill -9 $(pgrep -f nexai)` because the substitution is opaque
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("NEXAI_YOLO_MODE")) or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = os.getenv("NEXAI_INTERACTIVE")
    ...
    if is_gateway or os.getenv("NEXAI_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("NEXAI_YOLO_MODE")) or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = os.getenv("NEXAI_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = os.getenv("NEXAI_EXEC_ASK")
