...
it is automatically downloaded from GitHub releases to $NEXAI_HOME/bin/tirith.
...
def _get_nexai_home() -> str:
    """Return the nexai home directory, respecting NEXAI_HOME env var."""
    return str(get_nexai_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_nexai_home(), ".tirith-install-failed")
...
def _nexai_bin_dir() -> str:
    """Return $NEXAI_HOME/bin, creating it if needed."""
    d = os.path.join(_get_nexai_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $NEXAI_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_nexai_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $NEXAI_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $NEXAI_HOME/bin/tirith
    ...
    nexai_bin = os.path.join(_nexai_bin_dir(), "tirith")
    if os.path.isfile(nexai_bin) and os.access(nexai_bin, os.X_OK):
        _resolved_path = nexai_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return nexai_bin
