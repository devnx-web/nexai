...
Uses the Vercel Python SDK to run commands in cloud sandboxes through nexai'
shared ``BaseEnvironment`` shell contract. When persistence is enabled, the
backend stores task-scoped snapshot metadata under ``NEXAI_HOME`` and restores
...
def _snapshot_store_path() -> Path:
    return get_nexai_home() / _SNAPSHOT_STORE_NAME
...
class VercelSandboxEnvironment(BaseEnvironment):
    ...
    def _configure_attached_sandbox(self, *, requested_cwd: str) -> None:
        ...
        if self._remote_home == "/":
            container_base = "/.nexai"
        else:
            container_base = f"{self._remote_home.rstrip('/')}/.nexai"
    ...
    def _vercel_bulk_download(self, dest_tar_path: Path) -> None:
        remote_nexai = (
            "/.nexai"
            if self._remote_home == "/"
            else f"{self._remote_home.rstrip('/')}/.nexai"
        )
        archive_member = remote_nexai.lstrip("/")
        remote_tar = f"/tmp/.nexai_sync.{os.getpid()}.tar"
        sandbox = self._sandbox
        if sandbox is None:
            raise RuntimeError("Vercel sandbox is not attached")
