"""Central registry for all nexai-agent tools.
...
# timescales. Cache results for ~30 s so env-var flips via ``nexai tools``
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``nexai tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``nexai tools enable foo``)
