...
def _is_nexai_provider_credential(name: str) -> bool:
    """True if ``name`` is a nexai-managed provider credential (API key,
    token, or similar) per ``_NEXAI_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-nexai API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    in the blocklist and remain legitimately registerable — skills that
    wrap third-party APIs still work.
    """
    try:
        from tools.environments.local import _NEXAI_PROVIDER_ENV_BLOCKLIST
    except Exception:
        return False
    return name in _NEXAI_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are nexai-managed provider credentials (from
    ``_NEXAI_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a nexai-managed
    ...
    Non-nexai third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_nexai_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register nexai provider "
                "credential %r (blocked by _NEXAI_PROVIDER_ENV_BLOCKLIST). "
