...
This module provides the glue:
    - ``nexaiTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "nexai Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses NEXAI_HOME so each profile gets its own OAuth tokens.
    Layout: ``NEXAI_HOME/mcp-tokens/``
    """
    try:
        from nexai_constants import get_nexai_home
        base = Path(get_nexai_home())
    except ImportError:
        base = Path(os.environ.get("NEXAI_HOME", str(Path.home() / ".nexai")))
...
# ---------------------------------------------------------------------------
# nexaiTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class nexaiTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        NEXAI_HOME/mcp-tokens/<server_name>.json         -- tokens
        NEXAI_HOME/mcp-tokens/<server_name>.client.json   -- client info
        NEXAI_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # nexai records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to nexai.</p></body></html>"
            ) if code else (
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "nexai Agent")
    scope = cfg.get("scope")
