...
One ``CDPSupervisor`` runs per nexai ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "nexai-dialog-bridge.invalid"
...
(() => {
  if (window.__nexaiDialogBridgeInstalled) return;
  window.__nexaiDialogBridgeInstalled = true;
  const ENDPOINT = "http://nexai-dialog-bridge.invalid/";
