...
transport, discovers their tools, and registers them into the nexai-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.nexai/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.nexai/logs/mcp-stderr.log), tagged with the
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            nexai_home = os.path.expanduser(
                os.getenv(
                    "NEXAI_HOME", os.path.join(os.path.expanduser("~"), ".nexai")
                )
            )
            candidates = [
                os.path.join(nexai_home, "node", "bin", resolved_command),
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → nexai MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that nexai gateways know how to render.
    ...
    async def run(self, config: dict):
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until nexai is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `nexai mcp login {server_name}` (or delete the tokens "
            f"file under ~/.nexai/mcp-tokens/ and restart). Do NOT retry "
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the nexai config file.
    ...
    ``os.environ`` (which includes ``~/.nexai/.env`` loaded at startup).
    """
    try:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through nexai' MEDIA: tag convention and out to messaging
            ...
            # nexai' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves nexai's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the nexai registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``nexai tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the nexai registry.
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple nexai
