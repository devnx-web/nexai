...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/nexai-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    nexai_meta = (fm.get("metadata") or {}).get("nexai") or {}
    tags = nexai_meta.get("tags") or []
    related = nexai_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `nexai skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with nexai Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "nexai ships with a large built-in skill library copied into `~/.nexai/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "nexai also syncs bundled skills on `nexai update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.nexai/skills/` tree, it is still shipped with nexai; restore it with `nexai skills reset <name> --restore`.",
        "",
