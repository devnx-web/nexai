#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the nexai docs site.
...
Both publish at:
  https://nexai-agent.nousresearch.com/docs/llms.txt
  https://nexai-agent.nousresearch.com/docs/llms-full.txt
...
SITE_BASE = "https://nexai-agent.nousresearch.com/docs"
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use nexai as a Python Library", None),
        ("guides/use-mcp-with-nexai", "Use MCP with nexai", None),
        ("guides/use-voice-mode-with-nexai", "Use Voice Mode with nexai", None),
        ("guides/use-soul-with-nexai", "Use SOUL.md with nexai", None),
        ("guides/build-a-nexai-plugin", "Build a nexai Plugin", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with nexai"),
...
def emit_llms_index() -> str:
    ...
    lines: list[str] = []
    lines.append("# nexai Agent")
    lines.append("")
    lines.append(
        "> The self-improving AI agent built by DevNX. A terminal-native "
    ...
    lines.append(
        ...
        "nexai-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/NousResearch/nexai-agent")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# nexai Agent — Full Documentation\n",
        (
            "This file is the entire nexai Agent documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using nexai, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://nexai-agent.nousresearch.com/docs\n",
        "Short index: https://nexai-agent.nousresearch.com/docs/llms.txt\n",
        "\n---\n\n",
    ]
