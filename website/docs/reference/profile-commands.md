...
This page covers all commands related to [nexai profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `nexai profile` without a subcommand shows help.
...
```bash
nexai profile use <name>
```

Sets `<name>` as the active profile. All subsequent `nexai` commands (without `-p`) will use this profile.
...
nexai profile use default
```

## `nexai profile create`

```bash
nexai profile create <name> [options]
```
...
nexai profile create mybot

# Clone config only from current profile
nexai profile create work --clone

# Clone everything from current profile
nexai profile create backup --clone-all

# Clone config from a specific profile
nexai profile create work2 --clone --clone-from work
```

## `nexai profile delete`

```bash
nexai profile delete <name> [options]
```
...
```bash
nexai profile delete mybot
nexai profile delete mybot --yes
```
...
```bash
nexai profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's nexai home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ nexai profile show work
Profile: work
Path:    ~/.nexai/profiles/work
...
```bash
nexai profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your nexai installation.
...
```bash
nexai profile alias work
# Creates/updates ~/.local/bin/work

nexai profile alias work --name mywork
# Creates ~/.local/bin/mywork

nexai profile alias work --remove
# Removes the wrapper script
```

## `nexai profile rename`

```bash
nexai profile rename <old-name> <new-name>
```
...
```bash
nexai profile rename mybot assistant
# ~/.nexai/profiles/mybot → ~/.nexai/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `nexai profile export`

```bash
nexai profile export <name> [options]
```
...
```bash
nexai profile export work
# Creates work.tar.gz in the current directory

nexai profile export work -o ./work-2026-03-29.tar.gz
```

## `nexai profile import`

```bash
nexai profile import <archive> [options]
```
...
```bash
nexai profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

nexai profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`nexai profile export` / `import` are still the right commands for
...
### `nexai profile install`

```bash
nexai profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `nexai -p telemetry`). |
...
nexai profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
nexai profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
nexai profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
nexai profile install ./telemetry/
```

### `nexai profile update`

```bash
nexai profile update <name> [--force-config] [--yes]
```
...
```bash
nexai profile info <name>
```

Prints the profile's distribution manifest — name, version, required
nexai version, author, env var requirements, the source URL/path, and
...
`nexai profile list` also shows the distribution name and version in a
`Distribution` column, and `nexai profile show <name>` / `delete <name>`
...
nexai profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
nexai profile install https://github.com/your-org/internal-assistant.git
```
...
`nexai_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
nexai version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host nexai can clone from.
3. Tell recipients to run `nexai profile install <your-repo-url>`.
...
## `nexai -p` / `nexai --profile`

```bash
nexai -p <name> <command> [options]
nexai --profile <name> <command> [options]
```

Global flag to run any nexai command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
nexai -p work chat -q "Check the server status"
nexai --profile dev gateway start
nexai -p personal skills list
nexai -p work config edit
```

## `nexai completion`

```bash
nexai completion <shell>
```
...
- `nexai profile <TAB>` — subcommands (list, use, create, etc.)
- `nexai profile use <TAB>` — profile names
- `nexai -p <TAB>` — profile names
