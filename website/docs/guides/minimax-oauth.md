...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in NexAi Agent — no API key required"
---

# MiniMax OAuth

NexAi Agent supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and nexai automatically refreshes your session.
...
# → nexai opens your browser to the MiniMax authorization page
...
After the first login, credentials are stored under `~/.nexai/auth.json` and are refreshed automatically before each session.
...
```bash
nexai auth add minimax-cn --type oauth   # if OAuth is supported on your CN account
# or simpler:
echo 'MINIMAX_CN_API_KEY=your-key' >> ~/.nexai/.env
```
...
```bash
nexai auth add minimax-oauth --no-browser
```

nexai will print the verification URL and user code — open the URL on any device and enter the code when prompted.

## The OAuth Flow

nexai implements a PKCE device-code flow against the MiniMax OAuth endpoints:

1. nexai generates a PKCE verifier / challenge pair and a random state value.
...
4. nexai polls `{base_url}/oauth/token` until the token arrives (or the deadline passes).
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.nexai/auth.json` under the `minimax-oauth` key.
...
```bash
nexai config set model MiniMax-M2.7
nexai config set provider minimax-oauth
```

## Configuration Reference

After login, `~/.nexai/config.yaml` will contain entries similar to:
...
```bash
nexai --provider minimax-oauth    # canonical
nexai --provider minimax-portal   # alias
nexai --provider minimax-global   # alias
nexai --provider minimax_oauth    # alias (underscore form)
```
...
```bash
NEXAI_INFERENCE_PROVIDER=minimax-oauth nexai
```
...
nexai refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, nexai marks the session as requiring re-login.

**Fix:** run `nexai auth add minimax-oauth` again to start a fresh login.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, nexai raises a timeout error.

**Fix:** re-run `nexai auth add minimax-oauth` (or `nexai model`). The flow starts fresh.

### State mismatch (possible CSRF)

nexai detected that the `state` value returned by the authorization server does not match what it sent.
...
If `nexai` cannot open a browser window, use `--no-browser`:

```bash
nexai auth add minimax-oauth --no-browser
```

nexai prints the URL and code. Open the URL on any device and complete the flow there.
...
**Fix:** run `nexai model` and select MiniMax (OAuth), or run `nexai auth add minimax-oauth`.
...
```bash
nexai auth remove minimax-oauth
```
...
- [nexai doctor](../reference/cli-commands.md)
