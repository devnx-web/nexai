---
sidebar_position: 6
title: "Use MCP with NexAi"
description: "A practical guide to connecting MCP servers to NexAi Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with NexAi

This guide shows how to actually use MCP with NexAi Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native nexai tool
- you want nexai to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect nexai to internal APIs, databases, or company systems without modifying nexai core

Do not use MCP when:
- a built-in nexai tool already solves the job well
...
- nexai remains the agent
- MCP servers contribute tools
- nexai discovers those tools at startup or reload time
...
If you installed nexai with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- nexai banner/status should show MCP integration when configured
- ask nexai what tools it has available
...
## WSL2: bridge nexai in WSL to Windows Chrome

This is the practical setup when:

- nexai runs inside WSL2
...
In this setup, nexai does **not** connect to Chrome directly. Instead:

- nexai runs in WSL
- nexai starts a local stdio MCP server
...
```text
nexai (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- nexai stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on nexai core browser transport
...
```bash
nexai mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
nexai mcp test chrome-devtools-win
```

Then start a fresh nexai session or run:
...
Once loaded, nexai can use the MCP-prefixed browser tools directly. For example:
...
If nexai runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start nexai from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start nexai from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in nexai:
...
2. nexai-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So nexai will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want nexai to reason over a bounded workspace.
...
Now nexai can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing nexai core.
...
Because nexai now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
