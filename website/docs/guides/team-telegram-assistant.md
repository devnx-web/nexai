...
This tutorial walks you through setting up a Telegram bot powered by NexAi Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **NexAi Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.nexai/.env`

:::tip
A $5/month VPS is plenty for running the gateway. nexai itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team nexai Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_nexai_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by NexAi Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting nexai Gateway...
...
# View live logs
journalctl --user -u nexai-gateway -f
...
nexai gateway start
nexai gateway stop
tail -f ~/.nexai/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `nexai gateway install` to update the plist.
:::
...
```bash
nexai gateway stop && nexai gateway start
```
...
nexai pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.nexai/.env`:
...
Control how much detail the bot shows when using tools. In `~/.nexai/config.yaml`:
...
Customize how the bot communicates by editing `~/.nexai/SOUL.md`:

For a full guide, see [Use SOUL.md with NexAi](/docs/guides/use-soul-with-nexai).
...
```bash
# From the CLI
nexai cron list          # View all scheduled jobs
nexai cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u nexai-gateway` (Linux) or `~/.nexai/logs/gateway.log` (macOS) |
| Cron job output | `~/.nexai/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.nexai/cron/jobs.json` |
| Pairing data | `~/.nexai/pairing/` |
| Session history | `~/.nexai/sessions/` |
