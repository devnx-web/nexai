---
sidebar_position: 7
title: "Use SOUL.md with NexAi"
description: "How to use SOUL.md to shape NexAi Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with NexAi

`SOUL.md` is the **primary identity** for your nexai instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want nexai to feel like the same assistant every time you talk to it — or if you want to replace the NexAi persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm nexai should be
- what nexai should avoid stylistically
- how nexai should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who nexai is and how nexai speaks
...
nexai now uses only the global SOUL file for the current instance:

```text
~/.nexai/SOUL.md
```

If you run nexai with a custom home directory, it becomes:
...
nexai automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, nexai does not overwrite it
- if the file exists but is empty, nexai adds nothing from it to the prompt

## How nexai uses it

When nexai starts a session, it reads `SOUL.md` from `NEXAI_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how nexai feels.
...
nexai already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How nexai should sound.

# Avoid
What nexai should not do.

# Defaults
How nexai should behave when ambiguity appears.
```
...
Then restart nexai or start a new session.
...
### I edited SOUL.md but nexai still sounds the same

Check:
- you edited `~/.nexai/SOUL.md` or `$NEXAI_HOME/SOUL.md`
...
### NexAi is ignoring parts of my SOUL.md
