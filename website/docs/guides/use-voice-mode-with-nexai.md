---
sidebar_position: 8
title: "Use Voice Mode with NexAi"
description: "A practical guide to setting up and using nexai voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with NexAi
...
- you want nexai sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in nexai.
...
## Step 1: make sure normal nexai works first
...
nexai supports both local and cloud speech stacks.
...
### If you use `nexai setup`

If you choose NeuTTS in the setup wizard, nexai checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. nexai transcribes and responds
...
- asking nexai to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full nexai loop.

## Tuning CLI behavior

### Silence threshold

If nexai starts/stops too aggressively, tune:
...
nexai stays a normal chat bot, but can speak replies.
...
- you want nexai to function like a portable research or ops assistant
...
nexai joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- nexai detects speech boundaries
- transcripts are posted in the associated text channel
- nexai responds in text and audio
...
1. get text nexai working
2. install `nexai-agent[voice]`
