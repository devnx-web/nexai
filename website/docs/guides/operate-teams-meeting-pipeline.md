...
```bash
nexai teams-pipeline token-health
nexai teams-pipeline token-health --force-refresh
```
...
```bash
nexai teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
nexai teams-pipeline maintain-subscriptions
nexai teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: nexai cron (recommended if you already run the NexAi gateway)

nexai ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.nexai/scripts/`. First create the script:

```bash
mkdir -p ~/.nexai/scripts
cat > ~/.nexai/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec nexai teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.nexai/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
nexai cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/nexai-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=nexai Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=nexai
EnvironmentFile=/etc/nexai/env
ExecStart=/usr/local/bin/nexai teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/nexai-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run nexai Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now nexai-teams-pipeline-maintain.timer
systemctl list-timers nexai-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/nexai teams-pipeline maintain-subscriptions >> /var/log/nexai/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.nexai/.env` at the top of a wrapper script that crontab calls.
...
```bash
nexai teams-pipeline subscriptions   # should show expirationDateTime advanced
nexai teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
nexai teams-pipeline list
nexai teams-pipeline list --status failed
nexai teams-pipeline show <job-id>
```

### Replay a stored job

```bash
nexai teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
nexai teams-pipeline fetch --meeting-id <meeting-id>
nexai teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `nexai teams-pipeline maintain-subscriptions --dry-run`
- inspect `nexai teams-pipeline list --status failed`
...
- run `nexai teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `nexai teams-pipeline run`
...
- [ ] `nexai teams-pipeline validate` returns an OK snapshot
- [ ] `nexai teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (nexai cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
