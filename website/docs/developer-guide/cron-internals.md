...
description: "How nexai stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `nexai_cli/cron.py` | CLI `nexai cron` subcommands |
...
Jobs are stored in `~/.nexai/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `nexai cron` commands are run or during active CLI sessions.
...
```python
# ~/.nexai/scripts/check_competitors.py
...
2. **Environment variable** — `NEXAI_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.nexai/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `nexai cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `nexai cron` CLI provides direct job management:

```bash
nexai cron list                    # Show all jobs
nexai cron create                  # Interactive job creation (alias: add)
nexai cron edit <job_id>           # Edit job configuration
nexai cron pause <job_id>          # Pause a running job
nexai cron resume <job_id>         # Resume a paused job
nexai cron run <job_id>            # Trigger immediate execution
nexai cron remove <job_id>         # Delete a job
```
