...
NexAi Agent uses a SQLite database (`~/.nexai/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `nexai_state.py`


## Architecture Overview

```
~/.nexai/state.db (SQLite, WAL mode)
...
Multiple nexai processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.nexai/state.db
...
Default path: `~/.nexai/state.db`

This is derived from `nexai_constants.get_nexai_home()` which resolves to
`~/.nexai/` by default, or the value of `NEXAI_HOME` environment variable.
