...
description: "Building RL training environments, running evaluation benchmarks, and generating SFT data with the NexAi-Agent Atropos integration"
---

# Environments, Benchmarks & Data Generation

NexAi Agent includes a full environment framework that connects its tool-calling capabilities to the [Atropos](https://github.com/NousResearch/atropos) RL training framework. This enables three workflows:
...
The Python environment framework documented here lives under the repo's `environments/` directory and is the implementation-level API for NexAi/Atropos integration. This is separate from the user-facing `rl_*` tools, which operate as an orchestration surface for remote RL training workflows.
:::
...
classDiagram
    ...
    BaseEnv <|-- nexaiAgentBaseEnv
    nexaiAgentBaseEnv <|-- TerminalTestEnv
    nexaiAgentBaseEnv <|-- nexaiSweEnv
    nexaiAgentBaseEnv <|-- TerminalBench2EvalEnv
...
The NexAi-agent layer (`environments/nexai_base_env.py`). Adds:
- **Terminal backend configuration** — sets `TERMINAL_ENV` for sandboxed execution (local, Docker, Modal, Daytona, SSH, Singularity)
- **Tool resolution** — `_resolve_tools_for_group()` calls nexai-agent's `get_tool_definitions()` to get the right tool schemas based on enabled/disabled toolsets
- **Agent loop integration** — `collect_trajectory()` runs `nexaiAgentLoop` and scores the result
...
Your environment inherits from `nexaiAgentBaseEnv` and implements five methods:
...
`nexaiAgentLoop` (`environments/agent_loop.py`) is the reusable multi-turn agent engine. It runs the same tool-calling pattern as nexai-agent's main loop:
...
| **Generic** | `call_tool(name, args)` — escape hatch for any nexai-agent tool |
...
parser = get_parser("NexAi")  # or "mistral", "llama3_json", "qwen", "deepseek_v3", etc.
content, tool_calls = parser.parse(raw_model_output)
```

Available parsers: `nexai`, `mistral`, `llama3_json`, `llama4_json`, `qwen`, `qwen3_coder`, `deepseek_v3`, `deepseek_v3_1` (alias `deepseek_v31`), `kimi_k2`, `longcat`, `glm45`, `glm47`.
...
pip install "NexAi-agent[yc-bench]"
...
```bash
python environments/nexai_swe_env/nexai_swe_env.py serve \
...
- Set `tool_call_parser` in config to match your model's format (e.g., `"NexAi"`, `"qwen"`, `"mistral"`)
...
```python
from environments.nexai_base_env import nexaiAgentBaseEnv, nexaiAgentEnvConfig
from atroposlib.envs.server_handling.server_manager import APIServerConfig

class MyEnvConfig(nexaiAgentEnvConfig):
    my_custom_field: str = "default_value"

class MyEnv(nexaiAgentBaseEnv):
    name = "my-env"
...
### NexAiAgentEnvConfig Fields

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled_toolsets` | `List[str]` | `None` (all) | Which nexai toolsets to enable |
...
| `tool_call_parser` | `str` | `"NexAi"` | Parser for Phase 2 raw output |
...
env:
  ...
  tokenizer_name: "NousResearch/nexai-3-Llama-3.1-8B"
  use_wandb: true
...
- [Modal](https://modal.com) account and CLI: `pip install "NexAi-agent[modal]"`
- `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET` environment variables

### For YC-Bench

- `pip install "NexAi-agent[yc-bench]"` (installs the yc-bench CLI + SQLAlchemy)
...
```
environments/
├── nexai_base_env.py          # Abstract base class (nexaiAgentBaseEnv)
├── agent_loop.py               # Multi-turn agent engine (nexaiAgentLoop)
├── tool_context.py             # Per-rollout tool access for reward functions
├── patches.py                  # Async-safety patches for Modal backend
│
├── tool_call_parsers/          # Phase 2 client-side parsers
│   ├── nexai_parser.py        # NexAi/ChatML <tool_call> format
...
├── nexai_swe_env/             # SWE-bench training environment
│
