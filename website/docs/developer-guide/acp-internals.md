...
The ACP adapter wraps nexai' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
nexai acp / nexai-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> load ~/.nexai/.env
  -> configure stderr logging
  -> construct nexaiACPAgent
...
- `allow_once` -> nexai `once`
- `allow_always` -> nexai `always`
- reject options -> nexai `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps nexai tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["NexAi-acp"])
...
Instead it reuses nexai' runtime resolver:

- `acp_adapter/auth.py`
- `nexai_cli/runtime_provider.py`

So ACP advertises and uses the currently configured nexai provider/credentials.
...
- ACP sessions are persisted to the shared `~/.nexai/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `nexai-acp` toolset definition
- `nexai_cli/main.py` — `nexai acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `nexai-acp` script
