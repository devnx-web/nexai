...
[PR #12550](https://github.com/NousResearch/nexai-agent/pull/12550) proposed a
...
(`nexai-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
One `asyncio.Task` running in a background daemon thread per nexai `task_id`.
...
- `toolsets.py` — register `browser_dialog` in `browser`, `nexai-acp`, `nexai-api-server`, core toolsets (gated on CDP reachability)
...
- `nexai_cli/config.py` — add `browser.dialog_policy` and `browser.dialog_timeout_s` to `DEFAULT_CONFIG`
