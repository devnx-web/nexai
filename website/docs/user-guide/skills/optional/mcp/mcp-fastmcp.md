...
| | |
|---|---|
| Source | Optional — install with `nexai skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into nexai or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to nexai. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.nexai/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.nexai/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is nexai integration, either:

- configure the server in `~/.nexai/config.yaml` using the `native-mcp` skill, or
...
### NexAi cannot see the deployed server

The server-building part may be correct while the NexAi config is not. Load the `native-mcp` skill and configure the server in `~/.nexai/config.yaml`, then restart nexai.
