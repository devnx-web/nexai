---
title: "NexAi Atropos Environments — Build, test, and debug NexAi Agent RL environments for Atropos training"
sidebar_label: "NexAi Atropos Environments"
description: "Build, test, and debug NexAi Agent RL environments for Atropos training"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# NexAi Atropos Environments

Build, test, and debug NexAi Agent RL environments for Atropos training. Covers the NexAiAgentBaseEnv interface, reward functions, agent loop integration, evaluation with tools, wandb logging, and the three CLI modes (serve/process/evaluate). Use when creating, reviewing, or fixing RL environments in the NexAi-agent repo.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `nexai skills install official/mlops/nexai-atropos-environments` |
| Path | `optional-skills/mlops/nexai-atropos-environments` |
| Version | `1.1.0` |
| Author | NexAi Agent |
| License | MIT |
...
Guide for building RL environments in the NexAi-agent repo that integrate with the Atropos training framework.
...
Atropos BaseEnv (atroposlib/envs/base.py)
    └── nexaiAgentBaseEnv (environments/nexai_base_env.py)
...
nexai environments are special because they run a **multi-turn agent loop with tool calling** — not just single-turn completions. The base env handles the loop; you implement the task and scoring.

## File Locations

| File | Purpose |
|------|---------|
| `environments/nexai_base_env.py` | Base class with agent loop + tool resolution |
| `environments/agent_loop.py` | `nexaiAgentLoop` + `AgentResult` dataclass |
| `environments/tool_context.py` | `ToolContext` for reward verification |
| `environments/tool_call_parsers.py` | Phase 2 tool call parsers (nexai, mistral, etc.) |
...
The whole point of NexAi-agent environments is agentic evaluation:

```python
async def evaluate(self, *args, **kwargs) -> None:
    import time, uuid
    from environments.agent_loop import nexaiAgentLoop
...
2. **evaluate() must use nexaiAgentLoop, not chat_completion** — Single-turn chat_completion has no tools. The whole point of NexAi-agent benchmarks is agentic evaluation with tool use.
