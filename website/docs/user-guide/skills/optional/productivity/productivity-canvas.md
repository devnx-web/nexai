...
| | |
|---|---|
| Source | Optional — install with `nexai skills install official/productivity/canvas` |
...
4. Name the token (e.g., "NexAi Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.nexai/.env`:
...
```bash
CANVAS="python $NEXAI_HOME/skills/productivity/canvas/scripts/canvas_api.py"
