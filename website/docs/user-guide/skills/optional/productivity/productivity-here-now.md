...
| | |
|---|---|
| Source | Optional — install with `nexai skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${NEXAI_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${NEXAI_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${NEXAI_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client nexai
```
...
```bash
PUBLISH="${NEXAI_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client nexai
```
...
```bash
DRIVE="${NEXAI_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `nexai`)    |
