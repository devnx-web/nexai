...
| | |
|---|---|
| Source | Optional — install with `nexai skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by NexAi Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during nexai terminal calls (desktop app flow only)
...
### Service Account (recommended for NexAi)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.nexai/.env` (the skill will prompt for this on first load).
...
## NexAi Execution Pattern (desktop app flow)

nexai terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/nexai-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/nexai-op.sock"
