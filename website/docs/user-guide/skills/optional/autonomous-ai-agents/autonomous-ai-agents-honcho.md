...
description: "Configure and use Honcho memory with NexAi -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with NexAi -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `nexai skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for NexAi

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every nexai profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/nexai#running-honcho-locally-with-nexai

### Verify

```bash
nexai honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this nexai profile's AI peer
...
Honcho models conversations as interactions between **peers**. nexai creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this nexai instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each nexai run |
| `global` | Single session across all directories |

Manual override: `nexai honcho map my-project-name`
...
Each nexai profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block nexai.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `nexai.coder` host block in `honcho.json`
...
```bash
nexai honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for NexAi when Honcho memory is active.
...
Config file: `$NEXAI_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `nexai honcho setup`. Ensure `memory.provider: honcho` is in `~/.nexai/config.yaml`.

### Memory not persisting across sessions
Check `nexai honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `nexai profile create <name> --clone`. For existing profiles: `nexai honcho sync`.
...
| Command | Description |
|---------|-------------|
| `nexai honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `nexai honcho status` | Show resolved config, connection test, peer info for active profile |
| `nexai honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `nexai honcho disable` | Disable Honcho for the active profile |
| `nexai honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `nexai honcho peers` | Show peer identities across all profiles |
| `nexai honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `nexai honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `nexai honcho sessions` | List known directory-to-session-name mappings |
| `nexai honcho map <name>` | Map current working directory to a Honcho session name |
| `nexai honcho identity` | Seed AI peer identity or show both peer representations |
| `nexai honcho sync` | Create host blocks for all nexai profiles that don't have one yet |
| `nexai honcho migrate` | Step-by-step migration guide from OpenClaw native memory to nexai + Honcho |
| `nexai memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `nexai memory status` | Show active memory provider and config |
| `nexai memory off` | Disable external memory provider |
