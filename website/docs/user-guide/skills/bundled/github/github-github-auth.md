...
- Give it a name like "NexAi-agent"
- Select scopes:
...
- Give it a title like "NexAi-agent-&lt;machine-name>"
...
elif [ -f ~/.nexai/.env ] && grep -q "^GITHUB_TOKEN=" ~/.nexai/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.nexai/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
