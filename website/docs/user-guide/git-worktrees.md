...
title: "Git Worktrees"
description: "Run multiple nexai agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

NexAi Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with NexAi so each session has a clean, isolated working directory.

## Why Use Worktrees with NexAi?

nexai treats the **current working directory** as the project root:

- CLI: the directory where you run `nexai` or `nexai chat`
...
git worktree add ../repo-feature feature/nexai-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/nexai-experiment` checked out in that directory

Now you can `cd` into the new worktree and run nexai there:

```bash
cd ../repo-feature

# Start nexai in the worktree
nexai
```
...
git worktree add ../repo-experiment-a feature/nexai-a
git worktree add ../repo-experiment-b feature/nexai-b
```
...
- Works on its own branch (`feature/nexai-a` vs `feature/nexai-b`).
...
- nexai checkpoint data under `~/.nexai/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per nexai experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/nexai-checkpoints-docs`, `feature/nexai-refactor-tests`.
- **Commit frequently**
...
- **Avoid running nexai from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `nexai -w` (Automatic Worktree Mode)

nexai has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `nexai/nexai-<hash>`).
...
```bash
nexai -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `nexai -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each nexai session its own clean checkout.
