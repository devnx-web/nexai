...
sidebar_position: 4
title: "Slack"
description: "Set up NexAi Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect NexAi Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your nexai instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. nexai uses the modern
...
The fastest path is to paste a manifest nexai generates for you. It
...
### Option A: From a nexai-generated manifest (recommended)

1. Generate the manifest:
   ```bash
   nexai slack manifest --write
   ```
   This writes `~/.nexai/slack-manifest.json` and prints paste-in
   instructions.
...
4. Enter an app name (e.g., "NexAi Agent") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, nexai can chat but **cannot reliably read user-uploaded attachments**.
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `nexai-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a nexai configuration issue.
:::
...
nexai uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
nexai gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
nexai gateway              # Foreground
nexai gateway install      # Install as a user service
sudo nexai gateway install --system   # Linux only: boot-time system service
```
...
Every nexai command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
nexai command with its description.

Under the hood: nexai ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/NousResearch/nexai-agent/blob/main/nexai_cli/commands.py)
...
When nexai adds new commands (e.g. after `nexai update`), regenerate
...
3. Paste the new contents of `~/.nexai/slack-manifest.json`
4. **Save**. Slack will prompt to reinstall the app if scopes or slash
   commands changed.

### Legacy `/nexai <subcommand>` still works

For backward compatibility with older manifests, you can still type
`/nexai btw run the tests` — nexai routes it the same way as `/btw
run the tests`. Free-form questions also work: `/nexai what's the
...
```bash
nexai slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how nexai behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@NexAi Agent what time is it?`). In channels, nexai replies in a thread attached to that message. |
| **Threads** | If you @mention nexai inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.nexai/config.yaml`.
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to nexai in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before nexai will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where nexai will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @NexAi Agent`).

---

## Multi-Workspace Support

nexai can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, nexai also loads tokens from an **OAuth token file** at:

```
~/.nexai/slack_tokens.json
```
...
- When a message arrives, nexai uses the correct workspace-specific client to respond.
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @NexAi Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @NexAi Agent` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. nexai now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @NexAi Agent`)
...
- Tokens should be stored in `~/.nexai/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your nexai config directory
