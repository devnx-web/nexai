...
description: "Set up NexAi Agent as a Microsoft Teams bot"
---

# Microsoft Teams Setup

Connect NexAi Agent to Microsoft Teams as a bot. Unlike Slack's Socket Mode, Teams delivers messages by calling a **public HTTPS webhook**, so your instance needs a publicly reachable endpoint — either a dev tunnel (local dev) or a real domain (production).
...
Teams delivers @mentions as regular messages with `<at>BotName</at>` tags, which nexai strips automatically before processing.
...
```bash
# devtunnel (Microsoft)
devtunnel create nexai-bot --allow-anonymous
devtunnel port create nexai-bot -p 3978 --protocol https  # replace 3978 with TEAMS_PORT if changed
devtunnel host nexai-bot
...
```bash
NEXAI_UID=$(id -u) NEXAI_GID=$(id -g) docker compose up -d gateway
```
...
Alternatively, configure via `~/.nexai/config.yaml`:
...
| `No inference provider configured` | Check that `ANTHROPIC_API_KEY` (or another provider key) is set in `~/.nexai/.env` |
...
| Tunnel URL changes on restart | devtunnel URLs are persistent if you use a named tunnel (`devtunnel create nexai-bot`). ngrok and cloudflared generate a new URL each run unless you have a paid plan — update the bot endpoint with `teams app update` when it changes |
| Teams shows "This bot is not responding" | The webhook returned an error. Check `docker logs nexai` for tracebacks |
...
- Store credentials in `~/.nexai/.env` with permissions `600` (`chmod 600 ~/.nexai/.env`)
