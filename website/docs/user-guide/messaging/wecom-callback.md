...
Connect nexai to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
nexai supports two WeCom integration modes:
...
3. nexai decrypts the message, queues it for the agent
...
(Use `nexai gateway start` only after `nexai gateway install` has registered the systemd/launchd service.)
