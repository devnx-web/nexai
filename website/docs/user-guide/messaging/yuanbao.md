...
sidebar_position: 16
title: "Yuanbao"
description: "Connect NexAi Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect nexai to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.nexai/.env`:
...
All standard nexai commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.nexai/logs/gateway.log`
...
4. Consider enabling verbose logging: `NEXAI_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. nexai automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
NEXAI_LOG_LEVEL=debug nexai gateway
```
...
```bash
nexai chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
