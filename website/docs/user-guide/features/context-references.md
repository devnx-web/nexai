...
Type `@` followed by a reference to inject content directly into your message. nexai expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- nexai env: `$NEXAI_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$NEXAI_HOME/skills/.hub/`
