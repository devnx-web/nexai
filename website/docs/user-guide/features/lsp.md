...
nexai runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. nexai
...
1. nexai captures a baseline of current diagnostics for the file.
...
…). nexai auto-detects the binary on PATH or in
`<NEXAI_HOME>/lsp/bin/`.
...
`node_modules` tree — nexai installs both packages together when you
run `nexai lsp install typescript` or auto-install fires on first
use.

## CLI

```
nexai lsp status          # service state + per-server install status
nexai lsp list            # registry, optionally --installed-only
nexai lsp install <id>    # eagerly install one server
nexai lsp install-all     # try every server with a known recipe
nexai lsp restart         # tear down running clients
nexai lsp which <id>      # print resolved binary path
```

`nexai lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <NEXAI_HOME>/lsp/bin
...
When `install_strategy: auto`, nexai installs binaries into
`<NEXAI_HOME>/lsp/bin/`. NPM packages land in
`<NEXAI_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully nexai-owned and is
...
Servers are kept alive for the life of the NexAi process. There's
...
**`nexai lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<NEXAI_HOME>/lsp/bin/`. Run
`nexai lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `nexai lsp status`**
...
When `nexai lsp status` shows a `Backend warnings` section, install
...
`~/.nexai/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.nexai/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `nexai lsp restart` to clear the set;
