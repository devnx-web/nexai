...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with NexAi Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

NexAi Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with NexAi](/docs/guides/use-voice-mode-with-nexai).

## Prerequisites

Before using voice features, make sure you have:

1. **NexAi Agent installed** — `pip install nexai-agent` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `nexai model` or set your preferred provider credentials in `~/.nexai/.env`
3. **A working base setup** — run `nexai` to verify the agent responds to text before enabling voice

:::tip
The `~/.nexai/` directory and default `config.yaml` are created automatically the first time you run `nexai`. You only need to create `~/.nexai/.env` manually for API keys.
:::
...
pip install "NexAi-agent[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
pip install "NexAi-agent[messaging]"

# Premium TTS (ElevenLabs)
pip install "NexAi-agent[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "NexAi-agent[all]"
```
...
Voice mode is available in both the **classic CLI** (`nexai chat`) and the **TUI** (`nexai --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.nexai/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
nexai                # Start the interactive CLI
```
...
1. Start the CLI with `nexai` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.nexai/config.yaml` (default: `ctrl+b`).
:::
...
```bash
nexai gateway        # Start the gateway (connects to configured platforms)
nexai gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@nexaibyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.nexai/.env`:
```bash
...
```bash
nexai gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.nexai/.env`
...
- Review gateway logs: `tail -f ~/.nexai/logs/gateway.log`
