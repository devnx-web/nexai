...
description: "Customize NexAi Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

NexAi Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `NEXAI_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who nexai is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

nexai now seeds a default `SOUL.md` automatically in:

```text
~/.nexai/SOUL.md
```

More precisely, it uses the current instance's `NEXAI_HOME`, so if you run nexai with a custom home directory, it will use:
...
- nexai creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- nexai loads `SOUL.md` only from `NEXAI_HOME`
- nexai does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, nexai falls back to a built-in default identity
...
If nexai loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `NEXAI_HOME`, the personality belongs to the NexAi instance itself.

That also makes it easier to teach users:
- "Edit `~/.nexai/SOUL.md` to change nexai' default personality."
...
- how nexai should handle uncertainty, disagreement, or ambiguity
...
## What nexai injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, nexai falls back to a built-in default identity ("You are NexAi Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
nexai ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain nexai, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives nexai its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.nexai/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.nexai/SOUL.md`
...
- [SOUL.md Guide](/docs/guides/use-soul-with-nexai)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how nexai speaks
- `display.skin` and `/skin` affect how nexai looks in the terminal
