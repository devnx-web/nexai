...
description: "Run NexAi Agent directly on an Android phone with Termux"
---

# NexAi on Android with Termux

This is the tested path for running NexAi Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop nexai from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

nexai now ships a Termux-aware installer path:
...
- links `nexai` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `nexai` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/nexai" "$PREFIX/bin/nexai"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `nexai` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.nexai/.env`.
...
### `nexai doctor` says ripgrep or Node is missing
