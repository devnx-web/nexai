"""CLI entry point for the nexai-agent ACP adapter.

Loads environment variables from ``~/.nexai/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from NEXAI_HOME (default ``~/.nexai``)."""
    from nexai_cli.env_loader import load_nexai_dotenv

    nexai_home = get_nexai_home()
    loaded = load_nexai_dotenv(nexai_home=nexai_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", nexai_home / ".env"
        )
...
def main() -> None:
    ...
    logger.info("Starting nexai-agent ACP adapter")
    ...
    import acp
    from .server import nexaiACPAgent
