...
class TestnexaiConstantsFallback:
    """Tests for _nexai_home.py fallback when nexai_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_nexai_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _nexai_home.py with nexai_constants blocked."""
        monkeypatch.setitem(sys.modules, "nexai_constants", None)
        spec = importlib.util.spec_from_file_location("_nexai_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_nexai_home_env_var(self, monkeypatch, tmp_path):
        """When nexai_constants is missing, NEXAI_HOME comes from env var."""
        monkeypatch.setenv("NEXAI_HOME", str(tmp_path / "custom-nexai"))
        module = self._load_helper(monkeypatch)
        assert module.get_nexai_home() == tmp_path / "custom-nexai"

    def test_fallback_defaults_to_dot_nexai(self, monkeypatch):
        """When nexai_constants is missing and NEXAI_HOME unset, default to ~/.nexai."""
        monkeypatch.delenv("NEXAI_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_nexai_home() == Path.home() / ".nexai"

    def test_fallback_ignores_empty_nexai_home(self, monkeypatch):
        """Empty/whitespace NEXAI_HOME is treated as unset."""
        monkeypatch.setenv("NEXAI_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_nexai_home() == Path.home() / ".nexai"

    def test_fallback_display_nexai_home_shortens_path(self, monkeypatch):
        """Fallback display_nexai_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("NEXAI_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_nexai_home() == "~/.nexai"

    def test_fallback_display_nexai_home_profile_path(self, monkeypatch):
        """Fallback display_nexai_home() handles profile paths under ~/."""
        monkeypatch.setenv("NEXAI_HOME", str(Path.home() / ".nexai/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_nexai_home() == "~/.nexai/profiles/coder"

    def test_fallback_display_nexai_home_custom_path(self, monkeypatch):
        """Fallback display_nexai_home() returns full path for non-home locations."""
        monkeypatch.setenv("NEXAI_HOME", "/opt/nexai-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_nexai_home() == "/opt/nexai-custom"

    def test_delegates_to_nexai_constants_when_available(self):
        """When nexai_constants IS importable, _nexai_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_nexai_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import nexai_constants
        assert module.get_nexai_home is nexai_constants.get_nexai_home
        assert module.display_nexai_home is nexai_constants.display_nexai_home
