...
def test_save_twilio_writes_env_and_state(tmp_path: Path, monkeypatch):
    ...
    env_text = (tmp_path / ".nexai" / ".env").read_text(encoding="utf-8")
    state = json.loads((tmp_path / ".nexai" / "telephony_state.json").read_text(encoding="utf-8"))
...
def test_diagnose_includes_decision_tree_and_saved_state(tmp_path: Path, monkeypatch):
    ...
    mod._save_state(
        ...
        nexai_home / "telephony_state.json",
    )
    (nexai_home / ".env").parent.mkdir(parents=True, exist_ok=True)
