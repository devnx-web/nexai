...
def _run_handle_function_call(
    ...
    *,
    ...
    if invoke_hook is not _UNSET:
        ...
        monkeypatch.setattr("nexai_cli.plugins.invoke_hook", invoke_hook)
...
def test_result_unchanged_when_no_hook_registered(monkeypatch):
    ...
    monkeypatch.setenv("NEXAI_HOME", "/tmp/nexai_no_plugins")
...
def test_transform_tool_result_integration_with_real_plugin(monkeypatch, tmp_path):
    """End-to-end: load a real plugin from NEXAI_HOME and verify it rewrites results."""
    import yaml

    nexai_home = Path(os.environ["NEXAI_HOME"])
