...
class TestBypassWithBotnameSuffix:
    ...
    async def test_stop_with_botname(self):
        """/stop@MynexaiBot must bypass the guard."""
        adapter = _make_adapter()
        sk = _session_key()
        adapter._active_sessions[sk] = asyncio.Event()

        await adapter.handle_message(_make_event("/stop@MynexaiBot"))

        assert sk not in adapter._pending_messages, (
            "/stop@MynexaiBot was queued instead of bypassing"
        )
        assert any("handled:stop" in r for r in adapter.sent_responses)

    @pytest.mark.asyncio
    async def test_new_with_botname(self):
        """/new@MynexaiBot must bypass the guard."""
        adapter = _make_adapter()
        sk = _session_key()
        adapter._active_sessions[sk] = asyncio.Event()

        await adapter.handle_message(_make_event("/new@MynexaiBot"))
