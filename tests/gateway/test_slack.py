...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.nexai."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /nexai, /btw, /stop,
        ...
        for expected in ("/nexai", "/btw", "/stop", "/model", "/help"):
...
class TestSlashCommands:
    ...
    # instead of as /nexai subcommands. This is the Discord/Telegram parity
    # fix: the slash name itself becomes the command.
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /nexai btw."""
        command = {
            "command": "/btw",
    ...
    @pytest.mark.asyncio
    async def test_legacy_nexai_prefix_still_works(self, adapter):
        """Backward compat: /nexai btw foo must still route to /btw foo.

        Old workspace manifests only declared /nexai as the single slash.
    ...
    @pytest.mark.asyncio
    async def test_legacy_nexai_freeform_question(self, adapter):
        """/nexai <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/nexai",
...
class TestSlashEphemeralAck:
    ...
    @pytest.mark.asyncio
    async def test_legacy_nexai_slash_stashes_context(self, adapter):
        """Legacy /nexai <subcommand> also stashes context."""
        command = {
            "command": "/nexai",
            "text": "help",
            "user_id": "U_H",
    ...
    @pytest.mark.asyncio
    async def test_freeform_nexai_question_does_not_stash_context(self, adapter):
        """Free-form /nexai <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/nexai",
