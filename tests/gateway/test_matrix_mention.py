...
class TestIsBotMentioned:
    ...
    def test_full_user_id_in_body(self):
        assert self.adapter._is_bot_mentioned("hey @nexai:example.org help")

    def test_localpart_in_body(self):
        assert self.adapter._is_bot_mentioned("nexai can you help?")

    def test_localpart_case_insensitive(self):
        assert self.adapter._is_bot_mentioned("nexai can you help?")

    def test_matrix_pill_in_formatted_body(self):
        html = '<a href="https://matrix.to/#/@nexai:example.org">nexai</a> help'
        assert self.adapter._is_bot_mentioned("nexai help", html)
    ...
    def test_partial_localpart_no_match(self):
        # "nexaibot" should not match word-boundary check for "nexai"
        assert not self.adapter._is_bot_mentioned("nexaibot is here")
...
class TestStripMention:
    ...
    def test_strip_full_user_id(self):
        result = self.adapter._strip_mention("@nexai:example.org help me")
        assert result == "help me"

    def test_localpart_preserved(self):
        """Bare localpart (no @) is preserved — avoids false positives in paths."""
        result = self.adapter._strip_mention("nexai help me")
        assert result == "nexai help me"

    def test_localpart_in_path_preserved(self):
        """Localpart inside a file path must not be damaged."""
        result = self.adapter._strip_mention("read /home/nexai/config.yaml")
        assert result == "read /home/nexai/config.yaml"
    ...
    def test_does_not_strip_bare_localpart_word(self):
        # Regression: plain words like "nexai Agent" should not be mutated.
        result = self.adapter._strip_mention("nexai Agent")
        assert result == "nexai Agent"

    def test_strip_returns_empty_for_mention_only(self):
        result = self.adapter._strip_mention("@nexai:example.org")
        assert result == ""
...
async def test_require_mention_default_processes_mentioned(monkeypatch):
    ...
    adapter = _make_adapter()
    event = _make_event("@nexai:example.org help me")
...
async def test_require_mention_html_pill(monkeypatch):
    ...
    adapter = _make_adapter()
    formatted = '<a href="https://matrix.to/#/@nexai:example.org">nexai</a> help'
    event = _make_event("nexai help", formatted_body=formatted)
...
async def test_dm_preserves_localpart_in_body(monkeypatch):
    ...
    adapter = _make_adapter()
    _set_dm(adapter)
    event = _make_event("nexai help me")

    await adapter._on_room_message(event)
    adapter.handle_message.assert_awaited_once()
    msg = adapter.handle_message.await_args.args[0]
    assert msg.text == "nexai help me"
...
async def test_bare_mention_passes_empty_string(monkeypatch):
    ...
    adapter = _make_adapter()
    event = _make_event("@nexai:example.org")
...
async def test_require_mention_disabled_skips_stripping(monkeypatch):
    ...
    adapter.handle_message.assert_awaited_once()
    msg = adapter.handle_message.await_args.args[0]
    assert msg.text == "@nexai:example.org help me"
...
async def test_dm_mention_thread_disabled_by_default(monkeypatch):
    ...
    adapter = _make_adapter()
    _set_dm(adapter)
    event = _make_event("@nexai:example.org help me", event_id="$dm1")
...
async def test_dm_mention_thread_preserves_existing_thread(monkeypatch):
    ...
    event = _make_event("@nexai:example.org help me", thread_id="$existing_thread")
...
async def test_dm_mention_thread_tracks_participation(monkeypatch):
    ...
    adapter = _make_adapter()
    _set_dm(adapter)
    event = _make_event("@nexai:example.org help", event_id="$dm1")
