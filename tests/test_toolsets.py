...
class TestToolsetConsistency:
    ...
    def test_nexai_platforms_share_core_tools(self):
        """All nexai-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on nexai-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["nexai-cli", "nexai-telegram", "nexai-discord", "nexai-whatsapp", "nexai-slack", "nexai-signal", "nexai-homeassistant"]
