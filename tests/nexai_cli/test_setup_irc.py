"""Tests for IRC gateway configuration via `nexai setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh nexai install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new nexai install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#nexai")
            monkeypatch.setenv("IRC_NICKNAME", "nexai-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `nexai setup gateway` experience with IRC present."""
