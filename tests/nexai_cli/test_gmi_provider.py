...
from nexai_cli.auth import resolve_provider
...
class TestGmiAliases:
    ...
    def test_providers_normalize_provider(self):
        from nexai_cli.providers import normalize_provider as normalize_provider_in_providers
...
class TestGmiConfigRegistry:
    def test_optional_env_vars_include_gmi(self):
        from nexai_cli.config import OPTIONAL_ENV_VARS
        ...
        # nexai model, not via upgrade prompts.
...
class TestGmiModelCatalog:
    ...
    def test_provider_model_ids_prefers_live_api(self, monkeypatch):
        monkeypatch.setattr(
            "nexai_cli.auth.resolve_api_key_provider_credentials",
            lambda provider_id: {
        ...
        monkeypatch.setattr(
            "nexai_cli.models.fetch_api_models",
    ...
    def test_provider_model_ids_falls_back_to_static_models(self, monkeypatch):
        ...
        monkeypatch.setattr("nexai_cli.models.fetch_api_models", lambda api_key, base_url: None)
...
class TestGmiProvidersModule:
    def test_overlay_exists(self):
        ...
        assert "gmi" in NEXAI_OVERLAYS
        overlay = NEXAI_OVERLAYS["gmi"]
...
class TestGmiDoctor:
    ...
    def test_run_doctor_checks_gmi_models_endpoint(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(doctor_mod, "NEXAI_HOME", home)
        ...
        try:
            from nexai_cli import auth as _auth_mod
...
class TestGmiAuxiliary:
    ...
    def test_resolve_provider_client_uses_gmi_aux_default(self, monkeypatch):
        ...
        # GMI profile declares default_headers with a nexaiAgent User-Agent
        ...
        assert headers.get("User-Agent", "").startswith("nexaiAgent/")

    def test_gmi_profile_declares_nexai_user_agent(self):
        """The GMI plugin sets a nexaiAgent/<ver> User-Agent on its profile."""
        from providers import get_provider_profile

        profile = get_provider_profile("gmi")
        assert profile is not None
        ua = profile.default_headers.get("User-Agent", "")
        assert ua.startswith("nexaiAgent/"), (
            f"expected GMI profile User-Agent to start with 'nexaiAgent/', got {ua!r}"
        )
...
class TestGmiMainFlow:
    def test_chat_parser_accepts_gmi_provider(self, monkeypatch):
        ...
        monkeypatch.setattr("nexai_cli.config.get_container_exec_info", lambda: None)
        monkeypatch.setattr(
            "nexai_cli.main.cmd_chat",
            lambda args: recorded.setdefault("provider", args.provider),
        )
        monkeypatch.setattr(sys, "argv", ["nexai", "chat", "--provider", "gmi"])

        from nexai_cli.main import main
    ...
    def test_select_provider_and_model_routes_gmi_to_generic_flow(self, monkeypatch):
        ...
        monkeypatch.setattr("nexai_cli.auth.resolve_provider", lambda *args, **kwargs: None)
        ...
        monkeypatch.setattr("nexai_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
        monkeypatch.setattr("nexai_cli.main._model_flow_api_key_provider", fake_model_flow_api_key_provider)
    ...
    def test_model_flow_api_key_provider_persists_gmi_selection(self, monkeypatch):
        ...
        with patch(
            ...
            "nexai_cli.auth._prompt_model_selection",
            return_value="openai/gpt-5.4-mini",
        ), patch(
            "nexai_cli.auth.deactivate_provider",
        ), patch(
            "builtins.input",
            return_value="",
        ):
            from nexai_cli.main import _model_flow_api_key_provider
        ...
        config = yaml.safe_load((get_nexai_home() / "config.yaml").read_text()) or {}
