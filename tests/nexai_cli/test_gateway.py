...
import nexai_cli.gateway as gateway
...
def _install_fake_gateway_run(monkeypatch, start_gateway):
    ...
    # /nexai-gateway.service"`` and runs ``systemctl --user daemon-reload``
    # — both target the *real* user environment because the conftest only
    # sandboxes ``NEXAI_HOME``, not ``HOME``. Tests that drive
    ...
    # unit (baking in the test's pytest-tmp ``NEXAI_HOME`` value, which
...
def test_run_gateway_refuses_root_in_official_docker(monkeypatch, tmp_path, capsys):
    project_root = tmp_path / "opt" / "nexai"
    ...
    monkeypatch.delenv("NEXAI_ALLOW_ROOT_GATEWAY", raising=False)
    ...
    assert exc_info.value.code == 1
    out = capsys.readouterr().out
    assert "Refusing to run the nexai gateway as root" in out
    assert "/opt/nexai/docker/entrypoint.sh" in out
...
def test_run_gateway_root_guard_has_escape_hatch(monkeypatch):
    ...
    monkeypatch.setattr(gateway.os, "geteuid", lambda: 0)
    monkeypatch.setattr(gateway, "_is_official_docker_checkout", lambda: True)
    monkeypatch.setenv("NEXAI_ALLOW_ROOT_GATEWAY", "1")
...
def test_run_gateway_windows_foreground_keeps_ctrl_c_enabled(monkeypatch):
    ...
    monkeypatch.delenv("NEXAI_GATEWAY_DETACHED", raising=False)
...
def test_run_gateway_windows_detached_absorbs_console_controls(monkeypatch):
    ...
    monkeypatch.setattr(gateway, "supports_systemd_services", lambda: False)
    monkeypatch.setattr(gateway.sys, "stdin", _TTY())
    monkeypatch.setenv("NEXAI_GATEWAY_DETACHED", "1")
    monkeypatch.setattr(gateway.signal, "signal", fake_signal)
    monkeypatch.setattr(gateway.asyncio, "run", lambda coro: True)
...
def test_systemd_install_checks_linger_status(monkeypatch, tmp_path, capsys):
    unit_path = tmp_path / "systemd" / "user" / "nexai-gateway.service"
...
def test_systemd_install_system_scope_skips_linger_and_uses_systemctl(monkeypatch, tmp_path, capsys):
    unit_path = tmp_path / "etc" / "systemd" / "system" / "nexai-gateway.service"
...
def test_conflicting_systemd_units_warning(monkeypatch, tmp_path, capsys):
    user_unit = tmp_path / "user" / "nexai-gateway.service"
    system_unit = tmp_path / "system" / "nexai-gateway.service"
    ...
    assert "nexai gateway uninstall" in out
...
def test_install_linux_gateway_from_setup_system_choice_without_root_prints_followup(monkeypatch, capsys):
    ...
    assert "sudo nexai gateway install --system --run-as-user alice" in out
    assert "sudo nexai gateway start --system" in out
