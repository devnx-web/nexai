"""Unit tests for nexai_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf nexai-ok"])
        try:
            output = _read_until(bridge, b"nexai-ok")
            assert b"nexai-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$NEXAI_PTY_TEST\""],
            env={**os.environ, "NEXAI_PTY_TEST": "pty-env-works"},
        )
        try:
