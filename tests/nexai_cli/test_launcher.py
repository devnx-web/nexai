"""Tests for the top-level `./nexai` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./nexai` should use `nexai_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "nexai"
    called = []

    fake_main_module = types.ModuleType("nexai_cli.main")

    def fake_main():
        called.append("nexai_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "nexai_cli.main", fake_main_module)
