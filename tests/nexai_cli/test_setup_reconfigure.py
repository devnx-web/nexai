...
- Bare `nexai setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `nexai setup --quick` runs the narrower "fill in missing items" flow.
- `nexai setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("nexai_cli.setup.ensure_nexai_home", {}),
        ("nexai_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("nexai_cli.config.is_managed", {"return_value": False}),
        ("nexai_cli.setup.load_config", {"return_value": {}}),
        ("nexai_cli.setup.save_config", {}),
        ("nexai_cli.setup.get_env_value", {"return_value": None}),
        ("nexai_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("nexai_cli.setup._print_setup_summary", {}),
        ("nexai_cli.setup._offer_launch_chat", {}),
        ("nexai_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("nexai_cli.setup.load_config", {"return_value": {}}),
        ("nexai_cli.setup.save_config", {}),
        ("nexai_cli.auth.get_active_provider", {"return_value": None}),
        ("nexai_cli.setup.get_env_value", {"return_value": None}),
        ("nexai_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `nexai setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="nexai_cli.setup.prompt_choice",
                quick="nexai_cli.setup._run_quick_setup",
                model="nexai_cli.setup.setup_model_provider",
                terminal="nexai_cli.setup.setup_terminal_backend",
                agent="nexai_cli.setup.setup_agent_settings",
                gateway="nexai_cli.setup.setup_gateway",
                tools="nexai_cli.setup.setup_tools",
            )
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`nexai setup --reconfigure` behaves the same as bare `nexai setup`."""
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("nexai_cli.setup.prompt_choice", {"return_value": 0}),
                first="nexai_cli.setup._run_first_time_quick_setup",
            )
            from nexai_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "nexai_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["nexai", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_quick_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["nexai", "setup", "--quick"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_bare_setup_has_both_flags_false(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["nexai", "setup"])
        try:
            main()
        except SystemExit:
            pass
