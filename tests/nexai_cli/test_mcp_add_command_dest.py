"""Regression test: ``nexai mcp add --command`` must not clobber the
top-level ``args.command`` subparser dest.

The top-level argparse parser uses ``dest="command"`` for its subparsers
(``nexai_cli/_parser.py``).  The dispatcher in ``nexai_cli/main.py``
...
result, ``nexai mcp add foo --url ...`` silently launches chat instead
...
def _build_parser():
    """Minimal replica of the slice of the nexai parser that exhibits
...
class TestMcpAddCommandDest:
    def test_url_invocation_preserves_top_level_command(self):
        """`nexai mcp add foo --url ...` must keep args.command == "mcp".
