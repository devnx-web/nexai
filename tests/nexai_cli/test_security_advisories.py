"""Tests for nexai_cli.security_advisories.
...
import nexai_cli.security_advisories as adv
...
def isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect NEXAI_HOME so banner cache and config writes are sandboxed."""
    home = tmp_path / ".nexai"
    home.mkdir()
    (home / "cache").mkdir()
...
class TestAck:
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full nexai_cli.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "nexai_cli.config.load_config", lambda: store
        )
        monkeypatch.setattr(
            "nexai_cli.config.save_config",
