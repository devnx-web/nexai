"""Tests for nexai_cli.relaunch — unified self-relaunch utility."""
...
class TestResolvenexaiBin:
    def test_prefers_absolute_argv0_when_executable(self, monkeypatch):
        ...
        assert relaunch_mod.resolve_nexai_bin() == fake
    ...
    def test_resolves_relative_argv0(self, monkeypatch, tmp_path):
        ...
        # Ensure we don't accidentally match a real 'nexai' on PATH
        monkeypatch.setattr(relaunch_mod.shutil, "which", lambda _name: None)
        assert relaunch_mod.resolve_nexai_bin() == str(fake)

    def test_falls_back_to_path_which(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["-c"])  # not a real path
        monkeypatch.setattr(
            relaunch_mod.shutil, "which", lambda name: "/usr/bin/nexai" if name == "nexai" else None
        )
        assert relaunch_mod.resolve_nexai_bin() == "/usr/bin/nexai"

    def test_returns_none_when_unresolvable(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["-c"])
        monkeypatch.setattr(relaunch_mod.shutil, "which", lambda _name: None)
        assert relaunch_mod.resolve_nexai_bin() is None
...
class TestBuildRelaunchArgv:
    ...
    def test_can_disable_preserve(self, monkeypatch):
        ...
        assert argv == ["/usr/bin/nexai", "--resume", "abc"]
...
class TestRelaunch:
    def test_calls_execvp(self, monkeypatch):
        ...
        assert calls == [("/usr/bin/nexai", ["/usr/bin/nexai", "--resume", "abc"])]
    ...
    def test_windows_uses_subprocess_not_execvp(self, monkeypatch):
        ...
        nexai).  relaunch() must detect win32 and use subprocess.run +
        sys.exit instead."""
        monkeypatch.setattr(relaunch_mod.sys, "platform", "win32")
        monkeypatch.setattr(relaunch_mod, "resolve_nexai_bin", lambda: r"C:\Users\test\nexai.exe")
    ...
    def test_windows_propagates_child_exit_code(self, monkeypatch):
        ...
        monkeypatch.setattr(relaunch_mod.sys, "platform", "win32")
        monkeypatch.setattr(relaunch_mod, "resolve_nexai_bin", lambda: r"C:\nexai.exe")
    ...
    def test_windows_surfaces_oserror_with_help(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(relaunch_mod.sys, "platform", "win32")
        monkeypatch.setattr(relaunch_mod, "resolve_nexai_bin", lambda: r"C:\missing.exe")
...
class TestResolvenexaiBinWindowsPyGuard:
    """On Windows, resolve_nexai_bin MUST NOT return a .py path.
    ...
    def test_windows_rejects_py_argv0_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        # Force PATH lookup to return a nexai.exe so the test doesn't
        # exercise the None-fallback path (that's a separate test).
        monkeypatch.setattr(
            relaunch_mod.shutil, "which",
            lambda name: r"C:\venv\Scripts\nexai.exe" if name == "nexai" else None,
        )

        bin_path = relaunch_mod.resolve_nexai_bin()
        # Must NOT be the .py — must be the nexai.exe PATH entry.
    ...
    def test_posix_still_accepts_py_argv0(self, monkeypatch, tmp_path):
        ...
        script = tmp_path / "nexai"
        script.write_text("#!/usr/bin/env python3\n")
        script.chmod(0o755)
        monkeypatch.setattr(relaunch_mod.sys, "argv", [str(script), "chat"])
        assert relaunch_mod.resolve_nexai_bin() == str(script)

    def test_windows_py_argv0_with_no_nexai_on_path_returns_none(self, monkeypatch, tmp_path):
        """Bulletproof fallback: if argv0 is .py on Windows AND nexai.exe
        isn't on PATH, return None so the caller falls back to
        python -m nexai_cli.main."""
        script = tmp_path / "main.py"
        script.write_text("# stub")
