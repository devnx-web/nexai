...
``nexai_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``nexai --help``, ``nexai version``,
``nexai logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``nexai --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from nexai_cli import main as _main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["nexai", "-w", "--tui"], None),
        (["nexai", "version"], "version"),
        (["nexai", "--tui", "chat"], "chat"),
        (["nexai", "-w", "logs"], "logs"),
        (["nexai", "chat", "hello world"], "chat"),
        (["nexai", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["nexai", "-m", "gpt5", "chat"], "chat"),
        (["nexai", "--model", "gpt5", "chat", "hi"], "chat"),
        (["nexai", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["nexai", "-z", "hello world"], None),
        (["nexai", "-z", "hello", "chat"], "chat"),
        (["nexai", "--model=gpt5", "chat"], "chat"),     # inline form
        (["nexai", "--", "chat"], "chat"),               # -- terminator
        (["nexai", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["nexai", "some-plugin-cmd"], "some-plugin-cmd"),
        (["nexai", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["nexai"],                          # bare → chat
        ["nexai", "--help"],                # top-level help
        ["nexai", "-h"],
        ["nexai", "version"],               # known built-in
        ["nexai", "logs"],
        ["nexai", "gateway", "run"],
        ["nexai", "--tui"],
        ["nexai", "-w", "--tui"],
        ["nexai", "chat", "hi"],
        ["nexai", "help"],                  # accepted built-in-ish
        ["nexai", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["nexai", "meet", "join"],          # potential google_meet plugin
        ["nexai", "honcho", "status"],      # potential memory plugin
        ["nexai", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``nexai help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"nexai_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
