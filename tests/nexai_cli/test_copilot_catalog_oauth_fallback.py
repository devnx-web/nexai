...
``nexai auth add copilot`` or by ``_seed_from_env`` when the env var
is set in ``~/.nexai/.env`` — the picker was silently dropping back to
...
from nexai_cli.models import _resolve_copilot_catalog_api_key
...
class TestCopilotCatalogApiKeyResolution:
    def test_env_var_token_wins_over_pool(self):
        ...
        with patch(
            ...
            "nexai_cli.auth.read_credential_pool",
        ) as mock_pool:
    ...
    def test_falls_back_to_pool_oauth_token(self):
        ...
        with patch(
            ...
            "nexai_cli.copilot_auth.exchange_copilot_token",
