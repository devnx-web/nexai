...
def container_env(tmp_path, monkeypatch):
    """Set up a fake NEXAI_HOME with .container-mode file."""
    nexai_home = tmp_path / ".nexai"
    nexai_home.mkdir()
    monkeypatch.setenv("NEXAI_HOME", str(nexai_home))
    monkeypatch.delenv("NEXAI_DEV", raising=False)

    container_mode = nexai_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=nexai-agent\n"
        "exec_user=nexai\n"
        "nexai_bin=/data/current-package/bin/nexai\n"
    )
    return nexai_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("nexai_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "nexai-agent"
    assert info["exec_user"] == "nexai"
    assert info["nexai_bin"] == "/data/current-package/bin/nexai"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("nexai_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_nexai_dev(container_env, monkeypatch):
    """Returns None when NEXAI_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("NEXAI_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_nexai_dev_zero(container_env, monkeypatch):
    """NEXAI_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("NEXAI_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        nexai_home = Path(tmpdir) / ".nexai"
        nexai_home.mkdir()
        (nexai_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("nexai_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_nexai_home": lambda: nexai_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("NEXAI_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=nexai-custom\n"
        "exec_user=myuser\n"
        "nexai_bin=/opt/nexai/bin/nexai\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "nexai-custom"
    assert info["exec_user"] == "myuser"
    assert info["nexai_bin"] == "/opt/nexai/bin/nexai"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from nexai_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "nexai"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "nexai-agent" in cmd
    assert "/data/current-package/bin/nexai" in cmd
    assert "chat" in cmd
