"""Tests for the update check mechanism in nexai_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    with patch("nexai_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NEXAI_HOME", str(tmp_path))
    with patch("nexai_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    import nexai_cli.banner as banner

    # Create a fake banner.py so the fallback path also has no .git
    fake_banner = tmp_path / "nexai_cli" / "banner.py"
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when NEXAI_HOME has no git repo."""
    import nexai_cli.banner as banner
    ...
    # Point NEXAI_HOME at a temp dir with no nexai-agent/.git
    monkeypatch.setenv("NEXAI_HOME", str(tmp_path))
    with patch("nexai_cli.banner.subprocess.run") as mock_run:
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from nexai_cli.main import _invalidate_update_cache

    # Build a fake ~/.nexai with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"NEXAI_HOME": str(default_home)}):
