...
When ``NEXAI_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.nexai/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.nexai            (has its own auth.json fixture)
    * Profile     -> tmp_path/.nexai/profiles/coder   (active, NEXAI_HOME points here)
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (NEXAI_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.nexai as the "real home" — which is NOT equal
    # to the NEXAI_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    nexai_home = tmp_path / "classic"
    nexai_home.mkdir()
    monkeypatch.setenv("NEXAI_HOME", str(nexai_home))

    _write(nexai_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from nexai_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: NEXAI_HOME is set to a custom path that is NOT under
    # ~/.nexai/profiles/ — get_default_nexai_root() returns NEXAI_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from nexai_cli.auth import read_credential_pool, write_credential_pool
