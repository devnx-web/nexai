...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (NEXAI_OVERLAYS) — bedrock
...
class TestProviderModelIdsBedrock:
    ...
    def test_falls_back_to_static_list_when_discovery_empty(self, monkeypatch):
        ...
        from nexai_cli.models import _PROVIDER_MODELS, provider_model_ids
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via NEXAI_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in NEXAI_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from nexai_cli.providers import NEXAI_OVERLAYS
        assert "bedrock" in NEXAI_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from nexai_cli.providers import NEXAI_OVERLAYS
        assert NEXAI_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from nexai_cli.providers import NEXAI_OVERLAYS
        assert NEXAI_OVERLAYS["bedrock"].auth_type == "aws_sdk"
