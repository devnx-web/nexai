"""Tests for the `nexai memory reset` CLI command.
...
- Profile-scoped reset (uses NEXAI_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake NEXAI_HOME with memory files."""
    nexai_home = tmp_path / ".nexai"
    memories = nexai_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("NEXAI_HOME", str(nexai_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nnexai repo is at ~/.nexai/nexai-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `nexai memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        nexai_home = tmp_path / ".nexai"
        (nexai_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("NEXAI_HOME", str(nexai_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's NEXAI_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        nexai_home = tmp_path / ".nexai"
        nexai_home.mkdir(parents=True)
        # No memories dir
        monkeypatch.setenv("NEXAI_HOME", str(nexai_home))

        # The memories dir won't exist; get_nexai_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
