...
def test_main_top_level_tui_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(sys, "argv", ["nexai", "--tui", "--toolsets", "web,terminal"])
    monkeypatch.setitem(
        sys.modules,
        "nexai_cli.plugins",
...
def test_main_top_level_oneshot_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys, "argv", ["nexai", "-z", "hello", "--toolsets", "web,terminal"]
    )
    monkeypatch.setitem(
        sys.modules,
        "nexai_cli.plugins",
...
def test_oneshot_filters_invalid_toolsets_before_redirect(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    from nexai_cli.oneshot import _validate_explicit_toolsets
...
def test_oneshot_rejects_disabled_mcp_toolset(monkeypatch, capsys):
    ...
    assert valid is None
    assert error == "nexai -z: --toolsets did not contain any valid toolsets.\n"
    err = capsys.readouterr().err
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """nexai -z bypasses nexaiCLI, but recall still needs SessionDB."""
    from nexai_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "nexai_state", mod("nexai_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "nexai_cli.config",
        mod("nexai_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "nexai_cli.models",
        mod("nexai_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
        "nexai_cli.runtime_provider",
        mod(
            "nexai_cli.runtime_provider",
    ...
    monkeypatch.setitem(
        sys.modules,
        "nexai_cli.tools_config",
        mod("nexai_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["NEXAI_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="nous/nexai-test", provider="nous", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["NEXAI_MODEL"] == "nous/nexai-test"
    assert env["NEXAI_INFERENCE_MODEL"] == "nous/nexai-test"
    assert env["NEXAI_TUI_PROVIDER"] == "nous"
    assert env["NEXAI_INFERENCE_PROVIDER"] == "nous"
    assert env["NEXAI_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["NEXAI_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("nexai-tui-active-session-")
...
def test_print_tui_exit_summary_includes_resume_and_token_totals(monkeypatch, capsys):
    import nexai_cli.main as main_mod
    ...
    monkeypatch.setitem(
        sys.modules, "nexai_state", types.SimpleNamespace(SessionDB=lambda: _FakeDB())
    )
    ...
    assert "nexai --tui --resume 20260409_000001_abc123" in out
    assert 'nexai --tui -c "demo title"' in out
...
def test_print_tui_exit_summary_prefers_actual_active_session_file(
    ...
    assert "nexai --tui --resume actual_session" in out
