"""Tests for the Kanban DB layer (nexai_cli.kanban_db)."""
...
from nexai_cli import kanban_db as kb


@pytest.fixture
def kanban_home(tmp_path, monkeypatch):
    """Isolated NEXAI_HOME with an empty kanban DB."""
    home = tmp_path / ".nexai"
    home.mkdir()
...
def test_stale_claim_reclaimed(kanban_home, monkeypatch):
    import signal
    import nexai_cli.kanban_db as _kb
...
def test_dispatch_skips_nonspawnable_into_separate_bucket(kanban_home, monkeypatch):
    ...
    from nexai_cli import profiles
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real nexai profile — preserves the
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="nexai-task", assignee="daily")
...
def test_scratch_workspace_created_under_nexai_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
...
# spawned with `nexai -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's NEXAI_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's NEXAI_HOME."""

    def _set_home(self, monkeypatch, tmp_path, nexai_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NEXAI_HOME", str(nexai_home))
        monkeypatch.delenv("NEXAI_KANBAN_HOME", raising=False)

    def test_default_install_anchors_at_home_dot_nexai(
        self, tmp_path, monkeypatch
    ):
        # Standard install: NEXAI_HOME == ~/.nexai, no profile active.
        default_home = tmp_path / ".nexai"
        default_home.mkdir()
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.nexai/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.nexai/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.nexai/kanban.db.
        default_home = tmp_path / ".nexai"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # NEXAI_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `nexai -p coder`).
        monkeypatch.setenv("NEXAI_HOME", str(profile_home))
    ...
    def test_docker_custom_nexai_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: NEXAI_HOME points outside ~/.nexai.
        # `get_default_nexai_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".nexai"`.
        custom_root = tmp_path / "opt" / "nexai"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: NEXAI_HOME=/opt/nexai/profiles/coder;
        # `get_default_nexai_root()` walks up to /opt/nexai because
    ...
    def test_explicit_override_via_nexai_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # Explicit override: NEXAI_KANBAN_HOME beats every other
        # resolution rule.
        default_home = tmp_path / ".nexai"
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NEXAI_HOME", str(profile_home))
        monkeypatch.setenv("NEXAI_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        default_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NEXAI_HOME", str(default_home))
        monkeypatch.setenv("NEXAI_KANBAN_HOME", "   ")
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # NEXAI_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile NEXAI_HOME and reads.
        monkeypatch.setenv("NEXAI_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_nexai_kanban_db_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # NEXAI_KANBAN_DB pins the file path directly and beats both
        # NEXAI_KANBAN_HOME and the `get_default_nexai_root()` path.
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NEXAI_HOME", str(default_home))
        monkeypatch.setenv("NEXAI_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("NEXAI_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows NEXAI_KANBAN_HOME -- the pins
        # are independent.
        assert kb.workspaces_root() == umbrella / "kanban" / "workspaces"

    def test_nexai_kanban_workspaces_root_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # NEXAI_KANBAN_WORKSPACES_ROOT pins the workspaces root directly.
        default_home = tmp_path / ".nexai"
        default_home.mkdir()
        umbrella = tmp_path / "umbrella"
        umbrella.mkdir()
        ...
        monkeypatch.setenv("NEXAI_HOME", str(default_home))
        monkeypatch.setenv("NEXAI_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("NEXAI_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows NEXAI_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NEXAI_HOME", str(default_home))
        monkeypatch.setenv("NEXAI_KANBAN_DB", "   ")
        monkeypatch.setenv("NEXAI_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject NEXAI_KANBAN_DB
        # and NEXAI_KANBAN_WORKSPACES_ROOT into the worker env so the
        # worker converges on the dispatcher's paths even when the
        # `-p <profile>` flag rewrites NEXAI_HOME.
        default_home = tmp_path / ".nexai"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
        ...
        env = captured["env"]
        assert env["NEXAI_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["NEXAI_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["NEXAI_KANBAN_TASK"] == "t_dispatch_env"
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see nexai_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(kanban_home, caplog):
    ...
    with _patch("nexai_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="nexai_state"):
            conn = kb.connect()
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'nexai kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_nexai_argv()
#
# Workers spawned by the dispatcher must use a `nexai` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["nexai", ...]`
...
def test_resolve_nexai_argv_prefers_path_shim(monkeypatch):
    """When `nexai` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
    import nexai_cli.kanban_db as kb

    monkeypatch.setattr(shutil, "which", lambda name: "/usr/local/bin/nexai")
    argv = kb._resolve_nexai_argv()
    assert argv == ["/usr/local/bin/nexai"]


def test_resolve_nexai_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m nexai_cli.main`.

    Pins the correct module name (NOT `nexai` — there is no top-level
    `nexai` package). Regression for #23198: the original PR shipped
    `python -m nexai` which fails with `No module named nexai` on every
    invocation.
    """
    import shutil
    import sys
    ...
    argv = kb._resolve_nexai_argv()
    assert argv == [sys.executable, "-m", "nexai_cli.main"]


def test_resolve_nexai_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `nexai_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m nexai_cli.main --version`
    ...
    assert "nexai Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
