"""Tests for Codex auth — tokens stored in nexai auth store (~/.nexai/auth.json)."""
...
def _setup_nexai_auth(nexai_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the nexai auth store."""
    ...
    auth_file = nexai_home / "auth.json"
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    nexai_home = tmp_path / "nexai"
    _setup_nexai_auth(nexai_home, access_token="")
    monkeypatch.setenv("NEXAI_HOME", str(nexai_home))
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_nexai_auth(nexai_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("NEXAI_HOME", str(nexai_home))
    ...
    monkeypatch.setattr("nexai_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    nexai_home = tmp_path / "nexai"
    _setup_nexai_auth(nexai_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("NEXAI_HOME", str(nexai_home))
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to nexai auth store, not ~/.codex/."""
    nexai_home = tmp_path / "nexai"
    codex_home = tmp_path / "codex-cli"
    nexai_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "nexai-at", "refresh_token": "nexai-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches nexai store
    assert not (codex_home / "auth.json").exists()

    # nexai auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "nexai-at"


def test_resolve_returns_nexai_auth_store_source(tmp_path, monkeypatch):
    nexai_home = tmp_path / "nexai"
    _setup_nexai_auth(nexai_home)
    monkeypatch.setenv("NEXAI_HOME", str(nexai_home))

    creds = resolve_codex_runtime_credentials()
    assert creds["source"] == "nexai-auth-store"
...
def _patch_httpx(monkeypatch, response):
    ...
    monkeypatch.setattr("nexai_cli.auth.httpx.Client", _factory)
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "nexai_cli.auth.resolve_codex_runtime_credentials",
        lambda: {"base_url": DEFAULT_CODEX_BASE_URL},
    )
    monkeypatch.setattr(
        "nexai_cli.auth._import_codex_cli_tokens",
    ...
    monkeypatch.setattr("nexai_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("nexai_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
