"""Tests for nexai_constants module."""
...
from nexai_constants import (
    ...
    get_default_nexai_root,
    is_container,
    parse_reasoning_effort,
)


class TestGetDefaultnexaiRoot:
    """Tests for get_default_nexai_root() — Docker/custom deployment awareness."""

    def test_no_nexai_home_returns_native(self, tmp_path, monkeypatch):
        """When NEXAI_HOME is not set, returns ~/.nexai."""
        monkeypatch.delenv("NEXAI_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_nexai_root() == tmp_path / ".nexai"

    def test_nexai_home_is_native(self, tmp_path, monkeypatch):
        """When NEXAI_HOME = ~/.nexai, returns ~/.nexai."""
        native = tmp_path / ".nexai"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NEXAI_HOME", str(native))
        assert get_default_nexai_root() == native

    def test_nexai_home_is_profile(self, tmp_path, monkeypatch):
        """When NEXAI_HOME is a profile under ~/.nexai, returns ~/.nexai."""
        native = tmp_path / ".nexai"
    ...
    def test_nexai_home_is_docker(self, tmp_path, monkeypatch):
        """When NEXAI_HOME points outside ~/.nexai (Docker), returns NEXAI_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NEXAI_HOME", str(docker_home))
        assert get_default_nexai_root() == docker_home

    def test_nexai_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any NEXAI_HOME outside ~/.nexai is treated as the root."""
        custom = tmp_path / "my-nexai-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NEXAI_HOME", str(custom))
        assert get_default_nexai_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (NEXAI_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NEXAI_HOME", str(profile))
        assert get_default_nexai_root() == docker_root
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(nexai_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(nexai_constants, "_container_detected", True)
        assert is_container() is True
