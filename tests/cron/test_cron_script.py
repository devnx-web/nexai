...
- Path resolution (absolute, relative to NEXAI_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp NEXAI_HOME."""
    nexai_home = tmp_path / ".nexai"
    nexai_home.mkdir()
    (nexai_home / "cron").mkdir()
    (nexai_home / "cron" / "output").mkdir()
    (nexai_home / "scripts").mkdir()
    monkeypatch.setenv("NEXAI_HOME", str(nexai_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "NEXAI_DIR", nexai_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", nexai_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", nexai_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", nexai_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.nexai/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("NEXAI_SESSION_PLATFORM") is None
        assert os.environ.get("NEXAI_SESSION_CHAT_ID") is None
        assert os.environ.get("NEXAI_SESSION_CHAT_NAME") is None
