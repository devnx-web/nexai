"""
Tests for mcp_serve — nexai MCP server.
...
def _isolate_nexai_home(tmp_path, monkeypatch):
    """Redirect NEXAI_HOME to a temp directory."""
    monkeypatch.setenv("NEXAI_HOME", str(tmp_path))
    try:
        import nexai_constants
        monkeypatch.setattr(nexai_constants, "get_nexai_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking nexai_state schema."""
