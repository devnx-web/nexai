...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    nexai_state = ModuleType("nexai_state")
    nexai_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "nexai_state", nexai_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "nexai"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "nexai"
