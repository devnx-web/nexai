...
from nexai_cli import main as nexai_main
...
def test_nexai_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    cli = _import_cli()
    calls = {"count": 0}

    def _unexpected_runtime_resolve(**kwargs):
        calls["count"] += 1
        raise AssertionError("resolve_runtime_provider should not be called in nexaiCLI.__init__")

    monkeypatch.setattr("nexai_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
    monkeypatch.setattr("nexai_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))

    shell = cli.nexaiCLI(model="gpt-5", compact=True, max_turns=1)
...
def test_runtime_resolution_failure_is_not_sticky(monkeypatch):
    ...
    monkeypatch.setattr("nexai_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    monkeypatch.setattr("nexai_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_cli_prefers_config_provider_over_stale_env_override(monkeypatch):
    ...
    shell = cli.nexaiCLI(model="fireworks/minimax-m2p5", compact=True, max_turns=1)
...
def test_codex_provider_replaces_incompatible_default_model(monkeypatch):
    ...
    monkeypatch.setattr("nexai_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
    monkeypatch.setattr(
        "nexai_cli.codex_models.get_codex_model_ids",
        lambda access_token=None: ["gpt-5.2-codex", "gpt-5.1-codex-mini"],
    )

    shell = cli.nexaiCLI(compact=True, max_turns=1)
...
def test_model_flow_nous_prints_subscription_guidance_without_mutating_explicit_tts(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        "nexai_cli.auth.get_provider_auth_state",
    ...
    monkeypatch.setattr(
        "nexai_cli.auth.fetch_nous_models",
        lambda *args, **kwargs: ["claude-opus-4-6"],
    )
    monkeypatch.setattr("nexai_cli.auth._prompt_model_selection", lambda model_ids, current_model="", pricing=None, **kw: "claude-opus-4-6")
    monkeypatch.setattr("nexai_cli.auth._save_model_choice", lambda model: None)
    monkeypatch.setattr("nexai_cli.auth._update_config_for_provider", lambda provider, url: None)

    nexai_main._model_flow_nous(config, current_model="claude-opus-4-6")
...
def test_codex_provider_preserves_explicit_codex_model(monkeypatch):
    ...
    shell = cli.nexaiCLI(model="gpt-5.1-codex-mini", compact=True, max_turns=1)
...
def test_codex_provider_strips_provider_prefix_from_model(monkeypatch):
    ...
    shell = cli.nexaiCLI(model="openai/gpt-5.3-codex", compact=True, max_turns=1)
...
def test_cmd_model_falls_back_to_auto_on_invalid_provider(monkeypatch, capsys):
    monkeypatch.setattr(
        "nexai_cli.config.load_config",
        lambda: {"model": {"default": "gpt-5", "provider": "invalid-provider"}},
    )
    monkeypatch.setattr("nexai_cli.config.save_config", lambda cfg: None)
    monkeypatch.setattr("nexai_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("nexai_cli.config.save_env_value", lambda key, value: None)
    ...
    monkeypatch.setattr("nexai_cli.auth.resolve_provider", _resolve_provider)
    monkeypatch.setattr(nexai_main, "_prompt_provider_choice", lambda choices, **kwargs: len(choices) - 1)
    monkeypatch.setattr("sys.stdin", type("FakeTTY", (), {"isatty": lambda self: True})())

    nexai_main.cmd_model(SimpleNamespace())
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    monkeypatch.setattr(
        "nexai_cli.config.get_env_value",
        lambda key: "" if key in {"OPENAI_BASE_URL", "OPENAI_API_KEY"} else "",
    )
    saved_env = {}
    monkeypatch.setattr("nexai_cli.config.save_env_value", lambda key, value: saved_env.__setitem__(key, value))
    monkeypatch.setattr("nexai_cli.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("nexai_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("nexai_cli.main._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        "nexai_cli.models.probe_api_models",
...
def test_cmd_model_forwards_nous_login_tls_options(monkeypatch):
    monkeypatch.setattr(nexai_main, "_require_tty", lambda *a: None)
    monkeypatch.setattr(
        "nexai_cli.config.load_config",
    ...
    monkeypatch.setattr("nexai_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("nexai_cli.config.save_env_value", lambda key, value: None)
    monkeypatch.setattr("nexai_cli.auth.resolve_provider", lambda requested, **kwargs: "nous")
    monkeypatch.setattr("nexai_cli.auth.get_provider_auth_state", lambda provider_id: None)
    monkeypatch.setattr(nexai_main, "_prompt_provider_choice", lambda choices, **kwargs: 0)
    ...
    monkeypatch.setattr("nexai_cli.auth._login_nous", _fake_login)
...
def test_auto_provider_name_localhost():
    from nexai_cli.main import _auto_provider_name
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    import yaml
    from nexai_cli.main import _save_custom_provider

    cfg_path = tmp_path / "config.yaml"
    cfg_path.write_text(yaml.dump({}))

    monkeypatch.setattr(
        "nexai_cli.config.load_config", lambda: yaml.safe_load(cfg_path.read_text()) or {},
    )
    saved = {}
    def _save(cfg):
        saved.update(cfg)
    monkeypatch.setattr("nexai_cli.config.save_config", _save)
