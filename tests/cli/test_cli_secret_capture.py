...
def test_secret_capture_callback_can_be_completed_from_cli_state_machine():
    ...
    with patch("nexai_cli.callbacks.save_env_value_secure") as save_secret:
...
def test_secret_capture_uses_getpass_without_tui():
    ...
    with patch("nexai_cli.callbacks.getpass.getpass", return_value="secret-value"), patch(
        "nexai_cli.callbacks.save_env_value_secure"
    ) as save_secret:
        save_secret.return_value = {
            "success": True,
            "stored_as": "TENOR_API_KEY",
...
def test_secret_capture_timeout_clears_hidden_input_buffer():
    ...
    with patch("nexai_cli.callbacks.queue.Queue.get", side_effect=queue.Empty), patch(
        "nexai_cli.callbacks._time.monotonic",
        side_effect=[0, 121],
    ):
        result = prompt_for_secret(cli, "TENOR_API_KEY", "Tenor API key")
...
def test_cli_chat_registers_secret_capture_callback():
    ...
    with patch("cli.get_tool_definitions", return_value=[]), patch.dict(
        "os.environ", {"LLM_MODEL": "", "NEXAI_MAX_ITERATIONS": ""}, clear=False
