...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("nexai/nexai-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned nexai/* and pr-* branches."""

    def test_prunes_orphaned_nexai_branch(self, git_repo):
        """nexai/nexai-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "nexai/nexai-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "nexai/nexai-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "nexai/nexai-deadbeef" in result.stdout
        ...
        assert "nexai/nexai-deadbeef" in orphaned
        ...
        assert "nexai/nexai-deadbeef" not in result.stdout
