...
def _isolate(tmp_path, monkeypatch):
    """Isolate NEXAI_HOME so tests don't touch real config."""
    home = tmp_path / ".nexai"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal nexaiCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = nexaiCLI.__new__(nexaiCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["nexai-core"]
        obj.compact = False
