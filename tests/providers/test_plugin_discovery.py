...
Verifies that:
 ...
 2. User plugins at $NEXAI_HOME/plugins/model-providers/<name>/ override bundled
...
def _clear_provider_caches():
    ...
    for mod in list(sys.modules.keys()):
        if (
            ...
            or mod.startswith("_nexai_user_provider")
        ):
            del sys.modules[mod]
...
def test_general_plugin_manager_skips_model_provider_kind(tmp_path, monkeypatch):
    ...
    from nexai_cli import plugins as plugin_mod
    ...
    user_plugin = nexai_home / "plugins" / "test-model-provider"
