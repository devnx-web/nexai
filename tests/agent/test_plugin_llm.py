...
""",
        ...
        from nexai_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        nexai_home = tmp_path / ".nexai"
        nexai_home.mkdir()
        (nexai_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("NEXAI_HOME", str(nexai_home))
        from nexai_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from nexai_cli.plugins import PluginContext, PluginManifest, PluginManager
