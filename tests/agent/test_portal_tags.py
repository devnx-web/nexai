...
def test_nexai_client_tag_includes_current_version():
    """The client tag must reflect nexai_cli.__version__ verbatim."""
    from nexai_cli import __version__
    from agent.portal_tags import nexai_client_tag

    assert nexai_client_tag() == f"client=nexai-client-v{__version__}"


def test_nexai_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import nexai_client_tag

    tag = nexai_client_tag()
    assert tag.startswith("client=nexai-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import nexai_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=nexai-agent" in tags
    assert nexai_client_tag() in tags
    assert len(tags) == 2
