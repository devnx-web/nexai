...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "nexai Tests")
...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_nexai_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("NEXAI_HOME", str(tmp_path / ".nexai"))

    nexai_env = tmp_path / ".nexai" / ".env"
    nexai_env.parent.mkdir(parents=True)
    nexai_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.nexai/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
