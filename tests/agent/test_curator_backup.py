...
def backup_env(monkeypatch, tmp_path):
    """Isolate NEXAI_HOME + reload modules so every test starts clean."""
    home = tmp_path / ".nexai"
    home.mkdir()
    (home / "skills").mkdir()
    ...
    # Reload so get_nexai_home picks up the env var fresh.
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd nexai_constants
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under NEXAI_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level NEXAI_DIR picks up the tmp HOME."""
    import nexai_constants
    importlib.reload(nexai_constants)
...
def test_snapshot_without_cron_jobs_file_still_succeeds(backup_env):
    ...
    cb = backup_env["cb"]
    _write_skill(backup_env["skills"], "alpha")
    # Deliberately do not create ~/.nexai/cron/jobs.json
