...
class TestBuildOrHeaders:
    ...
    def test_none_config_falls_back_to_load_config(self):
        ...
        with patch("nexai_cli.config.load_config", return_value=fake_cfg):
    ...
    def test_none_config_load_config_fails_gracefully(self):
        ...
        with patch("nexai_cli.config.load_config", side_effect=RuntimeError("boom")):
            headers = build_or_headers(or_config=None)
...
class TestEnvVarOverrides:
    ...
    def test_env_enables_cache(self, monkeypatch):
        """NEXAI_OPENROUTER_CACHE=true enables cache even when config disables it."""
        from agent.auxiliary_client import build_or_headers

        monkeypatch.setenv("NEXAI_OPENROUTER_CACHE", "true")
        headers = build_or_headers(or_config={"response_cache": False})
        assert headers["X-OpenRouter-Cache"] == "true"

    def test_env_disables_cache(self, monkeypatch):
        """NEXAI_OPENROUTER_CACHE=false disables cache even when config enables it."""
        from agent.auxiliary_client import build_or_headers

        monkeypatch.setenv("NEXAI_OPENROUTER_CACHE", "false")
        headers = build_or_headers(or_config={"response_cache": True})
        assert "X-OpenRouter-Cache" not in headers
    ...
    def test_truthy_values(self, monkeypatch, value):
        ...
        monkeypatch.setenv("NEXAI_OPENROUTER_CACHE", value)
        headers = build_or_headers(or_config={})
        assert headers["X-OpenRouter-Cache"] == "true"
    ...
    def test_env_ttl_overrides_config(self, monkeypatch):
        """NEXAI_OPENROUTER_CACHE_TTL overrides config TTL."""
        from agent.auxiliary_client import build_or_headers

        monkeypatch.setenv("NEXAI_OPENROUTER_CACHE", "true")
        monkeypatch.setenv("NEXAI_OPENROUTER_CACHE_TTL", "1800")
    ...
    def test_invalid_env_ttl_dropped(self, monkeypatch, ttl):
        ...
        monkeypatch.setenv("NEXAI_OPENROUTER_CACHE", "1")
        monkeypatch.setenv("NEXAI_OPENROUTER_CACHE_TTL", ttl)
        headers = build_or_headers(or_config={})
        assert headers["X-OpenRouter-Cache"] == "true"
        assert "X-OpenRouter-Cache-TTL" not in headers
    ...
    def test_valid_env_ttl_boundaries(self, monkeypatch, ttl):
        ...
        monkeypatch.setenv("NEXAI_OPENROUTER_CACHE", "yes")
        monkeypatch.setenv("NEXAI_OPENROUTER_CACHE_TTL", ttl)
    ...
    def test_no_env_vars_falls_through_to_config(self, monkeypatch):
        ...
        monkeypatch.delenv("NEXAI_OPENROUTER_CACHE", raising=False)
        monkeypatch.delenv("NEXAI_OPENROUTER_CACHE_TTL", raising=False)
