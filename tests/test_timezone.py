"""
Tests for timezone support (nexai_time module + integration points).
...
def _reset_nexai_time_cache():
    """Reset the nexai_time module cache (replacement for removed reset_cache)."""
    nexai_time._cached_tz = None
    nexai_time._cached_tz_name = None
    nexai_time._cache_resolved = False


# =========================================================================
# nexai_time.now() — core helper
# =========================================================================

class TestnexaiTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_nexai_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["NEXAI_TIMEZONE"] = "Asia/Kolkata"
        result = nexai_time.now()
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["NEXAI_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="nexai_time"):
            result = nexai_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["NEXAI_TIMEZONE"] = "Europe/London"
        tz = nexai_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["NEXAI_TIMEZONE"] = "Not/A/Timezone"
        tz = nexai_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When NEXAI_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) NEXAI_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("NEXAI_TIMEZONE=" + os.environ.get("NEXAI_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "NEXAI_TIMEZONE=NOT_SET" in result["output"], (
            "NEXAI_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When NEXAI_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("NEXAI_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=nexai_tz) which shifted
        absolute time when system-local tz != nexai tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_nexai_tz(self):
        """Already-aware datetimes should be normalized to nexai tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in nexai tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of nexai tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), nexai is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when nexai tz
        ...
        # Use a nexai timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _nexai_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["NEXAI_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_nexai_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of nexai timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["NEXAI_TIMEZONE"] = "US/Eastern"
        _reset_nexai_time_cache()
