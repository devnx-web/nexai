...
def _clean_state():
    ...
    saved = {}
    for k in ("NEXAI_INTERACTIVE", "NEXAI_GATEWAY_SESSION", "NEXAI_EXEC_ASK", "NEXAI_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["NEXAI_INTERACTIVE"] = "1"
        session_key = os.getenv("NEXAI_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No NEXAI_INTERACTIVE or NEXAI_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
