...
def test_nexai_provider_subclass_exists():
    """nexaiMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _NEXAI_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _NEXAI_PROVIDER_CLS is not None
    assert issubclass(_NEXAI_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_nexai_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns nexaiMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _NEXAI_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("NEXAI_HOME", str(tmp_path))
    ...
    assert _NEXAI_PROVIDER_CLS is not None
    assert isinstance(provider, _NEXAI_PROVIDER_CLS)
    assert provider._nexai_server_name == "srv"
