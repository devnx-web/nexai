...
# ---------------------------------------------------------------------------
# Toolset: discord / discord_admin only in nexai-discord
# ---------------------------------------------------------------------------

class TestToolsetInclusion:
    def test_discord_tools_in_nexai_discord_toolset(self):
        from toolsets import TOOLSETS
        assert "discord" in TOOLSETS["nexai-discord"]["tools"]
        assert "discord_admin" in TOOLSETS["nexai-discord"]["tools"]

    def test_discord_tools_not_in_core_tools(self):
        from toolsets import _NEXAI_CORE_TOOLS
        assert "discord" not in _NEXAI_CORE_TOOLS
        assert "discord_admin" not in _NEXAI_CORE_TOOLS

    def test_discord_tools_not_in_other_toolsets(self):
        from toolsets import TOOLSETS
        for name, ts in TOOLSETS.items():
            if name in ("nexai-discord", "nexai-gateway", "discord", "discord_admin"):
                continue
            tools = ts.get("tools", [])
...
class TestConfigAllowlist:
    ...
    def test_config_load_failure_is_permissive(self, monkeypatch):
        ...
        monkeypatch.setattr("nexai_cli.config.load_config", bad_load)
        assert _load_allowed_actions_config() is None
...
class TestModelToolsIntegration:
    ...
    def test_discord_admin_schema_rebuilt_by_get_tool_definitions(
        ...
        tools = get_tool_definitions(enabled_toolsets=["nexai-discord"], quiet_mode=True)
