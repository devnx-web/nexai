...
def _skill_dir(tmp_path):
    ...
    only the temp directory — not the real ~/.nexai/skills/."""
...
class TestSecurityScanGate:
    ...
    def test_guard_flag_reads_config_default_false(self):
        ...
        with patch("nexai_cli.config.load_config", return_value={"skills": {}}):
    ...
    def test_guard_flag_reads_config_when_set(self):
        ...
        with patch("nexai_cli.config.load_config",
...
class TestExternalSkillMutations:
    ...
    caused agents to create duplicate copies in ~/.nexai/skills/ as a
    workaround.
    """
...
# come up. The user unpins via `nexai curator unpin <name>` to delete.
# ---------------------------------------------------------------------------
...
class TestPinnedGuard:
    ...
    def test_delete_refuses_pinned(self, tmp_path):
        ...
        assert "nexai curator unpin my-skill" in result["error"]
        # Skill still exists
        assert (tmp_path / "my-skill" / "SKILL.md").exists()
