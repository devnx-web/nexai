...
class TestFirecrawlClientConfig:
    ...
    def test_tool_gateway_domain_builds_firecrawl_gateway_origin(self):
        ...
        with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "devnx.com"}):
            with patch("tools.web_tools._read_nous_access_token", return_value="nous-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    ...
                    mock_fc.assert_called_once_with(
                        api_key="nous-token",
                        api_url="https://firecrawl-gateway.devnx.com",
                    )
        ...
        with patch.dict(os.environ, {
            ...
            with patch("tools.web_tools._read_nous_access_token", return_value="nous-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    ...
                    mock_fc.assert_called_once_with(
                        api_key="nous-token",
                        api_url="http://firecrawl-gateway.devnx.com",
                    )
                    assert result is mock_fc.return_value
    ...
    def test_nous_auth_token_respects_nexai_home_override(self, tmp_path):
        """Auth lookup should read from NEXAI_HOME/auth.json, not ~/.nexai/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".nexai").mkdir(parents=True)
...
class TestBackendSelection:
    ...
    ``nexai tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
