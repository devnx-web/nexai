...
def _restore_tool_modules():
    original_nexai_home = os.environ.get("NEXAI_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_nexai_home is None:
            os.environ.pop("NEXAI_HOME", None)
        else:
            os.environ["NEXAI_HOME"] = original_nexai_home
        _reset_modules(("tools", "nexai_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    nexai_cli = types.ModuleType("nexai_cli")
    nexai_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["nexai_cli"] = nexai_cli
    nexai_home = tmp_path / "nexai-home"
    os.environ["NEXAI_HOME"] = str(nexai_home)
    sys.modules["nexai_cli.config"] = types.SimpleNamespace(
        get_nexai_home=lambda: nexai_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="nexai-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": nexai_home / "modal_snapshots.json",
