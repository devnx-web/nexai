...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._nexai_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_nexai_bin(self):
        """After _INSTALL_FAILED, manual install in NEXAI_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        nexai_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(nexai_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(nexai_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._nexai_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == nexai_bin
            assert _tirith_mod._resolved_path == nexai_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# NEXAI_HOME isolation
# ---------------------------------------------------------------------------

class TestnexaiHomeIsolation:
    def test_nexai_bin_dir_respects_nexai_home(self):
        """_nexai_bin_dir must use NEXAI_HOME, not hardcoded ~/.nexai."""
        from tools.tirith_security import _nexai_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"NEXAI_HOME": tmpdir}):
            result = _nexai_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_nexai_home(self):
        """_failure_marker_path must use NEXAI_HOME, not hardcoded ~/.nexai."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"NEXAI_HOME": "/custom/nexai"}):
            result = _failure_marker_path()
        assert result == "/custom/nexai/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets NEXAI_HOME; verify it's active."""
        nexai_home = os.getenv("NEXAI_HOME")
        assert nexai_home is not None, "NEXAI_HOME should be set by conftest"
        assert "nexai_test" in nexai_home, "Should point to test temp dir"

    def test_get_nexai_home_fallback(self):
        """Without NEXAI_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_nexai_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove NEXAI_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".nexai")
            result = _get_nexai_home()
