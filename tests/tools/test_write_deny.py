...
class TestWriteDenyExactPaths:
    ...
    def test_nexai_env(self):
        # ``.env`` under the active NEXAI_HOME (profile-aware, not just
        # ``~/.nexai``) must be write-denied. The hermetic test conftest
        # points NEXAI_HOME at a tempdir — resolve via get_nexai_home()
        # to match the denylist.
        from nexai_constants import get_nexai_home
        path = str(get_nexai_home() / ".env")
        assert _is_write_denied(path) is True
...
class TestWriteAllowed:
    ...
    def test_nexai_config_not_env(self):
        path = os.path.join(str(Path.home()), ".nexai", "config.yaml")
