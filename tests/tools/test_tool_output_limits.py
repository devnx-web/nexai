...
class TestDefaults:
    ...
    def test_get_limits_returns_defaults_when_config_not_a_dict(self):
        ...
        with patch("nexai_cli.config.load_config", return_value="not a dict"):
            limits = tol.get_tool_output_limits()
    ...
    def test_get_limits_returns_defaults_when_load_config_raises(self):
        ...
        with patch("nexai_cli.config.load_config", side_effect=_boom):
            limits = tol.get_tool_output_limits()
...
class TestDefaultConfigHasSection:
    """The DEFAULT_CONFIG in nexai_cli.config must expose tool_output so
    that ``nexai setup`` and default installs stay in sync with the
    helpers here."""
