"""Shared fixtures for the nexai-agent test suite.
...
2. **Isolated NEXAI_HOME.** NEXAI_HOME points to a per-test tempdir so
   code reading ``~/.nexai/*`` via ``get_nexai_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".nexai"`` instead of the canonical
   ``get_nexai_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No NEXAI_SESSION_* inheritance** — the agent's current gateway
...
# nexai_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_NEXAI_BEHAVIORAL_VARS = frozenset({
    "NEXAI_YOLO_MODE",
    "NEXAI_INTERACTIVE",
    "NEXAI_QUIET",
    "NEXAI_TOOL_PROGRESS",
    ...
    "NEXAI_SESSION_CHAT_ID",
    "NEXAI_SESSION_CHAT_NAME",
    "NEXAI_SESSION_THREAD_ID",
    "NEXAI_SESSION_SOURCE",
    "NEXAI_SESSION_KEY",
    "NEXAI_GATEWAY_SESSION",
    "NEXAI_PLATFORM",
    "NEXAI_MODEL",
    "NEXAI_INFERENCE_MODEL",
    "NEXAI_INFERENCE_PROVIDER",
    "NEXAI_TUI_PROVIDER",
    "NEXAI_MANAGED",
    "NEXAI_DEV",
    "NEXAI_CONTAINER",
    ...
    "NEXAI_TIMEZONE",
    "NEXAI_REDACT_SECRETS",
    "NEXAI_BACKGROUND_NOTIFICATIONS",
    "NEXAI_EXEC_ASK",
    "NEXAI_HOME_MODE",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.nexai/kanban.db instead of the per-test NEXAI_HOME.
    "NEXAI_KANBAN_DB",
    "NEXAI_KANBAN_BOARD",
    "NEXAI_KANBAN_WORKSPACES_ROOT",
    "NEXAI_KANBAN_LOGS_ROOT",
    "NEXAI_KANBAN_TASK",
    "NEXAI_KANBAN_WORKSPACE",
    "NEXAI_TENANT",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and NEXAI_HOME to per-test tempdirs so code that
    reads ``~/.nexai/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral nexai_* vars that could change test semantics.
    for name in _NEXAI_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect NEXAI_HOME to a per-test tempdir. Code that reads
    #    ``~/.nexai/*`` via ``get_nexai_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.nexai/*`` via
    #    ``Path.home() / ".nexai"`` instead of ``get_nexai_home()``
    #    is a bug to fix at the callsite.
    fake_nexai_home = tmp_path / "nexai_test"
    fake_nexai_home.mkdir()
    (fake_nexai_home / "sessions").mkdir()
    (fake_nexai_home / "cron").mkdir()
    (fake_nexai_home / "memories").mkdir()
    (fake_nexai_home / "skills").mkdir()
    monkeypatch.setenv("NEXAI_HOME", str(fake_nexai_home))
    ...
    #    ~/.nexai/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import nexai_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_nexai_home(_hermetic_environment):
...
def _reset_module_state():
    ...
    for _logger_name in ("tools", "run_agent", "trajectory_compressor", "cron", "nexai_cli"):
...
@pytest.fixture()
def mock_config():
    """Return a minimal nexai config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``nexai-gateway`` process
...
# correlated with a ``tests/nexai_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> nexai-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart nexai-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting nexai/python patterns are also blocked.
    """
    ...
    def _matches_nexai_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _NEXAI_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_nexai_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch nexai-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m nexai_cli.main".
                if (
                    "nexai" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live nexai-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting nexai/python could hit the live gateway. "
