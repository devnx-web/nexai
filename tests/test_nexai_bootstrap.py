"""Tests for nexai_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every nexai entry point
(nexai, nexai-agent, nexai-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every nexai entry point imports nexai_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported nexai_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("nexai_bootstrap", None)
    import nexai_bootstrap  # noqa: WPS433
    return nexai_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the nexai CLI
...
class TestEntryPointsImportBootstrap:
    """Every nexai entry point must import nexai_bootstrap as its
    ...
    # Entry points that invoke nexai as a process.  Each one must
    # import nexai_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "nexai_cli/main.py",   # nexai CLI (console_script)
        "run_agent.py",          # nexai-agent (console_script)
        "acp_adapter/entry.py",  # nexai-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import nexai_bootstrap' and that
        ...
        half-finished ``nexai update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``nexai_bootstrap`` as a top-level module) leaves nexai
        ...
        """
        # Resolve relative to the nexai-agent repo root.  Tests live
        # at tests/test_nexai_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # nexai start even when nexai_bootstrap hasn't been
        ...
        assert first_import_name == "nexai_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'nexai_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import nexai_bootstrap' line to be the first import."
        )
