...
def server():
    with patch.dict(
        "sys.modules",
        {
            "nexai_constants": MagicMock(
                get_nexai_home=MagicMock(return_value="/tmp/nexai_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'nexai-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'nexai-release' patched"
    }
