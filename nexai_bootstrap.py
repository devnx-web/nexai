"""Windows UTF-8 bootstrap for nexai entry points.
...
should be imported at the very top of every nexai entry point
(``nexai``, ``nexai-agent``, ``nexai-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import nexai_bootstrap``
# (or ``from nexai_bootstrap import apply_windows_utf8_bootstrap``) at
