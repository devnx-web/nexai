...
## Client ID
- Need to register a "Desktop app" OAuth client on a DevNX GCP project
...
## Token Lifecycle
- Store at `~/.nexai/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `nexai_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `nexai_cli/models.py` — Add Gemini model catalog
4. `nexai_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `nexai_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `nexai_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
## Prerequisites
- DevNX GCP project with Desktop OAuth client registered
- OR: accept user-provided client_id via NEXAI_GEMINI_CLIENT_ID env var
...
- nexai-agent Copilot OAuth: `nexai_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
