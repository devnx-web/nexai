"""
nexaiSweEnv -- SWE-Bench Style Environment with Modal Sandboxes
...
Usage:
    ...
    vllm serve YourModel --tool-parser nexai
    run-api
    python environments/nexai_swe_env.py serve \\
...
class nexaiSweEnv(nexaiAgentBaseEnv):
    ...
    name = "nexai-swe"
    env_config_cls = nexaiSweEnvConfig

    @classmethod
    def config_init(cls) -> Tuple[nexaiSweEnvConfig, List[APIServerConfig]]:
        """
        ...
        env_config = nexaiSweEnvConfig(
            ...
            # Atropos settings
            group_size=4,
            tokenizer_name="NousResearch/Deepnexai-3-Llama-3-3B-Preview",
            tool_call_parser="nexai",
            steps_per_eval=50,
            total_steps=500,
            use_wandb=True,
            wandb_name="nexai-swe",
        )

        server_configs = [
            APIServerConfig(
                base_url="http://localhost:8000/v1",
                model_name="NousResearch/Deepnexai-3-Llama-3-3B-Preview",
