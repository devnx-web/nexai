...
Training tasks (3):
    1. Create ~/greeting.txt with "Hello from nexai Agent"
...
TRAIN_TASKS = [
    {
        "prompt": "Create a file at ~/greeting.txt containing exactly the text: Hello from nexai Agent",
        "verify_path": "~/greeting.txt",
        "expected_content": "Hello from nexai Agent",
    },
    {
...
class TerminalTestEnv(nexaiAgentBaseEnv):
    ...
    def config_init(cls) -> Tuple[TerminalTestEnvConfig, List[APIServerConfig]]:
        ...
        Claude for inference. API keys loaded from ~/nexai-agent/.env.
        """
        env_config = TerminalTestEnvConfig(
            ...
            tokenizer_name="DevNX/q-30b-t-h45-e1",
