"""
nexaiAgentBaseEnv -- Abstract Base Environment for nexai-Agent + Atropos

Provides the Atropos integration plumbing that all nexai-agent environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via nexaiAgentLoop
...
# Ensure the nexai-agent repo root is on sys.path so that imports like
...
# Load API keys from nexai-agent/.env so all environments can access them
...
from environments.agent_loop import AgentResult, nexaiAgentLoop
...
# Import nexai-agent toolset infrastructure
...
class nexaiAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for nexai-agent Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of nexai toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: nexai, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
    ...
    default_result_size_chars: int = Field(
        ...
        "to sandbox. Results exceeding this are written to /tmp/nexai-results/ "
...
class nexaiAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for nexai-agent Atropos integration.
    ...
    name: Optional[str] = "nexai-agent"
    env_config_cls = nexaiAgentEnvConfig

    def __init__(
        self,
        config: nexaiAgentEnvConfig,
        ...
        # Set terminal environment variables so nexai tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY nexai-agent tool (terminal, file, web,
