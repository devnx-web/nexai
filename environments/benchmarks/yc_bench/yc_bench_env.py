...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs nexaiAgentLoop with terminal tool only
...
Requires: pip install nexai-agent[yc-bench]
"""
...
class YCBenchEvalConfig(nexaiAgentEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends nexaiAgentEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(nexaiAgentBaseEnv):
    """
    ...
    a competing built-in agent loop). The nexaiAgentLoop then drives the
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs nexaiAgentLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our nexaiAgentLoop.
