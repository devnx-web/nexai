"""Persistent slash-command worker — one nexaiCLI per TUI session.
...
def _run(cli: nexaiCLI, command: str) -> str:
...
def main():
    ...
    os.environ["NEXAI_SESSION_KEY"] = args.session_key
    os.environ["NEXAI_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = nexaiCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
