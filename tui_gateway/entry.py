...
# nexai modules.  nexai_cli sets NEXAI_PYTHON_SRC_ROOT before spawning this
# subprocess; inserting it first ensures the installed packages win.
_src_root = os.environ.get("NEXAI_PYTHON_SRC_ROOT", "")
...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `NEXAI_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("NEXAI_TUI_SIDECAR_URL")
...
# tested; override via ``NEXAI_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("NEXAI_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``NEXAI_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
# ``nexai --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
