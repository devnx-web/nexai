...
# 5. Symlinks the 'nexai' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ nexai Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/nexai-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink nexai into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up nexai command..."

NEXAI_BIN="$SCRIPT_DIR/venv/bin/nexai"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$NEXAI_BIN" "$COMMAND_LINK_DIR/nexai"
echo -e "${GREEN}✓${NC} Symlinked nexai → $COMMAND_LINK_DISPLAY_DIR/nexai"
...
# ============================================================================
# Seed bundled skills into ~/.nexai/skills/
# ============================================================================

NEXAI_SKILLS_DIR="${NEXAI_HOME:-$HOME/.nexai}/skills"
mkdir -p "$NEXAI_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.nexai/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$NEXAI_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  nexai status        # Check configuration"
if is_termux; then
    echo "  nexai gateway       # Run gateway in foreground"
else
    echo "  nexai gateway install # Install gateway service (messaging + cron)"
fi
echo "  nexai cron list     # View scheduled jobs"
echo "  nexai doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m nexai_cli.main setup
fi
