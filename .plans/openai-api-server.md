# OpenAI-Compatible API Server for NexAi Agent
...
SSE streaming. By exposing this endpoint, nexai-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  nexai-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.nexai/.env`
2. Run `nexai gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with NexAi-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns nexai-agent as a model) |
...
Authorization: Bearer nexai-api-key-here
...
But nexai-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "NexAi-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured nexai-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of NexAi-agent's core prompt
...
Once implemented, nexai-agent works as a drop-in backend for:
