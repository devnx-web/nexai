...
_NEXAI_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (NEXAI_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (NEXAI_KANBAN_TASK env "
    ...
    # ==========================================================================
    # Full nexai toolsets (CLI + messaging platforms)
    #
    ...
    "nexai-cli": {
        ...
        "tools": _NEXAI_CORE_TOOLS,
        "includes": []
    },

    "nexai-cron": {
        # Mirrors nexai-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `nexai tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant, rl) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as nexai-cli; gated by `nexai tools`",
        "tools": _NEXAI_CORE_TOOLS,
        "includes": []
    },
    ...
    "nexai-email": {
        "description": "Email bot toolset - interact with nexai via email (IMAP/SMTP)",
        "tools": _NEXAI_CORE_TOOLS,
        "includes": []
    },
    ...
    "nexai-sms": {
        "description": "SMS bot toolset - interact with nexai via SMS (Twilio)",
        "tools": _NEXAI_CORE_TOOLS,
        "includes": []
    },
    ...
    "nexai-gateway": {
        ...
        "tools": [],
        "includes": ["nexai-telegram", "nexai-discord", "nexai-whatsapp", "nexai-slack", "nexai-signal", "nexai-bluebubbles", "nexai-homeassistant", "nexai-email", "nexai-sms", "nexai-mattermost", "nexai-matrix", "nexai-dingtalk", "nexai-feishu", "nexai-wecom", "nexai-wecom-callback", "nexai-weixin", "nexai-qqbot", "nexai-webhook", "nexai-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (nexai-<name>).
        # Gives them _NEXAI_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("nexai-"):
            platform_name = name[len("nexai-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_NEXAI_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
