...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - nexai_home (str): The active NEXAI_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.nexai``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "nexai").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'nexai memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], nexai_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'nexai memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``nexai_home`` is the active NEXAI_HOME directory path.
