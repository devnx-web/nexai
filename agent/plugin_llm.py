...
Plugins built on nexai Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing nexai
...
:class:`~nexai_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors nexai' main config
...
handles every provider, fallback chain, and per-task override nexai
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`nexai_cli.plugins.PluginContext`
