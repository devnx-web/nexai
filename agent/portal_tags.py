...
Every nexai request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to nexai Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=nexai-agent",
        "client=nexai-client-v<__version__>",
    ]

The version is sourced live from ``nexai_cli.__version__`` so it auto-aligns
...
``nexai_cli.__version__`` is the canonical source of truth.
"""
...
def _nexai_version() -> str:
    """Return the current nexai release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``nexai_cli`` cannot be imported (should
...
def nexai_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=nexai-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=nexai-client-v{_nexai_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=nexai-agent", nexai_client_tag()]
