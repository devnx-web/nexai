...
from nexai_constants import get_config_path, get_skills_dir
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``NEXAI_PLATFORM`` or
            ``NEXAI_SESSION_PLATFORM`` env vars.  Falls back to the
...
# which becomes the dominant cost of ``nexai`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.nexai/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``nexai`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against NEXAI_HOME, not cwd
        if not p.is_absolute():
            p = (nexai_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.nexai/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    nexai = metadata.get("nexai") or {}
    if not isinstance(nexai, dict):
        nexai = {}
    return {
        "fallback_for_toolsets": nexai.get("fallback_for_toolsets", []),
        "requires_toolsets": nexai.get("requires_toolsets", []),
        "fallback_for_tools": nexai.get("fallback_for_tools", []),
        "requires_tools": nexai.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    nexai = metadata.get("nexai")
    if not isinstance(nexai, dict):
        return []
    raw = nexai.get("config")
    if not raw:
        return []
