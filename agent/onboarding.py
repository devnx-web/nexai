...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time nexai starts and finds ``~/.openclaw/``.

    Points users at ``nexai claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``nexai claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to nexai, run "
        "`nexai claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `nexai claw cleanup` (renames it to ~/.openclaw.pre-migration — "
