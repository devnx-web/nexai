...
A pre-run snapshot of ``~/.nexai/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.nexai/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.nexai/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def _backups_dir() -> Path:
    return get_nexai_home() / "skills" / ".curator_backups"
...
def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.nexai/cron/jobs.json``)."""
    return get_nexai_home() / "cron" / "jobs.json"
...
def snapshot_skills(reason: str = "manual") -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.nexai/skills/`` and prune old ones.
    ...
    if not skills.exists():
        logger.debug("No ~/.nexai/skills/ directory — nothing to back up")
        return None
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.nexai/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.nexai/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `nexai curator rollback --list` to see available snapshots)",
            None,
        )
