"""Anthropic Messages API adapter for nexai Agent.

Translates between nexai's internal OpenAI-style message format and
...
# nexai effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    nexai historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by nexai)
    ...
    # 1. nexai-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration nexai configs.
...
# ── nexai-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.nexai/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_NEXAI_OAUTH_FILE = get_nexai_home() / ".anthropic_oauth.json"
...
def run_nexai_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run nexai-native OAuth PKCE flow and return credential state."""
    import time
    import webbrowser
    ...
    print()
    print("Authorize nexai with your Claude Pro/Max subscription.")
    print()
...
def read_nexai_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read nexai-managed OAuth credentials from ~/.nexai/.anthropic_oauth.json."""
    if _NEXAI_OAUTH_FILE.exists():
        try:
            data = json.loads(_NEXAI_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read nexai OAuth credentials: %s", e)
    return None
    ...
    *base_url* is a Kimi / Moonshot host), unsigned thinking blocks
    ...
    for m in messages:
        ...
        if role == "assistant":
            ...
            # can validate the message history.  See nexai-agent#13848.
            #
    ...
    # are preserved.  See nexai-agent#13848 (Kimi) and #16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    # silently hides reasoning text that nexai surfaces in its CLI. We
