...
def _state_file() -> Path:
    return get_nexai_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.nexai/config.yaml. Tolerates missing file."""
    try:
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``nexai update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``nexai curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `nexai curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as nexai' background skill CURATOR. This is an "
    ...
    "into ~/.nexai/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: nexai-config-*, nexai-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.nexai/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.nexai/logs/curator/``)
    ...
    authored skill data in ``~/.nexai/skills/``.

    ``ensure_nexai_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_nexai_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every nexai install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: nexai curator status
        keep an umbrella stable: nexai curator pin document-tools
    ...
    lines.append("full report: nexai curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: nexai curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.nexai/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.nexai/skills/.archive/` for "
            "safety and can be restored via `nexai curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.nexai/skills/.archive/`. "
            "Restore any via `nexai curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `nexai curator restore <name>`")
    lines.append("- All archives live under `~/.nexai/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `nexai curator status`
    ...
    def _llm_pass():
        ...
        # recorded in state so `nexai curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``nexai model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `nexai model` → auxiliary
