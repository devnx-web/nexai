...
from nexai_cli.config import get_env_value, load_env
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``nexai model`` / ``nexai auth`` which
    ...
    def _sync_nous_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        avoiding a "refresh token reuse" revocation on the NexAi Portal.
        """
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("nexai_pkce"),
                )
                updated = replace(
                    entry,
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("nexai_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other nexai profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `nexai model` / `nexai auth` after a 429/401,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `nexai auth remove <provider> <N>` is stable across all source types.
    try:
        from nexai_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, nexai PKCE)
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_nexai_oauth_credentials

        for source_name, creds in (
            ("nexai_pkce", read_nexai_oauth_credentials()),
        ...
        if state and not _is_suppressed(provider, "device_code"):
            ...
            # `nexai auth add nous --label <name>`).  Fall back to the
        ...
        # nexai auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from nexai_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.nexai/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `nexai auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `nexai auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the nexai auth store.  Without this gate the removal is instantly
        ...
        # nexai owns its own Codex auth state — we do NOT auto-import from
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.nexai/.env over os.environ — the user's config file is the
    # authoritative source for nexai credentials. Stale env vars from parent
    ...
    # Honour user suppression — `nexai auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.nexai/.env.
    ...
    try:
        from nexai_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
        def _is_source_suppressed(_p, _s):  # type: ignore[misc]
            return False
    if provider == "openrouter":
        # Prefer ~/.nexai/.env over os.environ
...
def _prune_stale_seeded_entries(entries: List[PooledCredential], active_sources: Set[str]) -> bool:
    retained = [
        ...
        or not (
            ...
            or entry.source in {"claude_code", "nexai_pkce"}
        )
    ]
