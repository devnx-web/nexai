"""Unified removal contract for every credential source nexai reads from.

nexai seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.nexai/.env
    claude_code   — ~/.claude/.credentials.json
    nexai_pkce   — ~/.nexai/.anthropic_oauth.json
    ...
    manual        — user ran `nexai auth add`
...
unify here is **removal**:
    ...
    ``nexai auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), nexai_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.nexai/.env  → clear it
    ...
    """
    from nexai_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to nexai.",
            f"  The pool entry is now suppressed — nexai will ignore "
            f"{env_var} until you run `nexai auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so nexai stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `nexai auth add anthropic` to re-enable if needed.",
    ])


def _remove_nexai_pkce(provider: str, removed) -> RemovalResult:
    """~/.nexai/.anthropic_oauth.json is ours — delete it outright."""
    from nexai_constants import get_nexai_home

    result = RemovalResult()
    oauth_file = get_nexai_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared nexai Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    user's next `nexai login` run from writing providers.nous again
    before they decide to.  Suppression forces them to go through
    `nexai auth add nous` to re-engage, which is the documented re-add
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the nexai auth store is not enough — _seed_from_singletons() would
    ...
    via ``nexai auth add openai-codex``), but in both cases the re-seed
    ...
    """
    from nexai_cli.auth import suppress_credential_source
    ...
    result.hints.extend([
        ...
        "Run `nexai auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `nexai auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    nexai stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `nexai auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="nexai_pkce",
        remove_fn=_remove_nexai_pkce,
        description="~/.nexai/.anthropic_oauth.json",
    ))
    register(RemovalStep(
