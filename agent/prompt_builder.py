...
from nexai_constants import get_nexai_home, get_skills_dir, is_wsl
...
_NEXAI_MD_NAMES = (".nexai.md", "nexai.md")


def _find_nexai_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.nexai.md`` or ``nexai.md``.
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.nexai/kanban.db`. Your task id is in "
    "`$NEXAI_KANBAN_TASK`; your workspace is `$NEXAI_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $NEXAI_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `nexai kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the nexai WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where nexai itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host nexai runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where nexai itself is running. The host OS, home, and cwd "
                f"of the nexai process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where nexai "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_nexai_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.nexai/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("NEXAI_PLATFORM")
        or get_session_env("NEXAI_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot nexai Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `nexai-agent` skill "
            "first. It has the actual commands (e.g. `nexai config set …`, `nexai tools`, "
            "`nexai setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: nexai setup, nexai setup tools, nexai setup terminal, nexai status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from NEXAI_HOME and return its content, or None.
    ...
    """
    try:
        from nexai_cli.config import ensure_nexai_home
        ensure_nexai_home()
    except Exception as e:
        logger.debug("Could not ensure NEXAI_HOME before loading SOUL.md: %s", e)

    soul_path = get_nexai_home() / "SOUL.md"
...
def _load_nexai_md(cwd_path: Path) -> str:
    """.nexai.md / nexai.md — walk to git root."""
    nexai_md_path = _find_nexai_md(cwd_path)
    if not nexai_md_path:
        return ""
    try:
        content = nexai_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = nexai_md_path.name
        try:
            rel = str(nexai_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".nexai.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", nexai_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .nexai.md / nexai.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from NEXAI_HOME is independent and always included when present.
    ...
    # SOUL.md from NEXAI_HOME only — skip when already loaded as identity
    if not skip_soul:
