...
Resolution order for text tasks (auto mode):
  ...
  3. NexAi Portal (~/.nexai/auth.json active provider)
...
def _compression_threshold_for_model(model: Optional[str]) -> Optional[float]:
    ...
    consumed before nexai triggers summarization.  Higher values delay
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://docs.nexai.dev",
    "X-Title": "nexai Agent",
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``NEXAI_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``NEXAI_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("NEXAI_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("NEXAI_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
# NexAi Portal extra_body for product attribution.
# Callers should pass this as extra_body in chat.completions.create()
# when the auxiliary client is backed by NexAi Portal.
#
# The tags are computed from agent.portal_tags so the client= marker stays
# in lockstep with nexai_cli.__version__ across every Portal call site
# (main loop, aux, compression, web_extract). Do not inline a literal here;
# see agent/portal_tags.py for the rationale.
# from agent.portal_tags import nous_portal_tags as _nous_portal_tags  # removed with nous provider


def _nous_extra_body() -> dict:
    """Return a fresh NexAi Portal ``extra_body`` dict.

    Computed at call time so a hot-reloaded ``nexai_cli.__version__`` is
    reflected without restarting long-running processes.
    """
    return {}  # _nous_portal_tags removed
...
NOUS_EXTRA_BODY = {}  # _nous_extra_body() removed

# Set at resolve time — True if the auxiliary client points to NexAi Portal
auxiliary_is_nous: bool = False

# Default auxiliary models per provider
_OPENROUTER_MODEL = "google/gemini-3-flash-preview"
_NOUS_MODEL = "google/gemini-3-flash-preview"
_NOUS_DEFAULT_BASE_URL = "https://api.example.com/v1"
_ANTHROPIC_DEFAULT_BASE_URL = "https://api.anthropic.com"
_AUTH_JSON_PATH = get_nexai_home() / "auth.json"
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (nexai Agent)",
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``nexai_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.nexai/auth.json for an active Nous provider.
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from nexai auth store.
    ...
    try:
        from nexai_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from nexai_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from nexai_cli.auth import is_provider_explicitly_configured
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    try:
        ...
        if _remaining is not None and _remaining > 0:
            logger.debug(
                "Auxiliary: skipping NexAi Portal (rate-limited, resets in %.0fs)",
                _remaining,
            )
            return None, None
    except Exception:
    ...
    logger.debug("Auxiliary client: NexAi Portal")
    ...
    model = _NOUS_MODEL
    try:
        from nexai_cli.models import get_nous_recommended_aux_model
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `nexai setup` or `nexai model` and enter a valid http(s) base URL."
        ) from exc
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second nexai
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``nexai config aux reset``)."""
...
def _recoverable_pool_provider(resolved_provider: str, client: Any) -> Optional[str]:
    ...
    if base_url_host_matches(base, "openrouter.ai"):
        return "openrouter"
    if base_url_host_matches(base, "inference-api.devnx.com"):
        return "nous"
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    #    scenario where a user switches providers via `nexai model` but the
    #    old OPENAI_BASE_URL lingers in ~/.nexai/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: nexai model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.nexai/.env",
...
def resolve_provider_client(
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but NexAi Portal not configured (run: nexai auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: nexai model)")
                return None, None
    ...
    try:
        from nexai_cli.runtime_provider import _get_named_custom_provider
    ...
    try:
        ...
        logger.debug("nexai_cli.auth not available for provider %s", provider)
        return None, None
...
def resolve_vision_provider_client(
    ...
    *,
    ...
    if requested == "auto":
        ...
        #   3. NexAi Portal (vision-capable aggregator fallback)
        #   4. Stop
        main_provider = _read_main_provider()
...
def get_auxiliary_extra_body() -> dict:
    ...
    Includes NexAi Portal product tags when the auxiliary client is backed
    by NexAi Portal. Returns empty dict otherwise.
    """
    ...
    *,
    ...
    try:
        ...
        client_is_nous = (
            ...
            or base_url_host_matches(_base_info, "inference-api.devnx.com")
        )
...
async def async_call_llm(
    ...
    *,
    ...
    try:
        ...
        client_is_nous = (
            resolved_provider == "nous"
            or base_url_host_matches(_client_base, "inference-api.devnx.com")
        )
        if _is_auth_error(first_err) and client_is_nous:
            refreshed_client, refreshed_model = _refresh_nous_auxiliary_client(
                cache_provider=resolved_provider or "nous",
