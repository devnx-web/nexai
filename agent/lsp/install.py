...
appropriate.  All installs go to a nexai-owned bin staging dir,
``<NEXAI_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``nexai lsp
  status``.
...
# ``<NEXAI_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
def nexai_lsp_bin_dir() -> Path:
    """Return the nexai-owned bin staging dir for LSP servers."""
    home = os.environ.get("NEXAI_HOME")
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".nexai")
    p = Path(home) / "lsp" / "bin"
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = nexai_lsp_bin_dir().parent  # <NEXAI_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a nexai-owned target dir.
    ...
    console script.
    """
    pip_target = nexai_lsp_bin_dir().parent / "python-packages"
    ...
    if bin_path.exists():
        link = nexai_lsp_bin_dir() / bin_name
        if not link.exists():
            try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``nexai lsp status`` CLI to give users a quick
