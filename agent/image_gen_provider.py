...
as ``kind: backend``) or ``~/.nexai/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``nexai tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``nexai tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``nexai tools`` picker.
...
def _images_cache_dir() -> Path:
    """Return ``$NEXAI_HOME/cache/images/``, creating parents as needed."""
    from nexai_constants import get_nexai_home

    path = get_nexai_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$NEXAI_HOME/cache/images/``.
