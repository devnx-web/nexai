"""
Dump command for nexai CLI.

Outputs a compact, plain-text summary of the user's nexai setup
...
from nexai_cli.config import get_nexai_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``nexai dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from nexai_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(nexai_home: Path) -> int:
...
def _cron_summary(nexai_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = nexai_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"nexai_home:      {display_nexai_home()}")
    ...
    api_keys = [
        ...
        # ("NOUS_API_KEY", "nous"),  # provider removed
    ...
    toolsets = config.get("toolsets", ["nexai-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(nexai_home)}")
    lines.append(f"  skills:             {_count_skills(nexai_home)}")
