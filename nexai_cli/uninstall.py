"""
nexai Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.nexai/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove nexai PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing nexai-agent or nexai PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# nexai Agent" comment and following line
                if '# nexai Agent' in line or '# nexai-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('nexai' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing nexai
                if 'nexai' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the nexai wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "nexai",
        Path("/usr/local/bin/nexai"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains nexai_cli reference)
                content = wrapper.read_text()
                if 'nexai_cli' in content or 'nexai-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``nexai gateway run`` processes
    ...
    try:
        from nexai_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from nexai_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``NEXAI_HOME`` and ``NEXAI_GIT_BASH_PATH``
...
def _nexai_path_markers(nexai_home: Path) -> list[str]:
    """Path-entry substrings that identify nexai-owned User-PATH entries."""
    root = str(nexai_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare nexai-agent install dir.
    markers = [root + "\\nexai-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if NEXAI_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "nexai".  We don't want to catch
    # unrelated entries like "cnexai-foo" or "ephermeral", so we look for
    # backslash-nexai as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(nexai_home: Path) -> list[str]:
    """Strip nexai-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _nexai_path_markers(nexai_home)
            kept: list[str] = []
...
def remove_nexai_env_vars_windows() -> list[str]:
    """Delete NEXAI_HOME and NEXAI_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("NEXAI_HOME", "NEXAI_GIT_BASH_PATH"):
                try:
...
def _is_default_nexai_home(nexai_home: Path) -> bool:
    """Return True when ``nexai_home`` points at the default (non-profile) root."""
    try:
        from nexai_constants import get_default_nexai_root
        return nexai_home.resolve() == get_default_nexai_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its NEXAI_HOME directory.

    We shell out to ``nexai -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current NEXAI_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m nexai_cli.main` so we don't depend on a `nexai`
    #    wrapper that may be half-removed mid-uninstall.
    nexai_invocation = [_sys.executable, "-m", "nexai_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                nexai_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's NEXAI_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.nexai/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.nexai/ for future reinstall
    """
    project_root = get_project_root()
    nexai_home = get_nexai_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie NEXAI_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_nexai_home(nexai_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ nexai Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {nexai_home / 'config.yaml'}")
    print(f"  Secrets: {nexai_home / '.env'}")
    print(f"  Data:    {nexai_home / 'cron/'}, {nexai_home / 'sessions/'}, {nexai_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their NEXAI_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL nexai data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the nexai code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in nexai_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\nexai\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(nexai_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No nexai-owned PATH entries in User environment")

        log_info("Removing NEXAI_HOME / NEXAI_GIT_BASH_PATH User env vars...")
        removed_env = remove_nexai_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No nexai-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing nexai command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.nexai/, just remove the nexai-agent subdir
            if nexai_home in project_root.parents or project_root.parent == nexai_home:
    ...
    #     under NEXAI_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(nexai_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(nexai_home)
    ...
    # 5. Optionally remove ~/.nexai/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile NEXAI_HOME dirs live under
        ...
        try:
            if nexai_home.exists():
                shutil.rmtree(nexai_home)
                log_success(f"Removed {nexai_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {nexai_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {nexai_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  irm https://raw.githubusercontent.com/NousResearch/nexai-agent/main/scripts/install.ps1 | iex", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/NousResearch/nexai-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using nexai Agent! ⚕")
    print()
