"""
Multi-provider authentication system for nexai Agent.

Supports OAuth device code flows (NexAi Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.nexai/auth.json with cross-process file locking.
...
from nexai_cli.config import get_nexai_home, get_config_path, read_raw_config
from nexai_constants import OPENROUTER_BASE_URL
...
# NexAi Portal defaults
DEFAULT_NOUS_PORTAL_URL = "https://portal.example.com"
DEFAULT_NOUS_INFERENCE_URL = "https://api.example.com/v1"
DEFAULT_NOUS_CLIENT_ID = "nexai-cli"
...
SPOTIFY_DOCS_URL = "https://docs.nexai.dev/docs/user-guide/features/spotify"
...
def get_anthropic_key() -> str:
    ...
    """
    from nexai_cli.config import get_env_value
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from nexai_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and ~/.nexai/.env file
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `nexai model` to re-authenticate."

    if error.code == "subscription_required":
        return (
            "No active paid subscription found on NexAi Portal. "
    ...
    if error.code == "insufficient_credits":
        return (
            ...
            "Top up/renew credits in NexAi Portal, then retry."
        )
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("NEXAI_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.nexai/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_nexai_home() / "auth.json"
    # Seat belt: if pytest is running and NEXAI_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch NEXAI_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".nexai" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set NEXAI_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom NEXAI_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_nexai_root()
    except Exception:
        return None
    profile_home = get_nexai_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.nexai/auth.json`` even when NEXAI_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_nexai_root()`` for a profile-shaped NEXAI_HOME can
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".nexai" / "auth.json"
            try:
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``nexai auth add <provider>`` inside the profile, profile entries
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in nexai.
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `nexai logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from nexai_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'nexai doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'nexai model' for available providers, or run 'nexai doctor' to diagnose config issues."
    ...
    raise AuthError(
        "No inference provider configured. Run 'nexai model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.nexai/.env.",
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("NEXAI_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
#
# Tokens live in ~/.nexai/auth/google_oauth.json (managed by agent.google_oauth).
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    """Return a status dict for `nexai auth list` / `nexai status`."""
    try:
...
# Spotify auth — PKCE tokens stored in ~/.nexai/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("NEXAI_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set NEXAI_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("NEXAI_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("NEXAI_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("NEXAI_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `nexai auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `nexai auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `nexai auth spotify` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `nexai auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.nexai/.env, and return it.
    ...
    print("       App name:     anything (e.g. nexai-agent)")
    ...
    # Persist so subsequent `nexai auth spotify` runs skip the wizard.
    save_env_value("NEXAI_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("NEXAI_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved NEXAI_SPOTIFY_CLIENT_ID to ~/.nexai/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "NEXAI_SPOTIFY_CLIENT_ID is required".
    ...
    print()
    print("Open this URL to authorize nexai:")
    print(authorize_url)
    print()
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.nexai/auth.json (not ~/.codex/)
#
# nexai maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from nexai auth store (~/.nexai/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `nexai auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `nexai auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `nexai auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `nexai auth` to re-authenticate.",
            provider="openai-codex",
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to nexai auth store (~/.nexai/auth.json)."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating nexai auth state."""
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `nexai auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to nexai auth store automatically.
    """
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from nexai's own Codex token store."""
    data = _read_codex_tokens()
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("NEXAI_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other nexai processes
    ...
    base_url = (
        os.getenv("NEXAI_CODEX_BASE_URL", "").strip().rstrip("/")
...
# =============================================================================
# NexAi Portal — token refresh, agent key minting, model discovery
# =============================================================================

# -----------------------------------------------------------------------------
# Shared Nous token store — lets OAuth credentials persist across profiles
# so a new `nexai --profile <name> auth add nous --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${NEXAI_SHARED_AUTH_DIR}/nous_auth.json, defaulting to
# ``<nexai-root>/shared/nous_auth.json`` where ``<nexai-root>`` is what
# ``get_default_nexai_root()`` returns — ``~/.nexai`` on Linux/macOS,
# ``%LOCALAPPDATA%\nexai`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's NEXAI_HOME so named profiles (which
# typically live under ``<nexai-root>/profiles/<name>/``) all see the
# same file.
#
...
def _nous_shared_auth_dir() -> Path:
    ...
    Honors ``NEXAI_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<nexai-root>/shared/``, where ``<nexai-root>`` is what
    :func:`nexai_constants.get_default_nexai_root` returns — so
    Linux/macOS classic installs land at ``~/.nexai/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\nexai\\shared\\``, and
    Docker / custom ``NEXAI_HOME`` deployments at
    ``<NEXAI_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("NEXAI_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from nexai_constants import get_default_nexai_root
    return get_default_nexai_root() / "shared"


def _nous_shared_store_path() -> Path:
    path = _nous_shared_auth_dir() / NOUS_SHARED_STORE_FILENAME
    # Seat belt: if pytest is running and this resolves to a path under the
    # real user's nexai root, refuse rather than silently corrupt cross-profile
    # state. Tests must set NEXAI_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from nexai_constants import get_default_nexai_root
        real_home_shared = (
            get_default_nexai_root() / "shared" / NOUS_SHARED_STORE_FILENAME
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                ...
                f"{path}. Set NEXAI_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        # No NEXAI_HOME yet (pre-setup): fall through without locking.
        yield
        return
...
def _refresh_access_token(
    *,
    ...
    # Detect the OAuth 2.1 "refresh token reuse" signal from the NexAi portal
    ...
    # called POST /api/oauth/token with nexai's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, nexai's next refresh uses it, and the whole
    # session chain gets revoked as a token-theft signal (#15099).
    lowered = description.lower()
    if "reuse" in lowered or "reuse detected" in lowered:
        description = (
            "NexAi Portal detected refresh-token reuse and revoked this session.\n"
            "This usually means an external process (monitoring script, "
            "custom self-heal hook, or another nexai install sharing "
            "~/.nexai/auth.json) called POST /api/oauth/token with nexai's "
            "refresh token without persisting the rotated token back.\n"
            "Nous refresh tokens are single-use — only nexai may call the "
            "refresh endpoint. For health checks, use `nexai auth status` "
            "instead.\n"
            "Re-authenticate with: nexai auth add nous"
        )
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip nexai models — they're not reliable for agentic tool-calling
            if "nexai" in mid.lower():
                continue
...
def resolve_nous_access_token(
    *,
    ...
    """Resolve a refresh-aware NexAi Portal access token for managed tool gateways."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
        ...
        if not state:
            raise AuthError(
                "nexai is not logged into NexAi Portal.",
                provider="nous",
                relogin_required=True,
            )
        ...
        portal_base_url = (
            ...
            or os.getenv("NEXAI_PORTAL_BASE_URL")
        ...
        with _nous_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError(
                    "No access token found for NexAi Portal login.",
                    provider="nous",
                    relogin_required=True,
                )
...
def persist_nous_credentials(
    ...
    *,
    ...
    Historically ``nexai auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``nexai auth add nous --label <name>``).  It gets embedded in the
    ...
    # these credentials via `nexai auth add nous --type oauth`. Best-
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("nexai is not logged into NexAi Portal.",
        ...
        with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError("No access token found for NexAi Portal login.",
                                provider="nous", relogin_required=True)
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `nexai auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `nexai auth` and
    # `nexai model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("NEXAI_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("NEXAI_COPILOT_ACP_ARGS", "").strip()
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set NEXAI_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `nexai logout` historically keyed off auth.json.active_provider only.
...
def _prompt_model_selection(
    ...
    """
    from nexai_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from nexai_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'nexai model' or 'nexai setup' instead."""
    print("The 'nexai login' command has been removed.")
    print("Use 'nexai auth' to manage credentials,")
    print("'nexai model' to select a provider, or 'nexai setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.nexai/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing nexai-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in nexai auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("nexai will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("NEXAI_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
                config_path = _update_config_for_provider("openai-codex", base_url)
                print()
                print("Credentials imported. Note: if Codex CLI refreshes its token,")
                print("nexai will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — nexai gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(nexai creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to nexai auth store
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to nexai auth store (~/.nexai/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting nexai login via MiniMax ({region}) OAuth...")
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    if not state or not state.get("access_token"):
        raise AuthError(
            "Not logged into MiniMax OAuth. Run `nexai model` and select "
            "MiniMax (OAuth).",
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting nexai login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("Your NexAi Portal account does not have an active subscription.")
            print(f"  Subscribe here: {portal_url}/billing")
            print()
            print("After subscribing, run `nexai model` again to finish setup.")
            raise SystemExit(1)
        raise


def _login_nous(args, pconfig: ProviderConfig) -> None:
    """NexAi Portal device authorization flow."""
    ...
    try:
        ...
        try:
            ...
            if model_ids:
                ...
                if free_tier:
                    ...
                    # as free so users on older nexai builds still see
        ...
        if not selected_model:
            ...
            print("  Run `nexai model` again to switch to NexAi Portal.")
            return
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("nexai will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `nexai model` or configure an API key to use nexai.")
        else:
