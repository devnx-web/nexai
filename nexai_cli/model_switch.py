...
- ``nexai_cli.providers``        -- canonical provider identity + overlays
- ``nexai_cli.model_normalize``  -- per-provider name formatting
...
def is_nous_nexai_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous nexai 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_NEXAI_NON_AGENTIC_RE.search(model_name))


def _check_nexai_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous nexai 3/4 chat model."""
    if is_nous_nexai_non_agentic(model_name):
        return _NEXAI_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``nexai config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from nexai_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'nexai model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'nexai doctor' — config issues detected:"
    ...
    # Mirrors the same logic in nexai_cli.runtime_provider.resolve_runtime_provider;
    ...
    nexai_warn = _check_nexai_model_warning(new_model)
    if nexai_warn:
        warnings.append(nexai_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from nexai_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from nexai_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # --- 1. Check nexai-mapped providers ---
    for nexai_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(nexai_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (NEXAI_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from nexai_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and nexai_id in store.get("credential_pool", {}):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # show up in the picker without requiring a nexai release.
        model_ids = curated.get(nexai_id, [])
        if nexai_id in _MODELS_DEV_PREFERRED:
            model_ids = _merge_with_models_dev(nexai_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check nexai-only providers (nous, openai-codex, copilot, opencode-go) ---
    from nexai_cli.providers import NEXAI_OVERLAYS
    from nexai_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → nexai provider ID.
    # NEXAI_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use nexai IDs ("copilot").
    _mdev_to_nexai = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in NEXAI_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve nexai slug — e.g. "github-copilot" → "copilot"
        nexai_slug = _mdev_to_nexai.get(pid, pid)
        if nexai_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(nexai_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, nexai_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or nexai_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(nexai_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", nexai_slug, exc)
        ...
        # configured.
        if not has_creds and nexai_slug == "anthropic":
            try:
                ...
                if (nexai_creds and nexai_creds.get("accessToken")) or \
        ...
        if nexai_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = provider_model_ids(nexai_slug)
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                model_ids = _ids if _ids is not None else (curated.get(nexai_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(nexai_slug, []) or curated.get(pid, [])
        else:
            # Use curated list — look up by nexai slug, fall back to overlay key
            model_ids = curated.get(nexai_slug, []) or curated.get(pid, [])
            # Merge with models.dev for preferred providers (same rationale as above).
            if nexai_slug in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(nexai_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": nexai_slug,
            "name": get_label(nexai_slug),
            "is_current": nexai_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "nexai",
        ...
        seen_slugs.add(nexai_slug.lower())
        _record_builtin_endpoint(nexai_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or NEXAI_OVERLAYS (keeps /model in sync
    # with `nexai model`).
    try:
        from nexai_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is nexai's canonical write key (matches
            ...
            # nexai writes ``models:`` as a dict keyed by model id
            # (see nexai_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # nexai's own writer uses; a hyphen variant is accepted
            ...
            # active model. nexai's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, nexai_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`nexai_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from nexai_cli.models import fetch_openrouter_models
