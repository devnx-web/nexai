"""nexai hooks — inspect and manage shell-script hooks.

Usage::

    nexai hooks list
    nexai hooks test <event> [--for-tool X] [--payload-file F]
    nexai hooks revoke <command>
    nexai hooks doctor

Consent records live under ``~/.nexai/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.nexai/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``nexai hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: nexai hooks {list|test|revoke|doctor}")
        print("Run 'nexai hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.nexai/config.yaml.")
        print("See `nexai hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `nexai hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `nexai hooks test` and `nexai hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (nexai wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `nexai hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `nexai hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `nexai hooks doctor`.")
