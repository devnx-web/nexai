...
on Japanese locales, etc.).  nexai's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``nexai_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. nexai is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``NEXAI_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("NEXAI_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the nexai-managed Git install directories so
    ...
    # already-running shells don't see the change — which means nexai
    ...
    # install without nexai-managed PortableGit).
    ...
    # stdin is re-configured for completeness; nexai's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and nexai's
    ``nexai config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching nexai (User env var in Windows Settings, or
    export in a PowerShell profile) and nexai picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known nexai-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\nexai\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``nexai`` from right after install) retain their old PATH.

    Any subprocess nexai spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known nexai-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "nexai", "git", "cmd"),
        os.path.join(local_appdata, "nexai", "git", "bin"),
        os.path.join(local_appdata, "nexai", "git", "usr", "bin"),
        # nexai venv Scripts directory — host of the nexai.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes nexai, but harmless to include.
        os.path.join(local_appdata, "nexai", "nexai-agent", "venv", "Scripts"),
