"""`nexai checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.nexai/checkpoints/``.  Actions:

    nexai checkpoints               # same as `status`
    nexai checkpoints status        # total size, project count, breakdown
    nexai checkpoints list          # per-project checkpoint counts + workdir
    nexai checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    nexai checkpoints clear [-f]    # nuke the entire base (asks first)
    nexai checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    nexai checkpoints
    nexai checkpoints prune --retention-days 3 --max-size-mb 200
    nexai checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: nexai checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``nexai checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `nexai checkpoints` → status
