...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with nexai-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.nexai/plugins/<name>/``
3. **Project plugins** – ``./.nexai/plugins/<name>/`` (opt-in via
   ``NEXAI_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``nexai_agent.plugins``
   entry-point group.
...
# ---------------------------------------------------------------------------
#
# Set ``NEXAI_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.nexai/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("NEXAI_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When NEXAI_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other nexai loggers.
    """
    ...
    logger.debug(
        "NEXAI_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
ENTRY_POINTS_GROUP = "nexai_agent.plugins"

_NS_PARENT = "nexai_plugins"
...
class PluginManifest:
    ...
    #              in ~/.nexai/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``nexai plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``nexai honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``nexai <subcommand>``
    ...
    def register_skill(
        ...
        ``~/.nexai/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        #
        # Repo-shipped plugins live next to nexai_cli/. Two layouts are
        ...
        # 2. User plugins (~/.nexai/plugins/)
        user_dir = get_nexai_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.nexai/plugins/)
        if _env_enabled("NEXAI_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".nexai" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set NEXAI_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with nexai and must
            # just work. Selection among them (e.g. which image_gen backend
            # services calls) is driven by ``<category>.provider`` config,
            # enforced by the tool wrapper.
            #
            # Bundled platform plugins (gateway adapters like IRC) auto-load
            # for the same reason: every platform nexai ships must be
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `nexai plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``nexai_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``nexai_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="nexai-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``nexai tools`` TUI so plugin-provided toolsets appear
