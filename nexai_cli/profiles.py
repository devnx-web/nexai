"""
Profile management for multiple isolated nexai instances.

Each profile is a fully independent NEXAI_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.nexai/profiles/<name>/`` by default.

The "default" profile is ``~/.nexai`` itself — backward compatible,
zero migration needed.

Usage::

    nexai profile create coder          # fresh profile + bundled skills
    nexai profile create coder --clone  # also copy config, .env, SOUL.md, skills
    nexai profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    nexai -p coder chat                 # or via flag
    nexai profile use coder             # set as sticky default
    nexai profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See nexai_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# Marker file written by `nexai profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `nexai
...
# `nexai skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         nexai infrastructure directories that only the default profile
         (``~/.nexai``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_nexai_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.nexai) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "nexai-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "nexai", "default", "test", "tmp", "root", "sudo",
})

# nexai subcommands that cannot be used as profile names/aliases
_NEXAI_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the nexai root, NOT to the current NEXAI_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where NEXAI_HOME points outside
    ``~/.nexai``, profiles live under ``NEXAI_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_nexai_home() / "profiles"


def _get_default_nexai_home() -> Path:
    """Return the default (pre-profile) NEXAI_HOME path.

    In standard deployments this is ``~/.nexai``.
    In Docker/custom deployments where NEXAI_HOME is outside ``~/.nexai``
    (e.g. ``/opt/data``), returns NEXAI_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_nexai_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the nexai installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its NEXAI_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_nexai_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, nexai subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _NEXAI_SUBCOMMANDS:
        return f"'{canon}' conflicts with a nexai subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``nexai profile list`` for an unrelated profile.
    """
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_nexai_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``nexai update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.nexai)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.nexai/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `nexai update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`nexai profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `nexai update`.\n",
                encoding="utf-8",
            )
        except OSError:
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches NEXAI_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``nexai profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "NEXAI_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.nexai).\n"
            "To remove everything, use: nexai uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set NEXAI_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("NEXAI_HOME")
    try:
        os.environ["NEXAI_HOME"] = str(profile_dir)
        from nexai_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["NEXAI_HOME"] = old_home
        elif "NEXAI_HOME" in os.environ:
            del os.environ["NEXAI_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.nexai/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: nexai profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from NEXAI_HOME.

    Returns ``"default"`` if NEXAI_HOME is not set or points to ``~/.nexai``.
    Returns the profile name if NEXAI_HOME points into ``~/.nexai/profiles/<name>``.
    Returns ``"custom"`` if NEXAI_HOME is set to an unrecognized path.
    """
    from nexai_constants import get_nexai_home
    nexai_home = get_nexai_home()
    resolved = nexai_home.resolve()

    default_resolved = _get_default_nexai_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.nexai itself — its parent is ~/ and its
        # directory name is ".nexai", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: nexai profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.nexai itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.nexai). "
            "Specify a different name: nexai profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="nexai_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"nexai.{old_name}"
    new_host = f"nexai.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_nexai_home() / "honcho.json",
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a NEXAI_HOME path string.

    Called early in the CLI entry point, before any nexai modules
    are imported, to set the NEXAI_HOME environment variable.
    """
