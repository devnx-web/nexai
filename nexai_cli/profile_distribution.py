"""Profile distributions — shareable, packaged nexai profiles via git.

A distribution is a nexai profile published as a git repository (or
...
* ``nexai profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``nexai skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``nexai profile``, not a parallel tree):

    nexai profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "nexai-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            nexai_requires=str(data.get("nexai_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.nexai_requires:
            out["nexai_requires"] = self.nexai_requires
        if self.author:
...
def check_nexai_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires nexai {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``nexai profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a nexai profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a nexai distribution."
        )

    # Version check up-front so we fail fast
    check_nexai_requires(manifest.nexai_requires, nexai_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.nexai).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="nexai_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `nexai profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`nexai profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="nexai_dist_update_") as tmp:
        plan = plan_install(
