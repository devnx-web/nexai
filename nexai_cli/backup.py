"""
Backup and import commands for nexai CLI.

`nexai backup` creates a zip archive of the entire ~/.nexai/ directory
(excluding the nexai-agent repo and transient files).

`nexai import` restores from a backup zip, overlaying onto the current
NEXAI_HOME root.
"""
...
from nexai_constants import get_default_nexai_root, get_nexai_home, display_nexai_home
...
_EXCLUDED_DIRS = {
    "nexai-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to nexai root) should be skipped."""
...
def run_backup(args) -> None:
    """Create a zip backup of the nexai home directory."""
    nexai_root = get_default_nexai_root()

    if not nexai_root.is_dir():
        print(f"Error: nexai home directory not found at {nexai_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"nexai-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"nexai-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_nexai_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(nexai_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(nexai_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(nexai_root)

            if _should_exclude(rel):
                continue

            # Skip the output zip itself if it happens to be inside nexai root
            try:
    ...
    print(f"\nRestore with: nexai import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a nexai backup.
    ...
    # Look for telltale files that a nexai home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a nexai backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.nexai/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a nexai dir name
        if prefix in {".nexai", "nexai"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_nexai_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (nexai_root / "config.yaml").exists()
        has_env = (nexai_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has nexai configuration.")
        ...
        nexai_root.mkdir(parents=True, exist_ok=True)
        ...
        for member in members:
            ...
            target = nexai_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(nexai_root.resolve())
            except ValueError:
        ...
        profiles_dir = nexai_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # nexai_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: nexai profile list  (after installing nexai)")

        # Guidance
        print()
        if not (nexai_root / "nexai-agent").is_dir():
            print("Note: The nexai-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: nexai update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  nexai -p {pname} gateway install")

        print("Done. Your nexai configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and nexai backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to NEXAI_HOME).
...
# — `nexai update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(nexai_home: Optional[Path] = None) -> Path:
    home = nexai_home or get_nexai_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    nexai_home: Optional[Path] = None,
) -> Optional[str]:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(nexai_home)
    if not root.exists():
        return []
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(nexai_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for nexai backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_nexai_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, nexai_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``nexai_root`` to ``out_path``.
...
def _pre_update_backup_dir(nexai_home: Optional[Path] = None) -> Path:
    home = nexai_home or get_nexai_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of NEXAI_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<NEXAI_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``nexai update``) should continue even if the backup fails.
    """
    nexai_root = nexai_home or get_default_nexai_root()
    if not nexai_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(nexai_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, nexai_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `nexai claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of NEXAI_HOME under ``backups/`` before a
    ``nexai claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``nexai import <archive>``.  Writes to
    ``<NEXAI_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `nexai import` and the
