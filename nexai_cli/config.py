"""
Configuration management for nexai Agent.

Config files are stored in ~/.nexai/ for easy access:
- ~/.nexai/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.nexai/.env         - API keys and secrets

This module provides:
- nexai config          - Show current configuration
- nexai config edit     - Open config in editor
- nexai config set      - Set a specific value
- nexai config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.nexai/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``nexai logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  nexai config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `nexai plugins enable
    # observability/langfuse` or `nexai tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "NEXAI_LANGFUSE_ENV",
    "NEXAI_LANGFUSE_RELEASE",
    "NEXAI_LANGFUSE_SAMPLE_RATE",
    "NEXAI_LANGFUSE_MAX_CHARS",
    "NEXAI_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from nexai_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("NEXAI_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_nexai_home() / ".managed"
...
def is_managed() -> bool:
    """Check if nexai is running in package-manager-managed mode.

    Two signals: the NEXAI_MANAGED env var (set by the systemd service),
    or a .managed marker file in NEXAI_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade nexai-agent"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "nexai update"


def format_managed_message(action: str = "modify this nexai installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("NEXAI_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this nexai installation is managed by NixOS "
            f"(NEXAI_MANAGED={env_hint}).\n"
            "Edit services.nexai-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this nexai installation is managed by Homebrew "
            f"(NEXAI_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade nexai-agent"
        )

    return (
        f"Cannot {action}: this nexai installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall nexai."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from NEXAI_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, nexai_bin
    or None if container mode is not active, we're already inside the
    container, or NEXAI_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("NEXAI_DEV") == "1":
        return None
    ...
    container_mode_file = get_nexai_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "nexai-agent")
    exec_user = info.get("exec_user", "nexai")
    nexai_bin = info.get("nexai_bin", "/data/current-package/bin/nexai")
...
# Re-export from nexai_constants — canonical definition lives there.
from nexai_constants import get_nexai_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_nexai_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_nexai_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the nexai group can
    share state with the gateway service.

    The mode can be overridden via the NEXAI_HOME_MODE environment variable
    (e.g. NEXAI_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse NEXAI_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("NEXAI_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When nexai runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("NEXAI_CONTAINER") or os.environ.get("NEXAI_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set NEXAI_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into NEXAI_HOME if the user doesn't have one yet."""
...
def ensure_nexai_home():
    """Ensure ~/.nexai directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_nexai_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_nexai_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"NEXAI_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the nexai-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, nexai auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), nexai sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when nexai
        ...
        # bundled nexai image, which drops to the `nexai` user via gosu).
    ...
    "browser": {
        ...
        "camofox": {
            # When true, nexai sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and nexai should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``nexai chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.nexai/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: nexai sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, nexai truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``nexai kanban specify`` (single id or --all). Set a
        ...
        # model; override via `nexai model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `nexai --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `nexai -c` muscle memory.  Default off so existing
        # users aren't surprised.  NEXAI_TUI_RESUME=<id> always wins.
        ...
        # e.g. `model · 68% · ~/projects/nexai`. Per-platform overrides go under
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.nexai/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.nexai/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, nexai feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before nexai auto-pauses the goal and
    ...
    # always goes to ~/.nexai/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${NEXAI_SKILL_DIR} and ${NEXAI_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `nexai curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.nexai/skills/ into
        # ~/.nexai/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `nexai curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for nexai-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *nexai Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (NEXAI_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.nexai/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or NEXAI_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `nexai doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``nexai_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow nexai to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via NEXAI_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `nexai -p <assignee> chat -q ...` for
    ...
    "code_execution": {
        ...
        #     nexai-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.nexai/logs/.
    ...
    # curated model lists for OpenRouter and NexAi Portal from this URL,
    # falling back to the in-repo snapshot on network failure.  Lets us
    # update model picker lists without shipping a nexai-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://docs.nexai.dev/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `nexai model` invocation; network failures
    ...
    # Session storage — controls automatic cleanup of ~/.nexai/state.db.
    ...
    "sessions": {
        ...
        # default of ``nexai sessions prune``.
        "retention_days": 90,
    ...
    # ``nexai update`` behaviour.
    "updates": {
        # Run a full ``nexai backup``-style zip of NEXAI_HOME before every
        # ``nexai update``.  Backups land in ``<NEXAI_HOME>/backups/`` and
        # can be restored with ``nexai import <path>``.  Off by default —
        # on large NEXAI_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<NEXAI_HOME>/lsp/bin/`` on first use.
...
# selection step (NexAi Portal / OpenRouter / Custom endpoint), so this
...
OPTIONAL_ENV_VARS = {
    ...
    "NOUS_BASE_URL": {
        "description": "NexAi Portal base URL override",
        "prompt": "NexAi Portal base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'nexai model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Nous Subscribers only, used to derive vendor hosts, e.g. devnx.com -> firecrawl-gateway.devnx.com",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the nexai auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @nexai:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. NEXAI_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #nexai)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: nexai-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'nexai-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote nexai API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote nexai API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote nexai API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # NEXAI_TOOL_PROGRESS and NEXAI_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.nexai.config`` entries, then checks
    ...
    try:
        ...
        # should never break `nexai update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older nexai versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``nexai_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'nexai setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — nexai won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'nexai doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("NEXAI_HOME", "~/.nexai")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("NEXAI_TOOL_PROGRESS")
            old_mode = get_env_value("NEXAI_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from NEXAI_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from NEXAI_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("NEXAI_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from NEXAI_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$NEXAI_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_nexai_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `nexai plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `nexai curator status` has no
    ...
    #   3. Creates `~/.nexai/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_nexai_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_nexai_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: nexai config set <key> <value>")
    ...
    # metadata.nexai.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _nexai_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.nexai/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.nexai/config.yaml.
    ...
    (which change ``NEXAI_HOME`` and therefore ``get_config_path()``)
    don't collide.
    """
    with _CONFIG_LOCK:
        ensure_nexai_home()
...
#   openai-codex (OAuth — nexai auth) — OpenAI Codex
#   nous         (OAuth — nexai auth) — NexAi Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.nexai/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.nexai/.env.
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real nexai env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.nexai/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.nexai/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.nexai/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.nexai/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to nexai — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known nexai vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.nexai/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ nexai Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  nexai config edit     # Edit config file", Colors.DIM))
    print(color("  nexai config set <key> <value>", Colors.DIM))
    print(color("  nexai setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: nexai config set <key> <value>")
            print()
            print("Examples:")
            print("  nexai config set model anthropic/claude-sonnet-4")
            print("  nexai config set terminal.backend docker")
            print("  nexai config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'nexai config migrate' to add them")
        ...
        print("  nexai config           Show current configuration")
        print("  nexai config edit      Open config in editor")
        print("  nexai config set <key> <value>   Set a config value")
        print("  nexai config check     Check for missing/outdated config")
        print("  nexai config migrate   Update config with new options")
        print("  nexai config path      Show config file path")
        print("  nexai config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``nexai config`` UI so users
