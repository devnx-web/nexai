"""
nexai fallback — manage the fallback provider chain.
...
Subcommands:
  nexai fallback [list]   Show the current fallback chain (default when no subcommand)
  nexai fallback add      Pick provider + model via the same picker as `nexai model`,
                           then append the selection to the chain
  nexai fallback remove   Pick an entry to delete from the chain
  nexai fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.nexai/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from nexai_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `nexai model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  nexai fallback add")
        print()
        return
    ...
    print("  Docs: https://nexai-agent.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `nexai model`, then append the selection to the chain."""
    from nexai_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `nexai model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `nexai fallback list` to view, or `nexai fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``nexai fallback [subcommand]``."""
