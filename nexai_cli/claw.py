"""nexai claw — OpenClaw migration commands.

Usage:
    nexai claw migrate              # Preview then migrate (always shows preview first)
    nexai claw migrate --dry-run    # Preview only, no changes
    nexai claw migrate --yes        # Skip confirmation prompt
    nexai claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    nexai claw migrate --no-backup  # Skip pre-migration snapshot
    nexai claw cleanup              # Archive leftover OpenClaw directories
    nexai claw cleanup --dry-run    # Preview what would be archived
"""
...
from nexai_cli.config import get_nexai_home, get_config_path, load_config, save_config
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "nexai may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a nexai gateway is running with connected platforms.
    ...
    print()
    print_error(
        "nexai gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'nexai stop'.")
    print()
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_nexai.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_nexai", script_path)
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print("Usage: nexai claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to nexai")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'nexai claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → nexai migration."""
    ...
    # migrate-nexai posture (two-phase: run once without secrets, rerun with
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: nexai claw migrate --source /path/to/.openclaw")
        return
    ...
    print_info(f"Target:      {nexai_home}")
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in ~/.nexai/. "
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: nexai claw migrate --yes")
            return
        if not prompt_yes_no("Proceed with migration?", default=True):
            print_info("Migration cancelled.")
            return

    # ── Phase 2b: Pre-apply backup of the nexai home ─────────
    # Delegates to nexai_cli.backup.create_pre_migration_backup(), which
    # shares implementation with the pre-update backup (same exclusion
    # rules, same SQLite safe-copy, zip format) so the archive is
    # restorable with `nexai import`.  Mirrors OpenClaw's
    ...
    if not no_backup:
        try:
            from nexai_cli.backup import create_pre_migration_backup, _format_size
            backup_archive = create_pre_migration_backup(nexai_home=nexai_home)
            if backup_archive:
                size_str = _format_size(backup_archive.stat().st_size)
                print()
                print_success(f"Pre-migration backup: {backup_archive} ({size_str})")
                print_info(f"Restore with: nexai import {backup_archive.name}")
        except Exception as e:
            print()
            print_error(f"Could not create pre-migration backup: {e}")
            print_info(
                "Re-run with --no-backup to skip, or free up disk space under the nexai home."
            )
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: nexai claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: nexai claw cleanup --yes")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  nexai claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  nexai claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  nexai config set OPENROUTER_API_KEY sk-or-v1-...")
