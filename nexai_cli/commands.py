"""Slash command definitions and autocomplete for the nexai CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of nexai config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal nexai works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while nexai is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.nexai/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update nexai Agent to the latest version", "Info",
               gateway_only=True),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`nexai_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/nexai`` subcommand mapping, and
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``nexai skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``nexai skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``nexai skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/nexai <verb>`` subcommand.
    ...
    can still reach them via ``/nexai <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/nexai`` is always reserved as the first entry so the
    legacy ``/nexai <subcommand>`` form keeps working for anything that
    ...
    # Reserve /nexai as the catch-all top-level command.
    entries.append(("nexai", "Talk to nexai or run a subcommand", "[subcommand] [args]"))
    seen.add("nexai")
...
def slack_app_manifest(request_url: str = "https://nexai-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /nexai handler.

    Maps both canonical names and aliases so /nexai bg do stuff works
    the same as /nexai background do stuff.

    Plugin-registered slash commands are included so ``/nexai <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from nexai_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
