"""
Status command for nexai CLI.

Shows the status of all nexai Agent components.
"""
...
from nexai_cli.auth import AuthError, resolve_provider
from nexai_cli.colors import Colors, color
from nexai_cli.config import get_env_path, get_env_value, get_nexai_home, load_config
from nexai_cli.models import provider_label
from nexai_cli.nous_subscription import get_nous_subscription_features
from nexai_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``nexai config``'s
...
def show_status(args):
    """Show status of all nexai Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ nexai Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    nous_label = "logged in" if nous_logged_in else "not logged in (run: nexai auth add nous --type oauth)"
    print(
        f"  {'NexAi Portal':<12}  {check_mark(nous_logged_in)} "
        f"{nous_label}"
    )
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: nexai model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: nexai auth add minimax-oauth)'}"
    )
    ...
    if managed_nous_tools_enabled():
        ...
        if not features.nous_auth_present:
            print("  NexAi Portal   ✗ not logged in")
        else:
            print("  NexAi Portal   ✓ managed tools available")
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: nexai model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from nexai_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'nexai-agent[vercel]')"
    ...
    try:
        from nexai_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   nexai gateway")
    ...
    jobs_file = get_nexai_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_nexai_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'nexai doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'nexai setup' to configure", Colors.DIM))
    print()
