"""
Unified tool configuration for nexai Agent.

`nexai tools` and `nexai setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.nexai/config.yaml under
...
from nexai_cli.nous_subscription import (
...
from nexai_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _NEXAI_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
_DEFAULT_OFF_TOOLSETS = {"moa", "homeassistant", "rl", "spotify", "discord", "discord_admin", "video"}

# Platform-scoped toolsets: only appear in the `nexai tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``nexai tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from nexai_cli.plugins import discover_plugins, get_plugin_toolsets
...
from nexai_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. NEXAI_CUA_DRIVER_VERSION is an
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "NEXAI_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "NEXAI_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "NEXAI_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "NEXAI_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "NEXAI_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``nexai update`` and
      by ``nexai computer-use install --upgrade``.
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS — `nexai update` calls this for every
    ...
    try:
        ...
        if result.returncode == 0 and shutil.which("cua-driver"):
            if verbose:
                ...
                _print_info("      System Settings > Privacy & Security > Accessibility")
                _print_info("      System Settings > Privacy & Security > Screen Recording")
                _print_info("    Both must allow the terminal / nexai process.")
            return True
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_nexai_home()}/nexai-agent && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in nexai-agent directory)")
            return
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.nexai/config.yaml")
        ...
        # Run the full `nexai auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.nexai/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from nexai_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: nexai auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `nexai auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: nexai auth spotify")
        except Exception as exc:
        ...
        try:
            from nexai_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: nexai plugins enable observability/langfuse")
        _print_info("    Restart nexai for tracing to take effect.")
        _print_info("    Verify: nexai plugins list")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "nexai-cli" (which include all _NEXAI_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[nexai-cli, spotify]`` after enabling Spotify via ``nexai
        ...
        # (e.g. "nexai-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
    ...
    # otherwise saving via `nexai tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"nexai-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("nexai-")}
    ...
    # shipped as a bundled plugin but user must opt in via `nexai tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `nexai tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by nexai tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (nexai-cli, nexai-telegram, etc.)
    ...
    # Opening `nexai tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `nexai tools` gets a silent no-op
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `nexai tools` and `nexai setup tools`.
    ...
    print()
    from nexai_constants import display_nexai_home
    print(color(f"  Tool configuration saved to {display_nexai_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'nexai' or gateway restart.", Colors.DIM))
    print()
