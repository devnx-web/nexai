"""
Single source of truth for provider identity in nexai Agent.
...
2. **nexai overlays** — transport type, auth patterns, aggregator flags,
...
# -- nexai overlay ----------------------------------------------------------
# nexai-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class nexaiOverlay:
    """nexai-specific provider metadata layered on top of models.dev."""
...
NEXAI_OVERLAYS: Dict[str, nexaiOverlay] = {
    "openrouter": nexaiOverlay(
    ...
    "qwen-oauth": nexaiOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="NEXAI_QWEN_BASE_URL",
    ),
    "google-gemini-cli": nexaiOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "lmstudio": nexaiOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": nexaiOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": nexaiOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": nexaiOverlay(
    ...
    "kimi-for-coding": nexaiOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": nexaiOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": nexaiOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": nexaiOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": nexaiOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": nexaiOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": nexaiOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": nexaiOverlay(
        transport="openai_chat",
    ...
    "opencode": nexaiOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": nexaiOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "nexai", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. nexai overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + nexai overlay
    ...
    overlay = NEXAI_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + nexai extra
    ...
    if overlay is not None:
        # nexai-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in NEXAI_OVERLAYS
    if provider == "bedrock":
