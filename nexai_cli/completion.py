"""Shell completion script generation for nexai CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_nexai_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# nexai Agent bash completion
# Add to ~/.bashrc:
#   eval "$(nexai completion bash)"

_nexai_profiles() {{
    local profiles_dir="$HOME/.nexai/profiles"
...
_nexai_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_nexai_profiles)" -- "$cur"))
        return
    fi
...
complete -F _nexai_completion nexai
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef nexai
# nexai Agent zsh completion
# Add to ~/.zshrc:
...
_nexai_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.nexai/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.nexai/profiles 2>/dev/null)}}")
    fi
...
_nexai() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_nexai_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'nexai command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# nexai Agent fish completion",
        "# Add to your config:",
        "#   nexai completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __nexai_profiles",
        "    echo default",
        "    if test -d $HOME/.nexai/profiles",
        "        ls $HOME/.nexai/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c nexai -f -s p -l profile"
        " -d 'Profile name' -xa '(__nexai_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__nexai_profiles)' -d 'Profile name'"
                )
