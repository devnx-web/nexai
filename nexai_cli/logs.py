"""``nexai logs`` — view and filter nexai log files.
...
Usage examples::
    ...
    nexai logs                    # last 50 lines of agent.log
    nexai logs -f                 # follow agent.log in real time
    nexai logs errors             # last 50 lines of errors.log
    nexai logs gateway -n 100    # last 100 lines of gateway.log
    nexai logs --level WARNING    # only WARNING+ lines
    nexai logs --session abc123   # filter by session ID substring
    nexai logs --component tools  # only tool-related lines
    nexai logs --since 1h         # lines from the last hour
    nexai logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_nexai_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when nexai runs — try 'nexai chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_nexai_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_nexai_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_nexai_home()}/logs/")
        return

    print(f"Log files in {display_nexai_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'nexai chat' to generate logs)")
