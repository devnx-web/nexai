# Contributing to NexAi Agent

Thank you for contributing to NexAi Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want nexai to work everywhere.
...
Bundled skills (in `skills/`) ship with every nexai install. They should be **broadly useful to most users**:
...
If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `nexai skills install`.
...
```bash
mkdir -p ~/.nexai/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.nexai/config.yaml
touch ~/.nexai/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.nexai/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/nexai" ~/.local/bin/nexai
...
├── cli.py                    # NexAiCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (nexai-cli, nexai-telegram, etc.)
├── nexai_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── nexai_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.nexai/skills/ on install)
...
### User configuration (stored in `~/.nexai/`)

| Path | Purpose |
|------|---------|
| `~/.nexai/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.nexai/.env` | API keys and secrets |
| `~/.nexai/auth.json` | OAuth credentials (Nous Portal) |
| `~/.nexai/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.nexai/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.nexai/state.db` | SQLite session database |
| `~/.nexai/sessions/` | JSON session logs |
| `~/.nexai/cron/` | Scheduled job data |
| `~/.nexai/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`nexai_state.py`) with full-text search and unique session titles. JSON logs go to `~/.nexai/sessions/`.
...
(for example `_NEXAI_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.nexai`:
...
The user may skip setup and keep loading the skill. nexai only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `nexai setup` or update `~/.nexai/.env` locally.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing nexai tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `nexai --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

nexai uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.nexai/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `nexai_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `nexai_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

nexai runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the NexAi wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `nexai_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `nexai_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `nexai_cli/gateway_windows.py::
...
nexai has terminal access. Security matters.
...
2. **Test manually**: Run `nexai` and exercise the code path you changed
