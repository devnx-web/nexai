/**
 * ChatPage — embeds `nexai --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (nexai_cli/web_server.py)                          .
 *          ▼                                                              .
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    typeof window !== "undefined" && !window.__NEXAI_SESSION_TOKEN__
      ? "Session token unavailable. Open this page through `nexai dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const term = new Terminal({
      ...
      // let the inner nexai TUI own transcript history/scroll behavior.
    ...
    // nexai TUI own transcript scrolling.
    //
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[nexai-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // forwarding them into nexai. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2 normal-case">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="nexai-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
...
declare global {
  interface Window {
    __NEXAI_SESSION_TOKEN__?: string;
  }
}
