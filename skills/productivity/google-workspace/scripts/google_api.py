#!/usr/bin/env python3
"""Google Workspace API CLI for nexai Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing nexai-facing JSON contract and falls back to the Python client
...
from _nexai_home import get_nexai_home
...
def _gws_binary() -> str | None:
    override = os.getenv("NEXAI_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for nexai Agent")
