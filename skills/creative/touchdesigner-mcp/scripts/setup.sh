...
NEXAI_HOME_DIR="${NEXAI_HOME:-$HOME/.nexai}"
NEXAI_CFG="${NEXAI_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure nexai config has twozero_td MCP entry ──
if [[ ! -f "$NEXAI_CFG" ]]; then
    echo -e " ${FAIL} nexai config not found at ${NEXAI_CFG}"
    manual_steps+=("Create ${NEXAI_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$NEXAI_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in nexai config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to nexai config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${NEXAI_CFG} manually"); }
    manual_steps+=("Restart nexai session to pick up config change")
fi
