...
## 5. nexai Configuration

### Config location

`$NEXAI_HOME/config.yaml` (defaults to `~/.nexai/config.yaml` when `NEXAI_HOME` is unset)
...
Restart the NexAi session for changes to take effect. The MCP connection is
