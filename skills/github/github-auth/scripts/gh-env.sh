#!/usr/bin/env bash
# GitHub environment detection helper for nexai Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif [ -f "$HOME/.nexai/.env" ] && grep -q "^GITHUB_TOKEN=" "$HOME/.nexai/.env" 2>/dev/null; then
    GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$HOME/.nexai/.env" | head -1 | cut -d= -f2 | tr -d '\n\r')
