...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("NEXAI_HOME", os.path.expanduser("~/.nexai")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("NEXAI_HOME", Path.home() / ".nexai")) / "skills" / "red-teaming" / "godmode"
...
NEXAI_HOME = Path(os.getenv("NEXAI_HOME", Path.home() / ".nexai"))
CONFIG_PATH = NEXAI_HOME / "config.yaml"
PREFILL_PATH = NEXAI_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/nexai models — already uncensored, just needs clean prompt
    "nexai": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "nexai" in model_lower or "nous" in model_lower:
        return "nexai"
...
def _get_current_model() -> tuple:
    """Read current model and provider from nexai config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.nexai/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart nexai for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart nexai for changes to take effect.")
