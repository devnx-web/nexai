...
## 5. GODMODE FAST — nexai 4 405B (Zero Refusal)

**Model:** `nousresearch/nexai-4-405b`
**Strategy:** nexai models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with NexAi Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.nexai/config.yaml`:
