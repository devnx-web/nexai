...
nexai kanban watch --tenant <project-slug>

# Snapshot of the board
nexai kanban list --tenant <project-slug>
nexai kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
nexai kanban stats --tenant <project-slug>

# Visual dashboard (browser)
nexai dashboard

# Inspect a specific task (includes comments + events)
nexai kanban show <task-id>

# Follow a single task's event stream
nexai kanban tail <task-id>
```

Verify available subcommands with `nexai kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `nexai kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `nexai kanban block <id>` then `nexai kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `nexai kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `nexai kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `nexai profile list` (profile exists?), `nexai status` (gateway/dispatcher up?), and verify tenant. |
...
nexai kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
nexai kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(nexai kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
nexai kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
nexai kanban block <task-id>
nexai kanban archive <task-id>

# Diagnose what happened
nexai kanban show <task-id>      # task body, comments, recent events
nexai kanban tail <task-id>      # follow the live event stream
nexai kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `nexai kanban create` for the director
...
while true; do
    clear
    nexai kanban list --tenant <slug>
    echo "---"
    nexai kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `nexai kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `nexai profile list` and start
