...
Maps each role archetype to the NexAi skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public nexai-agent
...
## NexAi skills relevant to video production

### Visual / rendering skills (`nexai-agent/skills/creative/`)
...
### Design / pre-production skills (`nexai-agent/skills/creative/`)
...
### Audio / media skills (`nexai-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`nexai-agent/skills/devops/`)
...
These are **not** nexai skills but external CLIs / APIs that profiles invoke.
...
## Built-in nexai tools for media review

These are native nexai tools — not invoked via terminal but through their own
...
key is present in `~/.nexai/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every nexai profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.nexai/.env` → environment variable.
...
When a new nexai-public video skill ships:
