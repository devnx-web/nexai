"""
nexai MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (nexai-specific extra)

Usage:
    nexai mcp serve
    nexai mcp serve --verbose
...
logger = logging.getLogger("nexai.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using NEXAI_HOME."""
    try:
        from nexai_constants import get_nexai_home
        return get_nexai_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("NEXAI_HOME", Path.home() / ".nexai")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from nexai_constants import get_nexai_home
        directory_file = get_nexai_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("NEXAI_HOME", Path.home() / ".nexai")
...
class EventBridge:
    ...
    This is the nexai equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from nexai_constants import get_nexai_home
            db_file = get_nexai_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("NEXAI_HOME", Path.home() / ".nexai")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the nexai MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "nexai",
        instructions=(
            "nexai Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the nexai MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
