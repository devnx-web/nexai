"""
Email platform adapter for the nexai gateway.

Allows users to interact with nexai by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "nexai-agent" "version" "{_nexai_version}" '
            '"vendor" "DevNX" '
            '"support-email" "noreply@devnx.com")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "nexai Agent")
        ...
        msg_id = f"<nexai-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
