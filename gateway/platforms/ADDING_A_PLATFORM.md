...
There are two ways to add a platform to the NexAi gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.nexai/plugins/` (or under `plugins/platforms/`
...
**zero changes to core nexai code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `nexai gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `nexai_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the NexAi core.
...
And add it to the `nexai-gateway` composite:

```python
"NexAi-gateway": {
    "includes": [..., "NexAi-your-platform"]
}
```
...
## 12. Status Display (`nexai_cli/status.py`)
...
## 13. Gateway Setup Wizard (`nexai_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ nexai_cli/ toolsets.py \
  --include="*.py" -l | sort -u
