...
Override via ``NEXAI_GATEWAY_HTTPX_KEEPALIVE_EXPIRY`` /
``NEXAI_GATEWAY_HTTPX_MAX_KEEPALIVE`` env vars when tuning under load.
"""
...
def platform_httpx_limits() -> "httpx.Limits | None":
    ...
    keepalive_expiry = _env_float(
        "NEXAI_GATEWAY_HTTPX_KEEPALIVE_EXPIRY", _DEFAULT_KEEPALIVE_EXPIRY_S
    )
    max_keepalive = _env_int(
        "NEXAI_GATEWAY_HTTPX_MAX_KEEPALIVE", _DEFAULT_MAX_KEEPALIVE
    )
