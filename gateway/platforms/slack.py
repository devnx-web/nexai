...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        tokens_file = get_nexai_home() / "slack_tokens.json"
        ...
        try:
            ...
            # /model work directly without /nexai prefix). A single regex
            ...
            # manifest (see `nexai slack manifest`). In Socket Mode, Slack
            ...
            from nexai_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/nexai$")

            @self._app.command(_slash_pattern)
            async def handle_nexai_command(ack, command):
            ...
            for _action_id in (
                "nexai_approve_once",
                "nexai_approve_session",
                "nexai_approve_always",
                "nexai_deny",
            ):
            ...
            for _action_id in (
                "nexai_confirm_once",
                "nexai_confirm_always",
                "nexai_confirm_cancel",
            ):
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = f":thread: nexai handoff — *{(name or 'session').strip()[:80]}*"
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own nexai session — matching the per-thread behavior channels already
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "nexai_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "nexai_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "nexai_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "nexai_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "nexai_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "nexai_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "nexai_confirm_once": "once",
            "nexai_confirm_always": "always",
            "nexai_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "nexai_approve_once": "once",
            "nexai_approve_session": "session",
            "nexai_approve_always": "always",
            "nexai_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/nexai <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/nexai
        ...
        if slash_name in {"nexai", ""}:
            # Legacy /nexai <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from nexai_cli.commands import slack_subcommand_map
        ...
        # "Running /cmd…" ack shown by handle_nexai_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/nexai <question>" must produce public replies so
